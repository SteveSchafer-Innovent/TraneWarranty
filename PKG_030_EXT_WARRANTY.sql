...
***********************************************************************************/
	...
	WITH
			...
			(
				SELECT
					...
					FROM
						...
						/* -SS- */
						GLA1
					INNER JOIN UD_030_ACCT_RPT_TYP_XRF ART ON GLA1.R12_ACCOUNT = ART.GL_ACCOUNT -- -SS- ???? issue 70, R12_2_R12
		...
		SELECT
			...
			JOIN PROD_CODE_XREF_RCPO_DR PRODGRP ON GLA.COMPANY = PRODGRP.GL_LEDGER -- -SS- ???? issue 22
				AND PRODGRP.PRODUCT_CATEGORY IS NOT NULL
			LEFT OUTER JOIN R12_TRANE_ACCOUNTS_PS
				/* -SS- OTR */
				PSA ON GLA.R12_ACCOUNT = PSA.R12_ACCOUNT -- -SS- R12_2_R12
				-- -SS- GLA.ACCOUNT = PSA.ACCOUNT
				AND PSA.TRANE_ACCOUNT_IND = 'X'
			...
			JOIN DM_WAR_CSN_RSV_PCT_REF RES_PCT ON CTYPES.CLAIM_TYPE_DESCR = RES_PCT.CLAIM_TYPE AND ETS.EXPENSE_TYPE_DESCR = RES_PCT.EXPENSE_TYPE_DESCR AND SOS.COMPANY_OWNED_IND = RES_PCT.COMPANY_OWNED_IND AND( CASE WHEN CACCT.CUST_CREDIT_CATG_CODE = 'Z1' THEN 'Y' ELSE 'N' END) = RES_PCT.CUST_CREDIT_CATG_CODE
			WHERE
...
***********************************************************************************/
	...
	BEGIN
		...
		SELECT
			...
			LEFT OUTER JOIN TEMP_030_COST_FLOW ON LAG_RULES.LAG_PERIOD = TEMP_030_COST_FLOW.SHIP_TO_TRANS_LAG AND LAG_RULES.GENERATED_YEAR = TEMP_030_COST_FLOW.TRX_YEAR AND LAG_RULES.COUNTRY_INDICATOR = TEMP_030_COST_FLOW.COUNTRY_INDICATOR AND LAG_RULES.GL_ACCOUNT = TEMP_030_COST_FLOW.GL_ACCOUNT;
		COMMIT;
		L_LEVEL := 2;
	EXCEPTION
	WHEN OTHERS THEN
...
***********************************************************************************/
	...
	BEGIN
		...
		SELECT
			...
			WHERE
				TCDP.GL_ACCOUNT = TCRP.GL_ACCOUNT
			...
			FROM
				(
					SELECT
							...
							CASE WHEN COUNT(A.FORECAST_PERIOD) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'yyyy')) < 12 AND A.FORECAST_PERIOD <(RUN_MONTH)
								THEN A.MONTH_RR
							...
							CASE WHEN TO_CHAR(A.FORECAST_PERIOD, 'MON') = 'DEC'
								THEN(SUM(TOTAL_RECOGNIZED_REVENUE) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.PERIOD, 'MMYYYY'), TO_CHAR(A.FORECAST_PERIOD, 'YYYY')))
								ELSE 0
						...
						FROM
							(
								SELECT
										...
										CASE WHEN TO_CHAR(A.FORECAST_PERIOD, 'MON') = 'DEC'
											THEN SUM(A.FORECAST_REVENUE) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'YYYY'))
											ELSE 0
										...
										CASE WHEN A.FORECAST_PERIOD < TRUNC(ADD_MONTHS((A.RUN_MONTH), - 1), 'MM')
											THEN(
												CASE WHEN A.WARRANTY_FIRST_YR_IND IS NULL
													THEN(SUM(A.FORECAST_REVENUE) OVER(PARTITION BY A.GL_ACCOUNT, TRUNC(ADD_MONTHS(A.RUN_MONTH, - 1), 'MM'), A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'YYYY')) *
														CASE WHEN TO_NUMBER(TO_CHAR(A.FORECAST_PERIOD, 'YYYY')) <= TO_NUMBER(TO_CHAR(RUN_MONTH, 'YYYY')) + 2
															THEN NVL(B.SEASONAL_RATE, 0)
															ELSE 1
														END)
			...
			FROM
				(
					SELECT
							...
							CASE WHEN TO_CHAR(A.FORECAST_PERIOD, 'MON') = 'DEC'
								THEN(SUM(TOTAL_RECOGNIZED_COMMISSION) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.JOURNAL_PERIOD, 'MMYYYY'), TO_CHAR(A.FORECAST_PERIOD, 'YYYY')))
								ELSE 0
						...
						FROM
							(
								SELECT
										...
										CASE WHEN TO_CHAR(A.FORECAST_PERIOD, 'MON') = 'DEC'
											THEN SUM(A.FORECAST_COMMISSION) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'YYYY'))
											ELSE 0
										...
										CASE WHEN A.FORECAST_PERIOD < TRUNC(ADD_MONTHS(A.RUN_MONTH, - 1), 'MM')
											THEN(
												CASE WHEN A.WARRANTY_FIRST_YR_IND IS NULL
													THEN(SUM(A.FORECAST_COMMISSION) OVER(PARTITION BY A.GL_ACCOUNT, TRUNC(ADD_MONTHS(A.RUN_MONTH, - 1), 'MM'), A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'YYYY')) *
														CASE WHEN TO_NUMBER(TO_CHAR(A.FORECAST_PERIOD, 'YYYY')) <= TO_NUMBER(TO_CHAR(RUN_MONTH, 'YYYY')) + 2
															THEN NVL(B.SEASONAL_RATE, 0)
															ELSE 1
														END)
