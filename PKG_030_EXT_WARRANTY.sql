...
***********************************************************************************/
	...
	WITH LAG_RULES AS
		...
		FROM
			(SELECT DBO.F_GET_R12_ACCOUNT_STRING('ACCOUNT', NULL, ACCOUNT, NULL, NULL, NULL) AS ACCOUNT,
				COMPANY
			FROM GL_ACCOUNT_SCD
			) GLA
		RIGHT JOIN OTR_TRANE_ACCOUNTS_PS PSA
		ON GLA.ACCOUNT = PSA.ACCOUNT
		CROSS JOIN ACTUATE_SEC_XREF ASX
		...
		WHERE GLA.COMPANY  =(
			CASE
				...
				ELSE ASX.PSGL
			END)
		AND GLA.ACCOUNT           = ART.GL_ACCOUNT
...
***********************************************************************************/
	...
	SELECT COMPANY AS BUSINESS_UNIT,
	...
	LEFT OUTER JOIN ACTUATE_SEC_XREF ASX
	ON GLA.COMPANY = ASX.PSGL
	JOIN PROD_CODE_XREF_RCPO_DR PRODGRP
	ON GLA.COMPANY                = PRODGRP.GL_LEDGER
	AND PRODGRP.PRODUCT_CATEGORY IS NOT NULL
	LEFT OUTER JOIN OTR_TRANE_ACCOUNTS_PS PSA
	ON GLA.ACCOUNT            = PSA.ACCOUNT
	AND PSA.TRANE_ACCOUNT_IND = 'X'
		...
		FROM(
			...
			FROM
				...
				FROM
					...
					FROM
						(SELECT DBO.F_GET_R12_ACCOUNT_STRING('ACCOUNT', NULL, GL_ACCOUNT, NULL, NULL, NULL) AS GL_ACCOUNT,
							COUNTRY_INDICATOR,
							G_RUN,
							JOURNAL_DATE,
							REVENUE_AMOUNT
