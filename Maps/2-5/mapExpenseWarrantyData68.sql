		...
		(
			...
			SELECT
				...
				INNER JOIN TIME_DAY TD        ON TD.TIME_KEY = MLR.CCN_TRX_DATE_KEY AND TD.TIME_KEY = LR.CCN_TRX_DATE_KEY
					/* -SS- */
				INNER JOIN R12_GL_ACCOUNT GLA ON GLA.GL_ACCOUNT_SCD_KEY = MLR.GL_ACCOUNT_SCD_KEY
				WHERE
					1 = 2
				...
				INNER JOIN
					(SELECT DISTINCT CLAIM_NBR, DETAIL_NBR, STEP_NBR, APPR_AMT, CCN_TRX_DATE_KEY FROM WC_LABOR_ROLLUP LR1 WHERE CLAIM_TYPE_SCD_KEY = 11 AND EXISTS
								(SELECT 'X' FROM R12_GL_ACCOUNT_SCD
											/* -SS- */
										WHERE LR1.GL_ACCOUNT_SCD_KEY = GL_ACCOUNT_SCD_KEY AND(ACCOUNT LIKE '0620%' OR ACCOUNT LIKE '8062%')
								)
...
UNION
		...
		(
			SELECT
				...
				INNER JOIN PROD_CODE_XREF_RCPO_DR PRODGRP ON PCS.PROD_CODE = PRODGRP.ORA_PRODUCT AND GLA.R12_ENTITY = PRODGRP.GL_LEDGER -- -SS- MANF_PROD_CODE, COMPANY, FIXME neither join with work
					/* -SS- ,ACTUATE_SEC_XREF ASX */
				...
				LEFT OUTER JOIN DM_WAR_CSN_RSV_PCT_REF RES_PCT1 ON MLR.CLAIM_NBR = A.CLAIM_NUMBER
				LEFT OUTER JOIN UD_031_STDWTY_RSV_CLM_ADJ A     ON A.CLAIM_TYPE = RES_PCT1.CLAIM_TYPE
				WHERE
					1 = 2
