...
SELECT
		...
		CASE        WHEN PS.CURRENCY_CODE = 'USD'
			...
			ELSE - 10
		END)) AS DEPT_ID,
		COALESCE(DP.R12_LOCATION_DESCR,
		CASE                          WHEN PS.CURRENCY_CODE = 'USD'
		...
		CASE        WHEN PS.R12_ENTITY IN('5773', '5588')
			THEN 'CAD' WHEN PS.R12_ENTITY IN('5575', '5612', '5743', '9256', '9258', '9298', '9299')
			THEN 'USD'
			ELSE '???'
	...
	LEFT OUTER JOIN R12_LOCATION_STG DP        ON DP.R12_LOCATION = PS.R12_LOCATION
	WHERE
...
SELECT
		...
		CASE        WHEN L.R12_ENTITY IN('5773', '5588')
			THEN 'CAD' WHEN L.R12_ENTITY IN('5575', '5612', '5743', '9256', '9258', '9298', '9299')
			THEN 'USD'
			ELSE '???'
		END AS COUNTRY_INDICATOR,
		'N' AS TWO_FIVE
		...
		(
			SELECT
					...
					CASE        WHEN D.R12_ENTITY IN('5773', '5588')
						THEN 'CAD' WHEN D.R12_ENTITY IN('5575', '5612', '5743', '9256', '9258', '9298', '9299')
						THEN 'USD'
						ELSE '???'
					END AS NATION_CURR
				FROM
				...
				INNER JOIN R12_BI_HDR_PSB BH              ON 1 = 1 AND BH.BILL_SOURCE_ID = 'PBS' AND D.INVOICE = BH.INVOICE AND D.BUSINESS_UNIT = BH.PS_BUSINESS_UNIT AND D.CUSTOMER_TRX_ID = BH.CUSTOMER_TRX_ID
				INNER JOIN R12_TRNBI_BI_HDR_PSB TBH       ON 1 = 1 AND '7' = TBH.TRNBI_PROJECT_TYPE AND D.INVOICE = TBH.INVOICE AND D.BUSINESS_UNIT = TBH.BUSINESS_UNIT AND D.CUSTOMER_TRX_ID = TBH.CUSTOMER_TRX_ID
		...
		(
			...
			*/
			...
			SELECT
				...
				INNER JOIN R12_BI_ACCT_ENTRY_PSB D        ON D.LINE_SEQ_NUM = A.LINE_SEQ_NUM AND D.INVOICE = A.INVOICE AND D.BUSINESS_UNIT = A.BUSINESS_UNIT AND D.CUSTOMER_TRX_ID = A.CUSTOMER_TRX_ID
				INNER JOIN R12_ACCOUNT_FILTER_UPD AFU     ON AFU.R12_ACCOUNT = D.R12_ACCOUNT
				INNER JOIN R12_BI_HDR_PSB BH              ON 1 = 1 AND BH.BILL_SOURCE_ID = 'P21' AND D.INVOICE = BH.INVOICE AND D.BUSINESS_UNIT = BH.PS_BUSINESS_UNIT AND D.CUSTOMER_TRX_ID = BH.CUSTOMER_TRX_ID
				INNER JOIN R12_TRNBI_BI_HDR_PSB TBH       ON 1 = 1 AND '7' = TBH.TRNBI_PROJECT_TYPE AND D.INVOICE = TBH.INVOICE AND D.BUSINESS_UNIT = TBH.BUSINESS_UNIT AND D.CUSTOMER_TRX_ID = TBH.CUSTOMER_TRX_ID
				LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO X ON D.PS_PRODUCT = X.MANF_PROD_CODE AND D.BUSINESS_UNIT = X.GL_LEDGER -- (+)
	...
	FROM
		(
			SELECT
				...
				INNER JOIN R12_ACCOUNT_FILTER_UPD AFU     ON AFU.R12_ACCOUNT = D.R12_ACCOUNT
				INNER JOIN R12_BI_HDR_STG BH              ON D.INVOICE = BH.INVOICE AND D.BUSINESS_UNIT = BH.PS_BUSINESS_UNIT AND D.CUSTOMER_TRX_ID = BH.CUSTOMER_TRX_ID
...
SELECT
		...
		CASE        WHEN PR.R12_ENTITY IN('5773', '5588')
			THEN 'CAD' WHEN PR.R12_ENTITY IN('5575', '5612', '5743', '9256', '9258', '9298', '9299')
			THEN 'USD'
			ELSE '???'
		END AS COUNTRY_INDICATOR,
		'N' AS TWO_FIVE
