...
SELECT
  /*+ NO_CPU_COSTING */
  to_date('1-'||:RunDate, 'dd-mon-yy') AS gl_BeginDate, LAST_DAY(to_date('1-'||:RunDate, 'dd-mon-yy')) gl_End_Date,
  CASE
  ...
  CASE
    ...
    THEN a.GL_ACC_DESCR
    ELSE B.GL_ACC_DESCR
  END AS GL_ACC_DESCR, NVL(B.Amort_Comm_and_prepaid_comm, 0) AS Amort_Comm_and_prepaid_comm, NVL(C.SHORT_TERM_COMM, 0) AS SHORT_TERM_COMM, NVL(C.LONG_TERM_COMM, 0) AS LONG_TERM_COMM
FROM
  (SELECT
    /*+ NO_CPU_COSTING */
    TO_DATE('1-'||:RunDate, 'dd-mon-yy') AS gl_BeginDate, LAST_DAY(to_date('1-'||:RunDate, 'dd-mon-yy')) AS gl_End_Date,
  ...
  FROM
    (SELECT a.country_indicator, a.gl_account, A.GL_ACCOUNT_DESCR, to_date('1-'||:RunDate, 'dd-mon-yy'), (MAX(a.Comm_amort_mnthly) +
      CASE
        ...
        ELSE 0
      END) AS Amort_Comm_and_prepaid_comm, A.forecast_period
  ...
  SELECT
    /*+ NO_CPU_COSTING */
    TRUNC(TO_DATE(TO_DATE('1-'||:RunDate, 'dd-mon-yy')), 'YEAR') - 1 AS gl_BeginDate, LAST_DAY(to_date('1-'||:RunDate, 'dd-mon-yy')) gl_End_Date, B.country_indicator, B.gl_account AS account, 0 AS DOLLAR_AMOUNT, B.GL_ACCOUNT_DESCR AS GL_ACC_DESCR, SUM(b.SHORT_TERM_COMM) AS SHORT_TERM_COMM, SUM(B.LONG_TERM_COMM) AS LONG_TERM_COMM
  FROM
    (SELECT a.country_indicator, a.gl_account, A.GL_ACCOUNT_DESCR, to_date('1-'||:RunDate, 'dd-mon-yy'), MAX(a.short_term_pp_comm) AS SHORT_TERM_COMM, MAX(a.long_term_pp_comm) AS LONG_TERM_COMM, A.forecast_period
      /*TAY:            from DM_030_COMM_AMORTIZATION@DW_INTFC_DR.LAX.TRANE.COM a,OTR_TRANE_ACCOUNTS_PS psa*/
    FROM DW_DM_030_COMM_AMORTIZATION a
      -- -SS- NEW
    INNER JOIN R12_ACCOUNT_FILTER_UPD AFU
