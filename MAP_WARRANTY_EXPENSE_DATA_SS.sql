SELECT TO_CHAR(SYSDATE, 'MM-DD-YYYY HH24:MI:SS') "START LOAD WARR EXPENSE DATA" FROM DUAL;
COMMIT;
ALTER TABLE MAP_WARRANTY_EXPENSE_DATA_SS NOLOGGING;
TRUNCATE TABLE MAP_WARRANTY_EXPENSE_DATA_SS
DROP STORAGE;
COMMIT;
-- SECTION 1 - pre-load
...
TRUNCATE
		TABLE MAP_CCN_DATA;
COMMIT;
SELECT TO_CHAR(SYSDATE, 'MM-DD-YYYY HH24:MI:SS') "START SECTION 1 - PRELOAD 1" FROM DUAL;
INSERT /*+ APPEND */
	INTO MAP_CCN_DATA
SELECT DISTINCT
		/*+  no_cpu_costing */
...
SELECT
		TO_CHAR(SYSDATE, 'MM-DD-YYYY HH24:MI:SS') "START SECTION 1 - PRELOAD 2"
	FROM
		DUAL;
INSERT /*+ APPEND */
	INTO MAP_CCN_DATA
...
SELECT
		TO_CHAR(SYSDATE, 'MM-DD-YYYY HH24:MI:SS') "START SECTION 1"
	FROM
		DUAL;
INSERT /*+ APPEND */
	INTO MAP_WARRANTY_EXPENSE_DATA_SS
SELECT DISTINCT
		/*+  no_cpu_costing */
		...
		CASE WHEN A.CLAIM_NUMBER IS NULL
			THEN 10000 *(
				CASE WHEN(PCS.PROD_CODE IN('0054', '0197')) OR(NVL(FCW.WA_RANGE, '0') NOT IN('2', '5')) OR(ROUND((CCN_DATA.TRX_FULL_DATE - TD2.FULL_DATE) / 30.42) > 91) OR(ROUND((TD.FULL_DATE - TD2.FULL_DATE) / 30.42) > 24)
					THEN 0
	...
	INNER JOIN PROD_CODE_XREF_RCPO_DR PRODGRP       ON PCS.PROD_CODE = PRODGRP.MANF_PROD_CODE AND 'CSD' = PRODGRP.GL_LEDGER -- SR-r12???-- -SS- issues 55, 56 -- -SS- issue 22
...
SELECT
		TO_CHAR(SYSDATE, 'MM-DD-YYYY HH24:MI:SS') "START SECTION 2"
	FROM
		DUAL;
...
SELECT DISTINCT
		...
		CASE WHEN A.CLAIM_NUMBER IS NULL
			THEN 10000 *(
				CASE WHEN(PCS.PROD_CODE IN('0054', '0197')) OR(NVL(FCW.WA_RANGE, '0') NOT IN('2', '5')) OR(ROUND((MIN(TD3.FULL_DATE) - MIN(TD2.FULL_DATE)) / 30.42) > 91) OR(ROUND((MIN(TD.FULL_DATE) - MIN(TD2.FULL_DATE)) / 30.42) > 24)
					THEN 0
					ELSE RES_PCT.RESERVE_PCT
				END)
			ELSE RES_PCT1.RESERVE_PCT
	...
	INNER JOIN PROD_CODE_XREF_RCPO_DR PRODGRP       ON PCS.PROD_CODE = PRODGRP.MANF_PROD_CODE -- STEVE
...
SELECT
		TO_CHAR(SYSDATE, 'MM-DD-YYYY HH24:MI:SS') "START SECTION 3 - PRELOAD"
	FROM
		DUAL;
TRUNCATE TABLE MAP_CCN_DATA;
COMMIT;
INSERT /*+ APPEND */
	INTO MAP_CCN_DATA
SELECT
	...
	WHERE
		...
		OR GLA.PS_ACCOUNT LIKE '8062%') ;
COMMIT;
SELECT COUNT( *) FROM MAP_CCN_DATA;
-- SECTION 3 - requires pre-load /*SPD, Retrofit Labor, Extended Purchase Labor   */
SELECT
		TO_CHAR(SYSDATE, 'MM-DD-YYYY HH24:MI:SS') "START SECTION 3"
	FROM
		DUAL;
INSERT /*+ APPEND */
	INTO MAP_WARRANTY_EXPENSE_DATA_SS
SELECT
	...
	INNER JOIN CUST_ACCOUNT_SCD CACCT               ON CCN_DATA.CUST_ACCOUNT_SCD_KEY = CACCT.CUST_ACCOUNT_SCD_KEY
	INNER JOIN SUBMIT_OFFICE_SCD SOS                ON CCN_DATA.SUBMIT_OFFICE_SCD_KEY = SOS.SUBMIT_OFFICE_SCD_KEY
	INNER JOIN PROD_CODE_XREF_RCPO_DR PRODGRP       ON PCS.PROD_CODE = PRODGRP.MANF_PROD_CODE AND 'CSD' = PRODGRP.GL_LEDGER -- -SS- issues 55, 56 -- -SS- issue 22 -- SR-r12???
	INNER JOIN DM_WAR_CSN_RSV_PCT_REF RES_PCT       ON CCN_DATA.EXPENSE_TYPE_DESCR = RES_PCT.EXPENSE_TYPE_DESCR AND CCN_DATA.EXPENSE_TYPE_CATG = UPPER(RES_PCT.EXPENSE_TYPE_CATG) AND SOS.COMPANY_OWNED_IND = RES_PCT.COMPANY_OWNED_IND
	LEFT OUTER JOIN UD_031_STDWTY_RSV_CLM_ADJ A     ON CCN_DATA.CLAIM_NBR = A.CLAIM_NUMBER
	LEFT OUTER JOIN DM_WAR_CSN_RSV_PCT_REF RES_PCT1 ON A.CLAIM_TYPE = RES_PCT1.CLAIM_TYPE
	WHERE
		...
		END) = RES_PCT.CUST_CREDIT_CATG_CODE
		AND PRODGRP.PRODUCT_CATEGORY IS NOT NULL
		--AND CCN_DATA.CLAIM_NBR = 8870254
		;
COMMIT;
SELECT TO_CHAR(SYSDATE, 'MM-DD-YYYY HH24:MI:SS') "FINISH" FROM DUAL;
COMMIT;
