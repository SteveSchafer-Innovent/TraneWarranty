...
SELECT /*+ NO_CPU_COSTING */
 ...
 CASE WHEN sales.COUNTRY_INDICATOR IS NULL
      THEN begbalances.COUNTRY_INDICATOR ELSE sales.COUNTRY_INDICATOR END AS COUNTRY_INDICATOR,
 CAST(begbalances.ACCOUNT AS NUMBER) ACCOUNT,
 CASE WHEN sales.DESCR IS NULL THEN begbalances.DESCR ELSE sales.DESCR END AS GL_ACC_DESCR,
 SUM(NVL(begbalances.begbal_base,0)) *-1 AS Begning_Balance,
 SUM(NVL(perioddata.EndBal_base,0))  *-1 AS END_Blance,
 CASE WHEN sales.REVENUE_AMOUNT IS NULL THEN 0 ELSE sales.REVENUE_AMOUNT END AS REVENUE_AMOUNT,
...
FROM
 (/* Begning Balance DRTRNP */
  SELECT /*+ NO_CPU_COSTING */
   ...
   CASE WHEN gl_ledgers.ledger_id = 2022 THEN 'USA'
        WHEN gl_ledgers.ledger_id = 2041 THEN 'CAN' END AS COUNTRY_INDICATOR,
   ...
   psa.DESCR
    ...
    ||gl_ledgers.name
    ||')' AS DESCR
  FROM SY_120_GL_LEDGERS_EW gl_ledgers, SY_120_GL_BALANCES_EW gl_balances, SY_120_GL_CODE_COMBO_EW gl_code_combinations,
/*TAY:   OTR_TRANE_ACCOUNTS_PS psa,*/
  ...
  WHERE gl_balances.period_name=:RunDate
    AND CASE WHEN gl_ledgers.ledger_id = 2022 THEN 'USA' ELSE 'CAN' END = UPPER(:COUNTRY)
...
/*TAY:    AND Cross_Ref.PeopleSoft_ac (+)    = PSA.ACCOUNT WIP*/
  ...
  GROUP BY Cross_Ref.PeopleSoft_ac, gl_ledgers.ledger_id, gl_balances.period_name,
           CASE WHEN gl_ledgers.ledger_id=2022 THEN 'USA' WHEN gl_ledgers.ledger_id =2041 then 'CAN' END, 
           psa.DESCR
            || ' - (R12 A/C : '
            ||gl_code_combinations.segment4
            ||'-'
...
/*TAY:   OTR_TRANE_ACCOUNTS_PS psa,*/
    R12_TRANE_ACCOUNTS_PS psa,
    (SELECT a.BUSINESS_UNIT PS_BU , A.ORACLE_XREF_VALUE Oracle_BU
     FROM dbo.ps_trane_R12_xref a
     WHERE Recname_xref  IN ('ENTITY')
       AND a.BUSINESS_UNIT IN ('CAN','CSD')
       AND a.ps_attribute1  = ' '
...
/*TAY:    AND Cross_Ref.PeopleSoft_ac(+)     = PSA.ACCOUNT WIP*/
    AND Cross_Ref.PeopleSoft_ac(+)     = PSA.PS_ACCOUNT
    AND PSA.TRANE_ACCOUNT_IND          = 'X'
  GROUP BY Cross_Ref.PeopleSoft_ac, gl_ledgers.ledger_id, gl_balances.period_name, gl_balances.period_name,
           CASE WHEN gl_ledgers.ledger_id=2022 THEN 'USA'
                WHEN gl_ledgers.ledger_id =2041 then 'CAN' END,
           psa.DESCR
            || ' - (R12 A/C : '
            ||gl_code_combinations.segment4
            ||'-'
...
/*TAY:  FROM OTR_BI_ACCT_ENTRY_PSB A, OTR_TRNBI_BI_HDR_PSB B, OTR_BI_HDR_PSB C, OTR_TRANE_ACCOUNTS_PS psa, ACTUATE_SEC_XREF ASX*/
  FROM R12_BI_ACCT_ENTRY_PSB A, R12_TRNBI_BI_HDR_PSB B, R12_BI_HDR_PSB C, R12_TRANE_ACCOUNTS_PS psa --, ACTUATE_SEC_XREF ASX
  WHERE A.JOURNAL_DATE >= (to_date('1-'
        ||:RunDate,'dd-mon-yy'))
    AND A.JOURNAL_DATE<= (LAST_DAY(to_date('1-'
        ||:RunDate,'dd-mon-yy')))
/*TAY:    AND A.BUSINESS_UNIT_GL IN ('CAN', 'CSD')*/
    AND A.PS_BUSINESS_UNIT_GL IN ('CAN', 'CSD')
/*TAY:       AND CASE WHEN ASX.NATION_CURR ='USD' THEN 'USA' ELSE 'CAN' END = UPPER(:COUNTRY)*/
    AND CASE WHEN A.r12_entity IN (5773, 5588) THEN 'CAN' ELSE 'USA' END = UPPER(:COUNTRY)
/*TAY:    AND A.BUSINESS_UNIT_GL   = ASX.PSGL(+)*/
    --AND A.BUSINESS_UNIT_GL   = ASX.PSGL(+)
/*TAY:    AND a.ACCOUNT            = PSA.ACCOUNT (+)*/
    AND A.PS_ACCOUNT            = PSA.PS_ACCOUNT (+)
    AND PSA.TRANE_ACCOUNT_IND='X'
    AND A.BUSINESS_UNIT      = B.BUSINESS_UNIT
    AND A.INVOICE            = B.INVOICE
/*TAY:    AND B.BUSINESS_UNIT      = C.BUSINESS_UNIT*/
    AND B.BUSINESS_UNIT      = C.PS_BUSINESS_UNIT
...
/*TAY:  GROUP BY ASX.NATION_CURR, A.ACCOUNT, PSA.DESCR*/
  GROUP BY A.r12_entity, A.PS_ACCOUNT, PSA.DESCR
 ) sales,
...
/*TAY:        FROM DBO.DM_030_REV_RELEASE@DW_INTFC_DR.LAX.TRANE.COM a, OTR_TRANE_ACCOUNTS_PS psa*/
        FROM DW_DM_030_REV_RELEASE a, R12_TRANE_ACCOUNTS_PS psa
...
/*TAY:          AND a.gl_account LIKE '5%' WIP*/
          ...
          AND A.FORECAST_PERIOD >=
               CASE WHEN to_date('1-' ||:RunDate,'dd-mon-yy') = 
                         TRUNC(TO_DATE(TO_DATE('1-'||:RunDate,'dd-mon-yy')),'YEAR')
                    THEN TRUNC(TRUNC(to_date('1-'||:RunDate,'dd-mon-yy'),'YEAR') -1 )-30
                    ELSE TRUNC(TO_DATE(TO_DATE('1-'||:RunDate,'dd-mon-yy')),'YEAR') END
 ...
 (/* DEFERRED*/
  ...
  FROM (SELECT a.gl_account,
         ...
         (MAX(a.rec_rev_mnthly) + CASE WHEN A.FORECAST_PERIOD = to_date('1-' ||:RunDate,'dd-mon-yy') THEN MAX(A.DEFERRED_REVENUE)
                                       ELSE 0 END ) AS DEFERRED_REVENUE,
         A.FORECAST_PERIOD
/*TAY:        FROM DBO.DM_030_REV_RELEASE@DW_INTFC_DR.LAX.TRANE.COM a, OTR_TRANE_ACCOUNTS_PS psa*/
        FROM DW_DM_030_REV_RELEASE a, R12_TRANE_ACCOUNTS_PS psa
/*TAY:        WHERE a.gl_account       = PSA.ACCOUNT (+) WIP*/
...
GROUP BY CAST(begbalances.ACCOUNT AS NUMBER),
         CASE WHEN sales.COUNTRY_INDICATOR IS NULL THEN begbalances.COUNTRY_INDICATOR ELSE sales.COUNTRY_INDICATOR END,
         CASE WHEN sales.DESCR IS NULL THEN begbalances.DESCR ELSE sales.DESCR END, begbalances.begbal_base ,
         CASE WHEN sales.REVENUE_AMOUNT IS NULL THEN 0 ELSE sales.REVENUE_AMOUNT END,
         deff.DEFERRED_REVENUE, Rev.SHORT_TERM_BALA, Rev.LONG_TERM_BALA
UNION
...
/*TAY: CAST (PSA.ACCOUNT AS NUMBER )AS ACCOUNT, WIP*/
...
/*TAY  AND PSA.ACCOUNT LIKE '5%' WIP*/
  ...
  AND NOT EXISTS (SELECT 'x'
                  FROM SY_120_GL_LEDGERS_EW gl_ledgers, SY_120_GL_BALANCES_EW gl_balances, 
...
/*TAY:                    AND Cross_Ref.PeopleSoft_ac        = PSA.ACCOUNT*/
                    AND Cross_Ref.PeopleSoft_ac        = PSA.PS_ACCOUNT
                    AND PSA.TRANE_ACCOUNT_IND          ='X'
                  GROUP BY Cross_Ref.PeopleSoft_ac, psa.DESCR, gl_balances.period_name, gl_ledgers.ledger_id,
                           CASE WHEN gl_ledgers.ledger_id = 2022 THEN 'USA' WHEN gl_ledgers.ledger_id = 2041 then 'CAN' END
                 )
