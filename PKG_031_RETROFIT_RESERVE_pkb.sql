...
***********************************************************************************/
	...
	BEGIN
		...
		FROM
			...
			INNER JOIN DM_FAL_CLAIMS_WARRANTY_XRF FCW
			...
			INNER JOIN PROD_CODE_XREF_RCPO_DR PRODGRP
			...
			INNER JOIN UD_031_RETROFIT_RULES RES_PCT
			...
			INNER JOIN UD_031_RETROFIT_ID RD
			ON MLR.RETRO_ID = RD.RETROFIT_ID
				-- -SS- NEW
			INNER JOIN R12_ACCOUNT_FILTER_UPD AFU
			ON AFU.R12_ACCOUNT = GLA.R12_ACCOUNT -- -SS- R12_2_R12
				-- -SS- /NEW
			LEFT OUTER JOIN UD_031_STDWTY_RSV_CLM_ADJ a
			ON MLR.CLAIM_NBR = a.CLAIM_NUMBER
			LEFT OUTER JOIN DM_WAR_CSN_RSV_PCT_REF RS_RES_PCT --TTP#12554
			ON a.claim_type           = RS_RES_PCT.claim_type
			WHERE MLR.CLAIM_TYPE_SCD_KEY = 3
			...
			UNION ALL
			--Retrofit Labor
			SELECT CCN_DATA.CLAIM_NBR AS CLAIM_NUMBER,
			...
			FROM
				...
				SELECT 'SPD/Retrofit Labor/Extended Purchased Labor' AS TYPE,
					LR.CLAIM_NBR,
					LR.RETRO_ID,
				...
				INNER JOIN TIME_DAY TD
				ON TD.TIME_KEY = LR.CCN_TRX_DATE_KEY
				INNER JOIN CLAIM_TYPE_SCD CT
				ON LR.CLAIM_TYPE_SCD_KEY = CT.CLAIM_TYPE_SCD_KEY
				INNER JOIN R12_GL_ACCOUNT_SCD GLA -- -SS-
		...
		WHERE TRX_DATE BETWEEN TRUNC(ADD_MONTHS(
			CASE
...
***********************************************************************************/
	...
	BEGIN
		...
		SELECT DM_031_RETROFIT_RSV_SEQ.NEXTVAL AS DM_031_RETROFIT_RSV_KEY,
			RUN_PERIOD,
			SHIP_PERIOD,
			COUNTRY_INDICATOR,
			CLAIM_AMT,
		...
		FROM
			...
			FROM DR_SY_031_SALES_SUM_STG SSS,
				(SELECT
					CASE
						WHEN COUNTRY_INDICATOR = 'USA' THEN 'USD'
						WHEN COUNTRY_INDICATOR = 'CAN' THEN 'CAD'
					END COUNTRY_INDICATOR,
					TRX_PERIOD,
					NVL(SUM(IN_RSV_EXPENSE_AMT), 0) AS IN_RSV_EXPENSE_AMT
				FROM dbo.SY_031_CLAIM_SUM_STG
				GROUP BY
					CASE
						WHEN COUNTRY_INDICATOR = 'USA' THEN 'USD'
						WHEN COUNTRY_INDICATOR = 'CAN' THEN 'CAD'
			...
			WHERE SSS.SHIP_PERIOD     = CSS.TRX_PERIOD (+)
			AND SSS.COUNTRY_INDICATOR = CSS.COUNTRY_INDICATOR (+)
			GROUP BY TO_NUMBER(TO_CHAR(
				CASE
					WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM')
					ELSE G_RUN
				END, 'YYYYMM')),
...
***********************************************************************************/
	...
	BEGIN
		l_level := 1;
		DELETE
		FROM DBO.DM_031_RETROFIT_RSV
		WHERE TO_DATE((SUBSTR(RUN_PERIOD, 5, 6) ||'/01/'||SUBSTR(RUN_PERIOD, 1, 4)), 'MM/DD/YYYY') = TRUNC(
			CASE
				WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM')
				ELSE G_RUN
			END) ;
...
***********************************************************************************/
	...
	BEGIN
		...
		DELETE
		FROM DBO.DM_031_RETROFIT_RSV
		WHERE TO_DATE((SUBSTR(RUN_PERIOD, 5, 6) ||'/01/'||SUBSTR(RUN_PERIOD, 1, 4)), 'MM/DD/YYYY') < ADD_MONTHS(
			CASE
				WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM')
				ELSE G_RUN
			END, - 13) ;
