...
rctl.interface_line_attribute5 claim_type,
...
DECODE(rbs.NAME,'HVAC TAVANT',rct.interface_header_attribute1,NULL) Claim_number,
(SELECT   mic.segment6
          FROM mtl_item_categories_v mic, mtl_system_items msi1
         WHERE mic.inventory_item_id = msi1.inventory_item_id
           AND msi1.segment1 =
           ...
           AND mic.organization_id = msi1.organization_id
           AND msi1.organization_id = NVL (rctl.warehouse_id, 99)
           AND ROWNUM = 1) Prod_code,
           hca_billto.customer_type
,rctl.quantity_credited Quantity
,msi.description R12_ITEM_DESCCRIPTION
  ...
  FROM ra_customer_trx_all rct,
      ...
      --END added by Anand Pingle for EC customer derivation
       ...
       gl_ledgers gll,
        ...
        AND NVL(al.attribute1,'XX') IN ('INTERNATIONAL')) all_source
