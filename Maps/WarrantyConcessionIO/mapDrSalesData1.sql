SELECT
	...
	FROM
		OTR_ORACLE_PS_REV_RCPO PS -- SS FIXME not changed to OTR
	LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO PX ON PS.PLNT_GL_BU = PX.GL_LEDGER     -- (+)
		AND PS.PLNT_GL_PROD = PX.ORA_PRODUCT -- -SS- MANF_PROD_CODE, FIXME join won't work -- (+)
	LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL ON PS.GL_DPT_ID = AOL.ORA_LOCATION --  /* -SS- FIXME */ /* -SS- DEPT_ID */ (+)
		AND PS.GL_BU_ID = AOL.ORA_ENTITY                                              --  /* -SS- FIXME */ /* -SS- BU_UNIT */ (+)
	WHERE
		1 = 2
...
SELECT
	...
	LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR   ON L.R12_PRODUCT = PR.R12_PRODUCT --  /* -SS- PRODUCT */ (+)-- /* -SS- OTR */
	LEFT OUTER JOIN OTR_TRANE_DEPTS_PS DP      ON L.R12_LOCATION = DP.DEPTID     -- SS FIXME not changed to R12 -- R/* -SS- DEPTID */(+)
	LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO PX ON L.BUSINESS_UNIT = PX.GL_LEDGER -- (+)
		AND L.R12_PRODUCT = PX.ORA_PRODUCT -- /* -SS- PRODUCT, MANF_PROD_CODE */ (+)
...
SELECT
	...
	LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR   ON PS.R12_PRODUCT = PR.R12_PRODUCT    -- /* -SS- PRODUCT */  /* -SS- PRODUCT */ (+) --  /* -SS- OTR */
	LEFT OUTER JOIN OTR_TRANE_DEPTS_PS DP      ON PS.R12_LOCATION = DP.DEPTID        -- /* -SS- DEPTID */ (+)
	LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO PX ON PS.R12_PRODUCT = PX.ORA_PRODUCT -- /* -SS- PRODUCT, MANF_PROD_CODE */ (+)
		AND PS.BUSINESS_UNIT = PX.GL_LEDGER                                             -- (+)
	LEFT OUTER JOIN ACTUATE_SEC_XREF ASX ON PS.BUSINESS_UNIT = ASX.PSGL              -- (+)
	WHERE
		1 = 2
		AND PS.FISCAL_YEAR IN('2001', '2002')
		...
		(
			SELECT
				...
				LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO X ON D.BUSINESS_UNIT = X.GL_LEDGER       -- (+)
					AND D.R12_PRODUCT = X.ORA_PRODUCT -- /* -SS- PRODUCT, MANF_PROD_CODE */ (+)
				LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR    ON D.R12_PRODUCT = PR.R12_PRODUCT    -- /* -SS- PRODUCT */ /* -SS- PRODUCT */ (+) /* -SS- OTR */
				LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL ON D.R12_LOCATION = AOL.ORA_LOCATION -- /* -SS- DEPTID */ /* -SS- DEPT_ID */ (+)
					AND D.R12_ENTITY = AOL.ORA_ENTITY                                               -- /* -SS- BUSINESS_UNIT_GL */ /* -SS- BU_UNIT */ (+)
				WHERE
					1 = 2
...
SELECT
	...
	WHERE
		...
		AND(
		(PR.R12_ACCOUNT = '411101' AND (PR.PS_ACCOUNT = '700000' OR PR.PS_ACCOUNT = '')) -- -SS- PR.ACCOUNT = '700000'
		OR PR.R12_ACCOUNT
		/* -SS- ACCOUNT */
		= '700020'
		/* -SS- ???? */
