-- Extract
SELECT DISTINCT
		CLAIM.CLAIM_NUMBER,
		CLAIM.TYPE as TAVANT_CLAIM_TYPE, -- used for claim_type
		CLAIM.COMMERCIAL_POLICY, -- used for claim_type
		EXPENSES.QUERY_SOURCE,
		EXPENSES.WARRANTY_TYPE AS TAVANT_WARRANTY_TYPE,
		EXPENSES.PRIORITY,
		EXPENSES.PART_NBR,
		EXPENSES.COST_CATEGORY,
		EXPENSES.GL_EXPENSE_STRING,
		-- 9258.115614.00000.511705.41125.0000.00000.00000
		REGEXP_SUBSTR(EXPENSES.GL_EXPENSE_STRING,'[^.]+', 1, 1) AS SEGMENT1,
		REGEXP_SUBSTR(EXPENSES.GL_EXPENSE_STRING,'[^.]+', 1, 2) AS SEGMENT2,
		REGEXP_SUBSTR(EXPENSES.GL_EXPENSE_STRING,'[^.]+', 1, 3) AS SEGMENT3,
		REGEXP_SUBSTR(EXPENSES.GL_EXPENSE_STRING,'[^.]+', 1, 4) AS SEGMENT4,
		REGEXP_SUBSTR(EXPENSES.GL_EXPENSE_STRING,'[^.]+', 1, 5) AS SEGMENT5,
		REGEXP_SUBSTR(EXPENSES.GL_EXPENSE_STRING,'[^.]+', 1, 6) AS SEGMENT6,
		REGEXP_SUBSTR(EXPENSES.GL_EXPENSE_STRING,'[^.]+', 1, 7) AS SEGMENT7,
		REGEXP_SUBSTR(EXPENSES.GL_EXPENSE_STRING,'[^.]+', 1, 8) AS SEGMENT8,
		EXPENSES.GL_EXPENSE_AMT,
		EXPENSES.GL_EXPENSE_CURR,
		EXPENSES.ACCEPTED_AMT,
		EXPENSES.ACCEPTED_CURR,
		CLAIM.FAILURE_DATE,
		CLAIM.FILED_ON_DATE,
		CLAIM_INFO.ACCEPTED_LABOR_AMT,
		CLAIM_INFO.ACCEPTED_MATERIAL_AMT
	FROM
		CLAIM
	INNER JOIN (
		SELECT C.CLAIM_NUMBER,
			SUM(CASE LIG.NAME
				WHEN 'Additional Travel Hours' THEN LIG.ACCEPTED_AMT
				WHEN 'Freight' THEN 0
				WHEN 'Labor' THEN LIG.ACCEPTED_AMT
				WHEN 'Local Purchase' THEN 0
				WHEN 'Non Oem Parts' THEN 0
				WHEN 'Oem Parts' THEN 0
				WHEN 'Others' THEN LIG.ACCEPTED_AMT
				WHEN 'Rebate' THEN 0
				WHEN 'Travel by Hours' THEN LIG.ACCEPTED_AMT
				WHEN 'Travel by Perdiem' THEN LIG.ACCEPTED_AMT
				ELSE 0
				END) AS ACCEPTED_LABOR_AMT,
			SUM(CASE LIG.NAME
				WHEN 'Additional Travel Hours' THEN 0
				WHEN 'Freight' THEN LIG.ACCEPTED_AMT
				WHEN 'Labor' THEN 0
				WHEN 'Local Purchase' THEN LIG.ACCEPTED_AMT
				WHEN 'Non Oem Parts' THEN LIG.ACCEPTED_AMT
				WHEN 'Oem Parts' THEN LIG.ACCEPTED_AMT
				WHEN 'Others' THEN 0
				WHEN 'Rebate' THEN LIG.ACCEPTED_AMT
				WHEN 'Travel by Hours' THEN 0
				WHEN 'Travel by Perdiem' THEN 0
				ELSE 0
				END) AS ACCEPTED_MATERIAL_AMT
			FROM
				CLAIM C
			INNER JOIN PAYMENT P             ON C.PAYMENT = P.ID
			INNER JOIN LINE_ITEM_GROUPS LIGS ON P.ID = LIGS.FOR_PAYMENT
			INNER JOIN LINE_ITEM_GROUP LIG   ON LIGS.LINE_ITEM_GROUPS = LIG.ID
		GROUP BY
			C.CLAIM_NUMBER
	) CLAIM_INFO
		ON CLAIM_INFO.CLAIM_NUMBER = CLAIM.CLAIM_NUMBER
	INNER JOIN (
		-- OEM
		-- Expense Query For Parts:
		-- r12 GL String Parts: ENTITY.LOCATION.COSTCENTER.ACCOUNT.R12PRODUCTCODE.INTERCOMPANY
		SELECT
				C.CLAIM_NUMBER,
				'OEM' AS QUERY_SOURCE,
				I.ITEM_NUMBER AS PART_NBR,
				LIG.NAME AS COST_CATEGORY,
				LIG.ACCEPTED_AMT,
				LIG.ACCEPTED_CURR,
				PPI.GL_CODE_COGS_REVERSAL AS GL_COGS_REVERSAL_STRING,
				PPI.GL_CODE_REVENUE_REVERSAL AS GL_REV_REVERSAL_STRING,
				PPI.GL_CODE_WNTY_EXPENSE_REV AS GL_EXPENSE_STRING,
				PPI.GL_EXPENSE_AMT,
				PPI.GL_EXPENSE_CURR,
				AP.ID AS AP_ID,
				PD.ID AS PD_ID,
				PD.WARRANTY_TYPE,
				PD.PRIORITY
			FROM
				CLAIM C
			INNER JOIN PAYMENT P             ON C.PAYMENT = P.ID
			INNER JOIN LINE_ITEM_GROUPS LIGS ON P.ID = LIGS.FOR_PAYMENT
			INNER JOIN LINE_ITEM_GROUP LIG   ON LIGS.LINE_ITEM_GROUPS = LIG.ID
			INNER JOIN CURRENT_PART_INFO CPI ON LIG.ID = CPI.LINE_ITEM_GROUP
			INNER JOIN PART_PAYMENT_INFO PPI ON CPI.CURRENT_PART_PAYMENT_INFO = PPI.ID --NOTE: If modifier is not picked, PART_PAYMENT_INFO_MODIFIER table will be empty.
			INNER JOIN ITEM I                ON I.ID = PPI.ITEM 
			LEFT OUTER JOIN APPLICABLE_POLICY AP ON PPI.APPLICABLE_POLICY = AP.ID -- LINE_ITEM_GROUP.APPLICABLE_POLICY can be null
			LEFT OUTER JOIN POLICY_DEFINITION PD ON AP.POLICY_DEFINITION = PD.ID
			WHERE 0 = 0
				AND PPI.GL_EXPENSE_AMT <> 0
		
		UNION ALL
		
		-- NON-OEM
		--Expense Query  for Cost Categories labor, non oem parts, travel, etc, everything but OEM parts
		SELECT DISTINCT
				C.CLAIM_NUMBER,
				'NON-OEM' AS QUERY_SOURCE,
				NULL AS ITEM_NUMBER,
				LIG.NAME AS COST_CATEGORY,
				LIG.ACCEPTED_AMT,
				LIG.ACCEPTED_CURR,
				NULL AS GL_COGS_REVERSAL_STRING,
				NULL AS GL_REV_REVERSAL_STRING,
				LIG.GL_CODE_WNTY_EXPENSE_REV AS GL_EXPENSE_STRING,
				LIG.GL_AMT AS EXPENSE_AMOUNT,
				LIG.GL_CURR AS EXPENSE_CURRENCY,
				AP.ID AS AP_ID,
				PD.ID AS PD_ID,
				PD.WARRANTY_TYPE,
				PD.PRIORITY
			FROM
				CLAIM C
			INNER JOIN PAYMENT P             ON C.PAYMENT = P.ID
			INNER JOIN LINE_ITEM_GROUPS LIGS ON P.ID = LIGS.FOR_PAYMENT
			INNER JOIN LINE_ITEM_GROUP LIG   ON LIGS.LINE_ITEM_GROUPS = LIG.ID
			LEFT OUTER JOIN APPLICABLE_POLICY AP ON LIG.APPLICABLE_POLICY = AP.ID -- LINE_ITEM_GROUP.APPLICABLE_POLICY can be null
			LEFT OUTER JOIN POLICY_DEFINITION PD ON AP.POLICY_DEFINITION = PD.ID
			WHERE
				0 = 0
				AND LIG.NAME NOT IN('Oem Parts', 'Claim Amount')
				AND LIG.GL_CODE_WNTY_EXPENSE_REV IS NOT NULL
				AND LIG.GL_AMT <> 0

		UNION ALL

		-- REBATES
		SELECT
				C.CLAIM_NUMBER,
				'REBATE' AS QUERY_SOURCE,
				I.ITEM_NUMBER AS PART_NBR,
				LIG.NAME AS COST_CATEGORY,
				LIG.ACCEPTED_AMT,
				LIG.ACCEPTED_CURR,
				PPI.GL_CODE_COGS_REVERSAL AS GL_COGS_REVERSAL_STRING,
				PPI.GL_CODE_REVENUE_REVERSAL AS GL_REV_REVERSAL_STRING,
				PPI.GL_CODE_WNTY_EXPENSE_REV AS GL_EXPENSE_STRING,
				-- GL_ACCOUNT is Segment4 of the GL_EXPENSE_STRING
				-- PPI.GL_COGS_AMT,
				-- PPI.GL_COGS_CURR AS GL_COGS_CURRENCY,
				-- PPI.GL_REVENUE_AMT,
				-- PPI.GL_REVENUE_CURR,
				REBATES.GL_AMT GL_EXPENSE_AMT,
				REBATES.GL_CURR GL_EXPENSE_CURR,
				AP.ID AS AP_ID,
				PD.ID AS PD_ID,
				PD.WARRANTY_TYPE,
				PD.PRIORITY
			FROM
				CLAIM C
			INNER JOIN PAYMENT P             ON C.PAYMENT = P.ID
			INNER JOIN LINE_ITEM_GROUPS LIGS ON P.ID = LIGS.FOR_PAYMENT
			INNER JOIN LINE_ITEM_GROUP LIG   ON LIGS.LINE_ITEM_GROUPS = LIG.ID
			INNER JOIN CURRENT_PART_INFO CPI ON LIG.ID = CPI.LINE_ITEM_GROUP
			INNER JOIN PART_PAYMENT_INFO PPI ON CPI.CURRENT_PART_PAYMENT_INFO = PPI.ID --NOTE: If modifier is not picked, PART_PAYMENT_INFO_MODIFIER table will be empty.
			INNER JOIN ITEM I                ON I.ID = PPI.ITEM 
			LEFT OUTER JOIN APPLICABLE_POLICY AP ON PPI.APPLICABLE_POLICY = AP.ID -- LINE_ITEM_GROUP.APPLICABLE_POLICY can be null
			LEFT OUTER JOIN POLICY_DEFINITION PD ON AP.POLICY_DEFINITION = PD.ID
			LEFT OUTER JOIN (
				--Expense Query For OEM Parts Modifiers:
				SELECT
						C.CLAIM_NUMBER,
						I.ITEM_NUMBER AS PART_NBR,
						MI.MODIFIER_NAME AS GL_NAME,
						PPI.GL_CODE_WNTY_EXPENSE_REV,
						MI.GL_AMT,
						MI.GL_CURR,
						LIG.NAME,
						LIG.ACCEPTED_AMT
					FROM
						CLAIM C
					INNER JOIN PAYMENT P                            ON C.PAYMENT = P.ID
					INNER JOIN LINE_ITEM_GROUPS LIGS                ON P.ID = LIGS.FOR_PAYMENT
					INNER JOIN LINE_ITEM_GROUP LIG                  ON LIGS.LINE_ITEM_GROUPS = LIG.ID
					INNER JOIN CURRENT_PART_INFO CPI                ON LIG.ID = CPI.LINE_ITEM_GROUP
					INNER JOIN PART_PAYMENT_INFO PPI                ON CPI.CURRENT_PART_PAYMENT_INFO = PPI.ID --NOTE: If modifier is not picked, PART_PAYMENT_INFO_MODIFIER table will be empty.
					LEFT OUTER JOIN PART_PAYMENT_INFO_MODIFIER PPIM ON PPI.ID = PPIM.PART_PAYMENT_INFO
					INNER JOIN MODIFIER_INFO MI                     ON MI.ID = PPIM.MODIFIER_INFO
					INNER JOIN ITEM I                               ON I.ID = PPI.ITEM
					WHERE
						0 = 0
						AND MI.MODIFIER_NAME LIKE 'Rebate%'
			) REBATES
				ON REBATES.CLAIM_NUMBER = C.CLAIM_NUMBER AND REBATES.PART_NBR = I.ITEM_NUMBER
			WHERE 0 = 0
				AND REBATES.GL_AMT <> 0
		
		UNION ALL

			-- COMMISSIONS
			SELECT
			CLAIM.CLAIM_NUMBER,
			'COMM' AS QUERY_SOURCE,
			NULL AS ITEM_NUMBER,
			'Commissions' as COST_CATEGORY,
			0 AS ACCEPTED_AMT,
			'' AS ACCEPTED_CURR,
			NULL AS GL_COGS_REVERSAL_STRING,
			NULL AS GL_REV_REVERSAL_STRING,
		    MIN(COMMISSION_GL_WNTY_EXPENSE) AS GL_EXPENSE_STRING,
			SUM(CD.SPLIT_AMOUNT_AMT) AS EXPENSE_AMOUNT,
			MIN(CD.SPLIT_AMOUNT_CURR) AS EXPENSE_CURRENCY,
			A.AP_ID,
			A.PD_ID,
			A.WARRANTY_TYPE,
			A.PRIORITY

			FROM CLAIM
			INNER JOIN PAYMENT
				ON CLAIM.PAYMENT = PAYMENT.ID
			LEFT OUTER JOIN (
				-- OEM
				SELECT
						C.CLAIM_NUMBER,
						P.ID AS PAYMENT_ID,
						'OEM' AS QUERY_SOURCE,
						I.ITEM_NUMBER AS PART_NBR,
						LIG.NAME AS COST_CATEGORY,
						PPI.GL_EXPENSE_AMT,
						PPI.GL_EXPENSE_CURR,
						AP.ID AS AP_ID,
						PD.ID AS PD_ID,
						PD.WARRANTY_TYPE,
						PD.PRIORITY
					FROM
						CLAIM C
					INNER JOIN PAYMENT P             ON C.PAYMENT = P.ID
					INNER JOIN LINE_ITEM_GROUPS LIGS ON P.ID = LIGS.FOR_PAYMENT
					INNER JOIN LINE_ITEM_GROUP LIG   ON LIGS.LINE_ITEM_GROUPS = LIG.ID
					INNER JOIN CURRENT_PART_INFO CPI ON LIG.ID = CPI.LINE_ITEM_GROUP
					INNER JOIN PART_PAYMENT_INFO PPI ON CPI.CURRENT_PART_PAYMENT_INFO = PPI.ID --NOTE: If modifier is not picked, PART_PAYMENT_INFO_MODIFIER table will be empty.
					INNER JOIN ITEM I                ON I.ID = PPI.ITEM 
					LEFT OUTER JOIN APPLICABLE_POLICY AP ON PPI.APPLICABLE_POLICY = AP.ID -- LINE_ITEM_GROUP.APPLICABLE_POLICY can be null
					LEFT OUTER JOIN POLICY_DEFINITION PD ON AP.POLICY_DEFINITION = PD.ID
					WHERE 0 = 0
						-- AND PPI.GL_EXPENSE_AMT <> 0
						AND LIG.ACCEPTED_RATE <> 0
				
				UNION ALL
				
				-- NON-OEM
				--Expense Query  for Cost Categories labor, non oem parts, travel, etc, everything but OEM parts
				SELECT DISTINCT
						C.CLAIM_NUMBER,
						P.ID AS PAYMENT_ID,
						'NON-OEM' AS QUERY_SOURCE,
						NULL AS ITEM_NUMBER,
						LIG.NAME AS COST_CATEGORY,
						LIG.GL_AMT AS EXPENSE_AMOUNT,
						LIG.GL_CURR AS EXPENSE_CURRENCY,
						AP.ID AS AP_ID,
						PD.ID AS PD_ID,
						PD.WARRANTY_TYPE,
						PD.PRIORITY
					FROM
						CLAIM C
					INNER JOIN PAYMENT P             ON C.PAYMENT = P.ID
					INNER JOIN LINE_ITEM_GROUPS LIGS ON P.ID = LIGS.FOR_PAYMENT
					INNER JOIN LINE_ITEM_GROUP LIG   ON LIGS.LINE_ITEM_GROUPS = LIG.ID
					LEFT OUTER JOIN APPLICABLE_POLICY AP ON LIG.APPLICABLE_POLICY = AP.ID -- LINE_ITEM_GROUP.APPLICABLE_POLICY can be null
					LEFT OUTER JOIN POLICY_DEFINITION PD ON AP.POLICY_DEFINITION = PD.ID
					WHERE
						0 = 0
						AND LIG.NAME NOT IN('Oem Parts', 'Claim Amount')
						AND LIG.GL_CODE_WNTY_EXPENSE_REV IS NOT NULL
						AND LIG.ACCEPTED_RATE <> 0
						-- AND LIG.GL_AMT <> 0
		
			) A
				ON A.CLAIM_NUMBER = CLAIM.CLAIM_NUMBER AND A.PAYMENT_ID = PAYMENT.ID
			LEFT OUTER JOIN CREDIT_MEMO
				ON PAYMENT.ACTIVE_CREDIT_MEMO = CREDIT_MEMO.ID -- (+)
			INNER JOIN CLAIMED_ITEM
				ON CLAIM.ID = CLAIMED_ITEM.CLAIM
			INNER JOIN INVENTORY_ITEM
				ON CLAIMED_ITEM.ITEM_REF_INV_ITEM = INVENTORY_ITEM.ID
			INNER JOIN ITEM
				ON ITEM.ID = INVENTORY_ITEM.OF_TYPE
			INNER JOIN SALES_ORDER
				ON INVENTORY_ITEM.FOR_SALES_ORDER = SALES_ORDER.ID
			INNER JOIN SERVICE_INFORMATION SI
				ON CLAIM.SERVICE_INFORMATION = SI.ID
			INNER JOIN SERVICE S
				ON SI.SERVICE_DETAIL = S.ID
			INNER JOIN SERVICE_COMMISSION_DETAILS SCD
				ON S.ID = SCD.SERVICE
			INNER JOIN COMMISSION_DETAIL CD
				ON SCD.COMMISSION_DETAILS = CD.ID
			INNER JOIN COMMISSION_CODE CC
				ON CC.ID = CD.COMMISSION_CODE
			
			GROUP BY
			CLAIM.CLAIM_NUMBER,
			A.AP_ID,
			A.PD_ID,
			A.WARRANTY_TYPE,
			A.PRIORITY
			
	) EXPENSES
		ON EXPENSES.CLAIM_NUMBER = CLAIM.CLAIM_NUMBER
	WHERE
		0 = 0
		AND CLAIM.BUSINESS_UNIT_INFO = 'HVAC TCP' --Jean Skemp recommends using original source id insteal
		AND CLAIM.STATE = 'ACCEPTED_AND_CLOSED'
		-- AND CLAIM.CLAIM_NUMBER = 'C-10764292'
		/**/
		AND CLAIM.CLAIM_NUMBER IN (
		'C-10763992',
		'C-10764004',
		'C-10764005',
		'C-10764006',
		'C-10764013',
		'C-10764014',
		'C-10764015',
		'C-10764019',
		'C-10764022',
		'C-10764023',
		'C-10764024',
		'C-10764025',
		'C-10764028',
		'C-10764038',
		'C-10764041',
		'C-10764048',
		'C-10764066',
		'C-10764067',
		'C-10764073',
		'C-10764079',
		'C-10764106',
		'C-10764106_I',
		'C-10764198',
		'C-10764218',
		'C-10764224',
		'C-10764227',
		'C-10764275',
		'C-10764284',
		'C-10764285',
		'C-10764288',
		'C-10764292',
		'C-10764323_D',
		'C-10764338',
		'C-10764534')
		/**/
		order by 1, 2, 3, 4, 5;