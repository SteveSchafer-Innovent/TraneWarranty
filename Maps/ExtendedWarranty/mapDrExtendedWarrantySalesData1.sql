SELECT
		...
		CAST(A.POSTING_CONTROL_ID AS VARCHAR2(10)) AS JOURNAL_ID,  -- -SS- FIXME Need to add Column, assigned to Pam
	...
	LEFT OUTER JOIN R12_TRANE_ACCOUNTS_PS PSA ON A.R12_ACCOUNT = PSA.R12_ACCOUNT -- -SS- FIXME join won't work, AP_030_ARC_BILL_MVW may be old and not used
...
SELECT
	...
	FROM
		DBO.R12_BI_ACCT_ENTRY_PSB A -- -SS- OTR
	INNER JOIN DBO.OTR_TRNBI_BI_HDR_PSB B ON A.BUSINESS_UNIT = B.BUSINESS_UNIT AND A.INVOICE = B.INVOICE
	INNER JOIN DBO.OTR_BI_HDR_PSB C       ON B.BUSINESS_UNIT = C.BUSINESS_UNIT AND B.INVOICE = C.INVOICE AND C.ENTRY_TYPE = 'IN'
		-- -SS- NEW
	INNER JOIN R12_ACCOUNT_FILTER_UPD AFU ON AFU.R12_ACCOUNT = DIST.R12_ACCOUNT
		-- -SS- /NEW
