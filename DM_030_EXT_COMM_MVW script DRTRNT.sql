...
CREATE MATERIALIZED VIEW DBO.DM_030_EXT_COMM_MVW(COUNTRY_INDICATOR, JRNL_YEAR_MONTH, GL_ACCOUNT, JOURNAL_DATE, COMMISSION_AMOUNT) TABLESPACE D1_AA NOCACHE LOGGING NOCOMPRESS NOPARALLEL BUILD DEFERRED REFRESH COMPLETE ON DEMAND
WITH PRIMARY KEY AS
	...
	FROM
		(
			SELECT
				...
				ON DIST.R12_ACCOUNT = PSA.R12_ACCOUNT -- -SS- R12_2_R12
				*/
				...
				WHERE
					...
					DIST.R12_ENTITY IN('5575', '5612', '5743', '9256', '9258', '9298', '9299', '9984') -- -SS- USA
			...
			SELECT
				...
				/* -SS- issue 88
				INNER JOIN R12_TRANE_ACCOUNTS_PS PSA -- -SS- OTR
				ON GL_CODE.R12_ACCOUNT = PSA.R12_ACCOUNT  -- -SS- R12_2_R12
				*/
					/* -SS- ,
					actuate_sec_xref asx */
				WHERE
					-- -SS- comm.code_combination_id = gl_code.code_combination_id AND
					GL_CODE.R12_ENTITY IN('5575', '5612', '5743', '9256', '9258', '9298', '9299', '9984') -- -SS- USA
					-- -SS- asx.nation_curr = 'USD'
			...
			SELECT
				...
				*/
				...
				WHERE
					-- -SS- comm.code_combination_id = gl_code.code_combination_id AND
					GL_CODE.R12_ENTITY IN('5773', '5588') -- -SS- CAN
					-- -SS- asx.nation_curr = 'CAD'
					-- -SS- AND GL_CODE.SEGMENT2 = PSA.ACCOUNT (+)
					-- -SS- issue 88: AND PSA.TRANE_ACCOUNT_IND = 'X'
					-- -SS- AND gl_code.segment1 = asx.psgl(+)
			...
			SELECT
				...
				/* -SS- issue 88
				INNER JOIN R12_TRANE_ACCOUNTS_PS PSA -- -SS- OTR
				ON UPD.R12_ACCOUNT = PSA.R12_ACCOUNT      -- -SS- R12_2_R12
				*/
				WHERE
