...
,MAX(( (LR.APPR_SUBLET_MAT_AMT + LR.APPR_SUBLET_REF_AMT + LR.APPR_SUBLET_SERV_AMT)/(SELECT COUNT(DISTINCT LRS.STEP_NBR) FROM WC_LABOR_ROLLUP LRS WHERE LRS.CLAIM_NBR = LR.CLAIM_NBR)* LR.CHARGE_COMPANY_PCT)  )AS DOLLAR_AMOUNT
,MLR.STEP_NBR 
...
,MLR.FAIL_DATE_KEY
,MLR.START_DATE_KEY
,MLR.GL_ACCOUNT_SCD_KEY /* R12_GL_ACCOUNT_SCD /* -SS- */.GL_ACCOUNT_SCD_KEY */
...
AND TD.FULL_DATE>=TO_DATE('1/1/2000','MM/DD/YYYY') AND TD.FULL_DATE<TO_DATE('12/31/2050','MM/DD/YYYY')
...
AND (GLA.R12_ACCOUNT /* -SS- ACCOUNT */ LIKE '0620%' OR  GLA.R12_ACCOUNT /* -SS- ACCOUNT */ LIKE '8062%')
...
, LR.CHARGE_COMM_PCT
, LR.CHARGE_COMPANY_PCT  
, ((SUM((LR2.APPR_AMT))+ MAX((LR.APPR_DIAGNOSTIC_AMT + LR.APPR_TRAVEL_AMT)/(SELECT COUNT(DISTINCT LRS.STEP_NBR) FROM WC_LABOR_ROLLUP LRS WHERE LRS.CLAIM_NBR = LR.CLAIM_NBR)) )*LR.CHARGE_COMPANY_PCT) AS DOLLAR_AMOUNT
,MLR.STEP_NBR 
/* DATES FROM DAY_TIME TABLE */
,MLR.CCN_TRX_DATE_KEY
,MLR.ORIGINAL_SHIP_DATE_KEY
...
,(SELECT DISTINCT CLAIM_NBR,DETAIL_NBR,STEP_NBR,APPR_AMT,CCN_TRX_DATE_KEY FROM WC_LABOR_ROLLUP LR1 WHERE CLAIM_TYPE_SCD_KEY=11
    and exists (select 'X' from R12_GL_ACCOUNT_SCD /* -SS- */ where lr1.GL_ACCOUNT_SCD_KEY=GL_ACCOUNT_SCD_KEY AND (ACCOUNT  LIKE '0620%' OR  ACCOUNT LIKE '8062%'))) LR2
...
AND (GLA.R12_ACCOUNT /* -SS- ACCOUNT */ LIKE '0620%' /* -SS- ???? */ OR  GLA.R12_ACCOUNT /* -SS- ACCOUNT */ LIKE '8062%' /* -SS- ???? */)
 --AND CT.CLAIM_TYPE_CODE ='CONCESSION'
--AND MLR.CLAIM_NBR = 1682063
...
and (MLR.CLAIM_TYPE_SCD_KEY=3 or MLR.CLAIM_TYPE_SCD_KEY=10)
...
AND TD3.FULL_DATE>=TO_DATE('1/1/2000','MM/DD/YYYY') AND TD3.FULL_DATE<TO_DATE('12/31/2050','MM/DD/YYYY')
...
,(
  CASE 
  WHEN GLA.R12_ENTITY <> 5773 /* -SS- ASX.NATION_CURR='USD' */ THEN 'USA'
  WHEN GLA.R12_ENTITY = 5773 /* -SS- ASX.NATION_CURR='CAD' */ THEN 'CAN' 
  ELSE 'CURRENCY: ' || 'UNKNOWN' /* -SS- ASX.NATION_CURR */ 
  END
)
/* NEW FIELDS ADDED 5/21/07 */
,MLR.RETRO_ID 
...
,LR.FAIL_DATE_KEY
,LR.START_DATE_KEY
,LR.GL_ACCOUNT_SCD_KEY /* R12_GL_ACCOUNT_SCD /* -SS- */.GL_ACCOUNT_SCD_KEY */
,LR.PROD_CODE_SCD_KEY /* PROD_CODE_SCD.PROD_CODE_SCD_KEY */
,LR.CUST_ACCOUNT_SCD_KEY /* CUST_ACCOUNT_SCD.CUST_ACCOUNT_SCD_KEY */
,LR.SUBMIT_OFFICE_SCD_KEY /* SUBMIT_OFFICE_SCD.SUBMIT_OFFICE_SCD_KEY */
,LR.TRX_CURRENCY 
