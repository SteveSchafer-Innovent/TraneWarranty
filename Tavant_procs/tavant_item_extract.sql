...
SELECT DISTINCT
		...
		SP.XREF_SERVICE_PROVIDER_NUMBER CUSTOMER_NUMBER,
		PARTY.NAME AS CUSTOMER_NAME,
		DEAL.NAME AS OFFICE_NAME
	...
	FROM
		...
		-- INNER JOIN POLICY
		-- 	ON WARRANTY.ID = POLICY.WARRANTY -- MULTIPLIER
		...
		LEFT OUTER JOIN PARTY 						   ON PARTY.ID = SP.ID
		LEFT OUTER JOIN PARTY DEAL 					 ON CLAIM.for_dealer = DEAL.ID
	WHERE
		0 = 0
...
-- SELECT * FROM WARRANTY WHERE ID = '110000151974360';
-- SELECT * FROM POLICY WHERE WARRANTY = '110000151974360';