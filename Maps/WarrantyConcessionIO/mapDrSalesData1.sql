SELECT
	...
	COALESCE(AOL_R12.OFFICE_NAME, COALESCE(AOL_PS.OFFICE_NAME, CASE
	...
	null /* -SS- ???? issue 55 PS.PLNT_GL_PROD */ AS MANF_PROD_ID,
	...
	-- PER PAT'S REQUEST 5/30/07
	CASE
		WHEN null /* -SS- ???? issue 55 PS.PART_TYPE */ = 'Y' AND null /* -SS- ???? issue 55 PS.PARTS_PROD_CODE_IND */ = 'PCR' THEN '804900'
		ELSE PS.R12_PRODUCT -- -SS- GL_PROD
	END AS DIST_GL_PRODUCT,
	/* PER JACKIE'S EMAIL 5/9, FOLLOWING LOGIC IS NEEDED*/
	NVL(PX.PRODUCT_CATEGORY, 'INVALID PROD CODE - ' || null /* -SS- ???? issue 55 PS.PLNT_GL_PROD */) AS RESERVE_GROUP, -- -SS- PRODUCT_ID
...
AND null /* ???? issue 55 PS.PLNT_GL_PROD */ = PX.MANF_PROD_CODE -- (+)

LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_PS
ON null /* -SS- ???? issue 73 PS.PS_GL_DPT_ID */ != 'NA' AND null /* -SS- ???? issue 73 PS.PS_GL_DPT_ID */ = AOL_PS.DEPT_ID AND null /* -SS- ???? issue 73 PS.PS_GL_BU_ID */ != 'NA' AND null /* -SS- ???? issue 73 PS.PS_GL_BU_ID */ = AOL_PS.BU_UNIT
LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_R12
ON AOL_R12.ORA_LOCATION IS NOT NULL AND PS.R12_LOCATION = AOL_R12.ORA_LOCATION AND AOL_R12.ORA_ENTITY IS NOT NULL AND null /* -SS- ???? issue 73 PS.R12_ENTITY */ = AOL_R12.ORA_ENTITY
...
GROUP BY PS.GL_BU_ID,
	...
	AOL_R12.OFFICE_NAME,
	-- -SS- AOL.OFFICE_NAME,
	null /* -SS- ???? issue 55 PS.PLNT_GL_PROD */,
...
ON -- -SS- OTR
	GA.BUSINESS_UNIT   = L.BUSINESS_UNIT
...
ON L.R12_PRODUCT = PR.R12_PRODUCT -- -SS- PRODUCT, (+), OTR
LEFT OUTER JOIN OTR_TRANE_DEPTS_PS DP
ON L.R12_LOCATION = DP.DEPTID -- SS DEPTID (+) ???? issue 69
LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO PX
ON L.BUSINESS_UNIT = PX.GL_LEDGER      -- (+)
AND L.PS_PRODUCT   = PX.MANF_PROD_CODE -- (+)
LEFT OUTER JOIN ACTUATE_SEC_XREF ASX
ON GA.BUSINESS_UNIT     = ASX.PSGL -- (+)
...
SELECT
	...
	ASX.NATION_CURR                                                               AS COUNTRY_INDICATOR
FROM R12_LEDGER2_PS PS -- -SS- OTR
	-- -SS- NEW
INNER JOIN R12_ACCOUNT_FILTER_UPD AFU
ON AFU.R12_ACCOUNT = PS.R12_ACCOUNT
	-- -SS- /NEW
LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR
ON PS.R12_PRODUCT = PR.R12_PRODUCT -- -SS- PRODUCT, PRODUCT, (+), OTR
LEFT OUTER JOIN OTR_TRANE_DEPTS_PS DP
ON PS.R12_LOCATION = DP.DEPTID -- -SS- DEPTID, (+)
LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO PX
ON PS.PS_PRODUCT     = PX.MANF_PROD_CODE -- (+)
AND PS.BUSINESS_UNIT = PX.GL_LEDGER      -- (+)
LEFT OUTER JOIN ACTUATE_SEC_XREF ASX
ON PS.BUSINESS_UNIT       = ASX.PSGL -- (+)
...
FROM
	(SELECT
		...
		CASE
			WHEN D.R12_PRODUCT IN ('41204', '41198') THEN '41204' ELSE D.R12_PRODUCT END AS GL_PRODCODE,
	...
	FROM R12_BI_LINE_PSB A -- -SS- OTR
	INNER JOIN R12_BI_ACCT_ENTRY_PSB D
	ON D.LINE_SEQ_NUM   = A.LINE_SEQ_NUM -- -SS- OTR
	...
	ON D.BUSINESS_UNIT = X.GL_LEDGER      -- (+)
	AND D.PS_PRODUCT   = X.MANF_PROD_CODE -- (+)
	LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR
	ON D.R12_PRODUCT = PR.R12_PRODUCT -- -SS- PRODUCT, PRODUCT, (+), OTR

	LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_PS
	ON D.PS_DEPTID = AOL_PS.DEPT_ID AND D.PS_BUSINESS_UNIT_GL = AOL_PS.BU_UNIT
	LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_R12
	ON AOL_R12.ORA_LOCATION IS NOT NULL AND D.R12_LOCATION = AOL_R12.ORA_LOCATION AND AOL_R12.ORA_ENTITY IS NOT NULL AND D.R12_ENTITY = AOL_R12.ORA_ENTITY
...
FROM
	...
	LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_PS
	ON D.PS_DEPTID != 'NA' AND D.PS_DEPTID = AOL_PS.DEPT_ID AND D.PS_BUSINESS_UNIT_GL != 'NA' AND D.PS_BUSINESS_UNIT_GL = AOL_PS.BU_UNIT
	LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_R12
	ON AOL_R12.ORA_LOCATION IS NOT NULL AND D.R12_LOCATION = AOL_R12.ORA_LOCATION AND AOL_R12.ORA_ENTITY IS NOT NULL AND D.R12_ENTITY = AOL_R12.ORA_ENTITY
	-- -SS- ACTUATE_OFFICE_LOCATION AOL,
...
SELECT
	...
	CASE
		WHEN PR. R12_ENTITY IN('5773', '5588') THEN 'CAN'
		ELSE 'USA'
	END AS COUNTRY_INDICATOR
FROM R12_PROJ_RESOURCE_PS PR --  /* -SS- OTR was AP_135_PROJ_RESOURCE */
	-- @ED_INTFC_DR.LAX.TRANE.COM PR,
INNER JOIN AP_135_TRNPC_PROJ_RES RES
ON PR.BUSINESS_UNIT = RES.BUSINESS_UNIT --@ED_INTFC_DR.LAX.TRANE.COM  RES,
AND PR.PROJECT_ID   = RES.PROJECT_ID
AND PR.RESOURCE_ID  = RES.RESOURCE_ID
AND PR.ACTIVITY_ID  = RES.ACTIVITY_ID
INNER JOIN AP_400_SEL_CRED_JB_CLSS_CD JBCLSCD
ON PR.PROJECT_ID = CAST(JBCLSCD.CREDIT_JOB_ID AS VARCHAR2(15 BYTE)) --@ED_INTFC_DR.LAX.TRANE.COM  JBCLSCD,
INNER JOIN AP_400_JOB_CODE JBCD
ON JBCD.JOB_CODE_ID = JBCLSCD.JOB_CODE_ID -- @ED_INTFC_DR.LAX.TRANE.COM  JBCD,
INNER JOIN AP_135_TRNPC_COMM_DATA CD
ON PR.BUSINESS_UNIT = CD.BUSINESS_UNIT --@ED_INTFC_DR.LAX.TRANE.COM  CD,
AND PR.PROJECT_ID   = CD.PROJECT_ID
AND PR.RESOURCE_ID  = CD.RESOURCE_ID
AND PR.ACTIVITY_ID  = CD.ACTIVITY_ID
INNER JOIN AP_400_COMM_CODE CC
ON CD.TRNPC_COMM_CODE  = CC.COMM_CODE --@ED_INTFC_DR.LAX.TRANE.COM  CC,
AND CD.SALES_OFFICE_ID = CC.SALES_OFFICE_ID
	-- -SS- MD_SECURITY_ENTITY_DRV SEC, @ED_INTFC_DR.LAX.TRANE.COM  SEC,
INNER JOIN MD_PRODUCT_CODE MPC
ON RES.TRNPC_MFG_PROD_CD = MPC.PRODUCT_CODE_VALUE --  @ED_INTFC_DR.LAX.TRANE.COM  MPC
	-- -SS- NEW
INNER JOIN R12_ACCOUNT_FILTER_UPD AFU
ON AFU.R12_ACCOUNT = PR.R12_ACCOUNT
	-- -SS- /NEW
WHERE 
