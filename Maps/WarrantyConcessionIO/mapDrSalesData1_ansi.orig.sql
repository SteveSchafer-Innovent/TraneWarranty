	...
	FROM
		(
			SELECT
				...
				FROM
					OTR_BI_LINE_PSB A
				LEFT OUTER JOIN OTR_BI_ACCT_ENTRY_PSB D     ON D.LINE_SEQ_NUM = A.LINE_SEQ_NUM AND D.INVOICE = A.INVOICE AND D.BUSINESS_UNIT = A.BUSINESS_UNIT
				LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO X   ON D.PRODUCT = X.MANF_PROD_CODE AND D.BUSINESS_UNIT = X.GL_LEDGER
				LEFT OUTER JOIN OTR_TRANE_PRODUCTS_PS PR    ON D.PRODUCT = PR.PRODUCT
				LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL ON D.DEPTID = AOL.DEPT_ID AND D.BUSINESS_UNIT_GL = AOL.BU_UNIT
				WHERE
	...
	FROM
		(
			SELECT
				...
				LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO X   ON D.BUSINESS_UNIT = X.GL_LEDGER AND D.PRODUCT = X.MANF_PROD_CODE
				LEFT OUTER JOIN OTR_TRANE_PRODUCTS_PS PR    ON D.PRODUCT = PR.PRODUCT
				LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL ON D.DEPTID = AOL.DEPT_ID AND D.BUSINESS_UNIT_GL = AOL.BU_UNIT
				WHERE
...
SELECT
	...
	FROM
		...
		INNER JOIN AP_135_TRNPC_COMM_DATA CD ON 1=1 AND PR.BUSINESS_UNIT = CD.BUSINESS_UNIT AND PR.PROJECT_ID = CD.PROJECT_ID AND PR.RESOURCE_ID = CD.RESOURCE_ID AND PR.ACTIVITY_ID = CD.ACTIVITY_ID
