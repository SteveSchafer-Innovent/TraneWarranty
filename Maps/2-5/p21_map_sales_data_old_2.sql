SELECT
	...
	FROM
		OTR_BI_LINE_PSB A
	INNER JOIN OTR_BI_ACCT_ENTRY_PSB D          ON 1 = 1 AND D.LINE_SEQ_NUM = A.LINE_SEQ_NUM AND D.INVOICE = A.INVOICE AND D.BUSINESS_UNIT = A.BUSINESS_UNIT
	LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO X   ON 1 = 1 AND A.IDENTIFIER = X.MANF_PROD_CODE AND X.GL_LEDGER = 'CSD'
	LEFT OUTER JOIN OTR_TRANE_PRODUCTS_PS PR    ON 1 = 1 AND D.PRODUCT = PR.PRODUCT
	LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL ON 1 = 1 AND D.DEPTID = AOL.DEPT_ID
	WHERE
