...
--DIST.JOURNAL_DATE AS JOURNAL_DATE ,
       SUM(Case when dist.debit_amt = 0 or dist.debit_amt is null or dist.credit_amount <> ''
                then dist.credit_amount * -1
                else dist.debit_amt end) AS DOLLAR_AMOUNT,
...
/*TAY:       GROUP BY CASE WHEN ASX.NATION_CURR ='USD' THEN 'USA' ELSE 'CAN' END, DIST.ACCOUNT, psa.DESCR*/
     ...
     (/* Amort_Comm_and_prepaid_comm (Month to date data)*/
      ...
      FROM (SELECT a.country_indicator,
             a.gl_account,A.GL_ACCOUNT_DESCR,
             to_date('1-'||:RunDate, 'dd-mon-yy'),
             (MAX(a.Comm_amort_mnthly)+ CASE WHEN A.FORECAST_PERIOD = to_date('1-'||:RunDate, 'dd-mon-yy')
...
/*TAY:              AND  a.gl_account like '5%' WIP*/
              AND a.gl_account like '5%'
              AND A.SHIP_PERIOD >= CASE WHEN to_date('1-'||:RunDate, 'dd-mon-yy') =
...
/*TAY:                    AND DIST.BUSINESS_UNIT_GL= ASX.PSGL*/
                    --AND DIST.BUSINESS_UNIT_GL= ASX.PSGL
                    AND DIST.JOURNAL_DATE BETWEEN TO_DATE('1-'||:RunDate, 'dd-mon-yy') AND
                                                  LAST_DAY(to_date('1-'||:RunDate, 'dd-mon-yy'))
/*TAY:                    AND DIST.ACCOUNT LIKE '5%' WIP*/
                    AND DIST.PS_ACCOUNT LIKE '5%'
/*TAY                    and CASE WHEN ASX.NATION_CURR ='USD' THEN 'USA' ELSE 'CAN' END = UPPER(:COUNTRY))*/
                    and CASE WHEN DIST.r12_entity IN (5773, 5588) THEN 'CAN' ELSE 'USA' END = UPPER(:COUNTRY))
