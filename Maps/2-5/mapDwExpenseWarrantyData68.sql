CREATE
		VIEW MAP_EXPENSEWARRANTYDATA68_VW AS
		(
			/* 2-5Year Specific */
			SELECT
					/* + NO_CPU_COSTING  */
					...
					GLA.R12_ENTITY AS BUSINESS_UNIT, -- -SS- COMPANY
					...
					GLA.R12_COST_CENTER AS GL_DEPT, -- -SS- COST_CENTER
					/* 2-5Year Specific */
					...
					( 
						...
						*/
						...
						SELECT
							...
							INNER JOIN
								(SELECT DISTINCT CLAIM_NBR, DETAIL_NBR, STEP_NBR, APPR_AMT, CCN_TRX_DATE_KEY FROM WC_LABOR_ROLLUP LR1 WHERE CLAIM_TYPE_SCD_KEY = 11 
								AND EXISTS (
											SELECT 'X' 
											FROM R12_GL_ACCOUNT_SCD GLA
											LEFT OUTER JOIN R12_ACCOUNT_FILTER_UPD AFU ON AFU.R12_ACCOUNT = GLA.R12_ACCOUNT
										WHERE
											...
											OR GLA.PS_ACCOUNT LIKE '8062%')
											-- -SS- /NEW
											-- -SS- AND(ACCOUNT LIKE '0620%' OR ACCOUNT LIKE '8062%')
											)
								)
	...
	UNION
			(
				/* 2nd Query - Material and Retrofit Material */
				SELECT
						/* + NO_CPU_COSTING  */
					...
					INNER JOIN TIME_DAY TD2                        ON MLR.ORIGINAL_SHIP_DATE_KEY = TD2.TIME_KEY -- TD1 FOR FAIL DATE
					INNER JOIN TIME_DAY TD1                        ON MLR.FAIL_DATE_KEY = TD1.TIME_KEY          -- TD2 FOR ORIGINAL SHIP DATE
		...
		UNION ALL
				(
					/* Query 3 - SPD, Retrofit Labor, Extended Purchase Labor   */
					SELECT
							/* + NO_CPU_COSTING  */
							CCN_DATA.CLAIM_NBR AS CLAIM_NUMBER,
							CCN_DATA.STEP_NBR AS STEP_NUMBER,
							...
							(
								...
								SELECT
									...
									WHERE
										...
										LR.CLAIM_TYPE_SCD_KEY IN(1, 2, 9)
										AND TD.FULL_DATE >= TO_DATE('1/1/2000', 'MM/DD/YYYY')
										AND TD.FULL_DATE < TO_DATE('12/31/2050', 'MM/DD/YYYY')
										--AND MONTHS_BETWEEN(last_day(SYSDATE), last_day(TD.full_date))<=12
										--AND MONTHS_BETWEEN(last_day(SYSDATE), last_day(TD.full_date))>=1
						...
						INNER JOIN SUBMIT_OFFICE_SCD SOS  ON CCN_DATA.SUBMIT_OFFICE_SCD_KEY = SOS.SUBMIT_OFFICE_SCD_KEY
							-- -SS- ,ACTUATE_SEC_XREF ASX
						INNER JOIN PROD_CODE_XREF_RCPO_DR PRODGRP ON GLA.PS_COMPANY = PRODGRP.GL_LEDGER -- -SS- issue 22 -- SR-r12???
							AND PCS.PROD_CODE = PRODGRP.MANF_PROD_CODE                            -- -SS- issues 55, 56 -- FOR RESERVE GROUP
