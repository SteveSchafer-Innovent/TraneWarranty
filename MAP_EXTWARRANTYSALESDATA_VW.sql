...
AS
   ...
   SELECT /*+ NO_CPU_COSTING */
	...
	SELECT
			/*+ NO_CPU_COSTING */
			CASE WHEN BAE.R12_ENTITY IN('5773', '5588') THEN 'CAD'
				ELSE 'USD'
				END AS COUNTRY_INDICATOR,
			TO_CHAR(BAE.JOURNAL_DATE, 'YYYYMM') AS JRNL_YEAR_MONTH,
			BAE.R12_ENTITY AS BU,
			BAE.R12_ACCOUNT AS GL_ACCOUNT,
			BAE.R12_LOCATION AS GL_DEP_ID,
			'R12 Ledger' AS QUERY_SOURCE,
			BAE.R12_PRODUCT AS GL_PRODUCT_ID,
			BAE.JOURNAL_DATE AS JOURNAL_DATE,
			BAE.JOURNAL_ID AS JOURNAL_ID,
			BAE.MONETARY_AMOUNT * - 1 AS REVENUE_AMOUNT,
			100 * (BAE.MONETARY_AMOUNT * - 1 - TRUNC(BAE.MONETARY_AMOUNT * - 1)) AS REVENUE_AMOUNT_DEC
		FROM
			DBO.R12_BI_ACCT_ENTRY_STG BAE
		INNER JOIN DBO.R12_TRNBI_BI_HDR_STG TBH ON BAE.INVOICE = TBH.INVOICE AND BAE.CUSTOMER_TRX_ID = TBH.CUSTOMER_TRX_ID
		INNER JOIN DBO.R12_BI_HDR_STG BH        ON TBH.INVOICE = BH.INVOICE AND TBH.CUSTOMER_TRX_ID = BH.CUSTOMER_TRX_ID
    INNER JOIN DBO.R12_BI_LINE_STG BL       ON BL.LINE_SEQ_NUM = BAE.LINE_SEQ_NUM
          AND BL.INVOICE = BAE.INVOICE  AND BL.CUSTOMER_TRX_ID = BAE.CUSTOMER_TRX_ID
		INNER JOIN R12_ACCOUNT_FILTER_UPD AFU   ON AFU.R12_ACCOUNT = BAE.R12_ACCOUNT
		WHERE
			BAE.JOURNAL_DATE BETWEEN TO_DATE('01/01/2003', 'MM/DD/YYYY') AND TO_DATE('12/31/2050', 'MM/DD/YYYY')
			AND BAE.R12_ENTITY IN('5773', '5588', '5575', '5612', '5743', '9256', '9258', '9298', '9299', '9984')
			AND BH.ENTRY_TYPE = 'INV'
			AND AFU.LIKE_52_53_54 = 'Y'
          -- Kelly is undecided about this:
          -- AND BL.MANF_PROD_ID IS NOT NULL
