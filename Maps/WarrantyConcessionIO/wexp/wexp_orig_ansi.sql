...
SELECT
	...
	FROM
		(
			...
			SELECT
				...
				INNER JOIN WC_LABOR_ROLLUP LR ON MLR.CCN_TRX_DATE_KEY = LR.CCN_TRX_DATE_KEY AND MLR.CLAIM_NBR = LR.CLAIM_NBR AND MLR.DETAIL_NBR = LR.DETAIL_NBR
				INNER JOIN TIME_DAY TD        ON TD.TIME_KEY = MLR.CCN_TRX_DATE_KEY AND TD.TIME_KEY = LR.CCN_TRX_DATE_KEY
				INNER JOIN GL_ACCOUNT_SCD GLA ON GLA.GL_ACCOUNT_SCD_KEY = MLR.GL_ACCOUNT_SCD_KEY
				WHERE
					1 = 1
			...
			SELECT
				...
				INNER JOIN TIME_DAY TD        ON TD.TIME_KEY = MLR.CCN_TRX_DATE_KEY AND TD.TIME_KEY = LR.CCN_TRX_DATE_KEY
				INNER JOIN GL_ACCOUNT_SCD GLA ON GLA.GL_ACCOUNT_SCD_KEY = MLR.GL_ACCOUNT_SCD_KEY AND GLA.GL_ACCOUNT_SCD_KEY = LR.GL_ACCOUNT_SCD_KEY
				INNER JOIN
	...
	INNER JOIN GL_ACCOUNT_SCD GLA                   ON CCN_DATA.GL_ACCOUNT_SCD_KEY = GLA.GL_ACCOUNT_SCD_KEY
	...
	INNER JOIN PROD_CODE_XREF_RCPO_DR PRODGRP       ON GLA.COMPANY = PRODGRP.GL_LEDGER AND PCS.PROD_CODE = PRODGRP.MANF_PROD_CODE
	LEFT OUTER JOIN ACTUATE_SEC_XREF ASX            ON GLA.COMPANY = ASX.PSGL
...
UNION
		...
		(
			SELECT
				...
				INNER JOIN GL_ACCOUNT_SCD GLA                   ON MLR.GL_ACCOUNT_SCD_KEY = GLA.GL_ACCOUNT_SCD_KEY
			...
			(
				SELECT
					...
					FROM
						(
							...
							SELECT
								...
								INNER JOIN GL_ACCOUNT_SCD GLA ON GLA.GL_ACCOUNT_SCD_KEY = LR.GL_ACCOUNT_SCD_KEY
								WHERE
									1 = 1
					...
					LEFT OUTER JOIN
						...
						) FCW                                          ON CCN_DATA.STEP_NBR = FCW.STEP_NBR AND CCN_DATA.CLAIM_NBR = FCW.CLAIM_NBR
					INNER JOIN TIME_DAY TD3                         ON CCN_DATA.CCN_TRX_DATE_KEY = TD3.TIME_KEY
					INNER JOIN TIME_DAY TD2                         ON CCN_DATA.ORIGINAL_SHIP_DATE_KEY = TD2.TIME_KEY
					INNER JOIN TIME_DAY TD1                         ON CCN_DATA.FAIL_DATE_KEY = TD1.TIME_KEY
					INNER JOIN TIME_DAY TD                          ON CCN_DATA.START_DATE_KEY = TD.TIME_KEY
					...
					INNER JOIN CUST_ACCOUNT_SCD CACCT               ON CCN_DATA.CUST_ACCOUNT_SCD_KEY = CACCT.CUST_ACCOUNT_SCD_KEY
					INNER JOIN SUBMIT_OFFICE_SCD SOS                ON CCN_DATA.SUBMIT_OFFICE_SCD_KEY = SOS.SUBMIT_OFFICE_SCD_KEY
					INNER JOIN PROD_CODE_XREF_RCPO_DR PRODGRP       ON PCS.PROD_CODE = PRODGRP.MANF_PROD_CODE AND GLA.COMPANY = PRODGRP.GL_LEDGER
					LEFT OUTER JOIN ACTUATE_SEC_XREF ASX            ON GLA.COMPANY = ASX.PSGL
					INNER JOIN DM_WAR_CSN_RSV_PCT_REF RES_PCT       ON CCN_DATA.EXPENSE_TYPE_CATG = UPPER(RES_PCT.EXPENSE_TYPE_CATG) AND CCN_DATA.EXPENSE_TYPE_DESCR = RES_PCT.EXPENSE_TYPE_DESCR AND SOS.COMPANY_OWNED_IND = RES_PCT.COMPANY_OWNED_IND
					LEFT OUTER JOIN UD_031_STDWTY_RSV_CLM_ADJ A     ON CCN_DATA.CLAIM_NBR = A.CLAIM_NUMBER
					LEFT OUTER JOIN DM_WAR_CSN_RSV_PCT_REF RES_PCT1 ON A.CLAIM_TYPE = RES_PCT1.CLAIM_TYPE
					WHERE
						1 = 1
