CREATE OR REPLACE PACKAGE BODY DBO.pkg_031_retrofit_reserve IS
	...
	SUBTYPE t_custom_excep_desc IS sy_log_error.oracle_error_message%TYPE;
	...
	***********************************************************************************/
     ...
     -- perform insert into error table
        INSERT INTO sy_log_error
                    (sy_log_error_key, package_name,
                     procedure_name, source_code_marker,
                     oracle_error_message, error_notes, error_timestamp
                    )
             VALUES (sy_log_error_seq.NEXTVAL, p_package_name_in,
                     p_procedure_name_in, p_source_code_marker_in,
                     p_oracle_error_message_in, p_error_notes_in, SYSDATE
                    );
    ...
    ***********************************************************************************/
    ...
    BEGIN
        ...
        */
        MERGE INTO
        UD_031_RETROFIT_ID a
        USING (SELECT DISTINCT RETRO_ID FROM WC_MAT_LBR_ROLLUP WHERE RETRO_ID IS NOT NULL) b
        on   (TRIM(a.RETROFIT_ID) = TRIM(b.RETRO_ID))
        WHEN NOT MATCHED THEN INSERT
        (RETROFIT_ID,SPECIFIC_RESERVE_IND,PCT_100_RECOVERY_IND,NEW_RESOLVED_IND,ED_CREATE_DATE,ED_CREATE_ID,ED_UPDATE_DATE,ED_UPDATE_ID)
        VALUES (TRIM(b.RETRO_ID), NULL,NULL,'N',SYSDATE,L_PROC_NAME,SYSDATE,L_PROC_NAME);
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            p_process_errors(
                ...
                l_level,
                SUBSTR(SQLERRM, 1, 255),
                'DBO.UD_031_RETROFIT_ID load terminated due to error');
    ...
    ***********************************************************************************/
    ...
    BEGIN
        ...
        INSERT /*+ APPEND */ INTO DBO.SY_031_CLAIM_SUM_STG
           SELECT
                     COUNTRY_INDICATOR,
                     GL_ACCOUNT,
           ...
           FROM
                      ...
                      AND MLR.CLAIM_NBR = a.CLAIM_NUMBER (+)        --TTP#12554
                  ...
                  WHERE TRX_DATE BETWEEN TRUNC(ADD_MONTHS(CASE WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM') ELSE G_RUN END, -24), 'MM')
                  AND LAST_DAY(TRUNC(ADD_MONTHS(CASE WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM') ELSE G_RUN END, -1), 'MM'))
                  GROUP BY
                  COUNTRY_INDICATOR,
                  GL_ACCOUNT,
                  TRXYEARMONTH ,
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            p_process_errors(
                ...
                l_level,
                SUBSTR(SQLERRM, 1, 255),
                'DBO.SY_031_CLAIM_SUM_STG load terminated due to error');
    ...
    ***********************************************************************************/
    ...
    PROCEDURE p_load_retrofit_reserve(G_RUN IN DATE)
    IS
    ...
    BEGIN
        ...
        INSERT /*+ APPEND */ INTO DBO.DM_031_RETROFIT_RSV
           SELECT
            ...
            (
                SELECT
                    TO_NUMBER(TO_CHAR(CASE WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM') ELSE G_RUN END, 'YYYYMM')) AS RUN_PERIOD
                    ,SSS.SHIP_PERIOD
                    ...
                    ,NVL(SUM(CSS.IN_RSV_EXPENSE_AMT) OVER (PARTITION BY TO_NUMBER(TO_CHAR(CASE WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM') ELSE G_RUN END, 'YYYYMM')),SSS.COUNTRY_INDICATOR)
                        / SUM(SSS. REVENUE_AMT / 1000000) OVER (PARTITION BY TO_NUMBER(TO_CHAR(CASE WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM') ELSE G_RUN END, 'YYYYMM')),SSS.COUNTRY_INDICATOR), 0) AS WEIGHTED_AVG_RATE
                    ,NVL(24 - (MONTHS_BETWEEN(TRUNC(CASE WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM') ELSE G_RUN END,'MM') , TO_DATE(SHIP_PERIOD, 'YYYYMM'))), 0) AS REMAINING_PDS
                    ,NVL((24 - ROUND(MONTHS_BETWEEN(TRUNC(CASE WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM') ELSE G_RUN END,'MM') , TO_DATE(SHIP_PERIOD, 'YYYYMM')),0)) / 24, 0) AS REMAINING_PDS_FACTOR
                    ,NVL(SSS.REVENUE_AMT / 1000000
                        * SUM(CSS.IN_RSV_EXPENSE_AMT) OVER (PARTITION BY TO_NUMBER(TO_CHAR(CASE WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM') ELSE G_RUN END, 'YYYYMM')),SSS.COUNTRY_INDICATOR)
                            / SUM(SSS. REVENUE_AMT / 1000000) OVER (PARTITION BY TO_NUMBER(TO_CHAR(CASE WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM') ELSE G_RUN END, 'YYYYMM')),SSS.COUNTRY_INDICATOR)
                        * (24 - ROUND(MONTHS_BETWEEN(TRUNC(CASE WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM') ELSE G_RUN END,'MM') , TO_DATE(SHIP_PERIOD, 'YYYYMM')),0)) / 24, 0) AS RESERVE_CALC_AMT
                FROM
                     DR_SY_031_SALES_SUM_STG SSS
                    ,(SELECT
                            CASE WHEN COUNTRY_INDICATOR =  'USA' THEN 'USD'  WHEN COUNTRY_INDICATOR =  'CAN' THEN 'CAD'   END COUNTRY_INDICATOR
                            ,TRX_PERIOD
                            ,NVL(SUM(IN_RSV_EXPENSE_AMT), 0) AS IN_RSV_EXPENSE_AMT
                      FROM
                            dbo.SY_031_CLAIM_SUM_STG
                      GROUP BY
                            CASE WHEN COUNTRY_INDICATOR =  'USA' THEN 'USD'  WHEN COUNTRY_INDICATOR =  'CAN' THEN 'CAD'   END
                            ,TRX_PERIOD
                            ) CSS
                WHERE
                ...
                GROUP BY
                    TO_NUMBER(TO_CHAR(CASE WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM') ELSE G_RUN END, 'YYYYMM'))
                    ,SSS.SHIP_PERIOD
                    ,SSS.COUNTRY_INDICATOR
                    ...
                    WHERE TO_DATE(SHIP_PERIOD, 'YYYYMM') BETWEEN ADD_MONTHS(TO_DATE(RUN_PERIOD, 'YYYYMM'), -24) AND ADD_MONTHS(TO_DATE(RUN_PERIOD, 'YYYYMM'), -1)
                    ;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            p_process_errors(
                ...
                l_level,
                SUBSTR(SQLERRM, 1, 255),
                'DBO.DM_031_RETROFIT_RSV load terminated due to error');
...
PROCEDURE p_del_retro_rsv_same_run(G_RUN IN DATE)
    IS
    ...
    BEGIN
          ...
          WHERE TO_DATE((SUBSTR(RUN_PERIOD,5,6)||'/01/'||SUBSTR(RUN_PERIOD,1,4)),'MM/DD/YYYY') = TRUNC(CASE WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM') ELSE G_RUN END);
    ...
    EXCEPTION
               WHEN OTHERS THEN
                ...
                l_level,
                SUBSTR(SQLERRM, 1, 255),
                'Error occurred while deleting a period in Retrofit Reserve tables');
...
PROCEDURE p_del_retro_rsv_older_run(G_RUN IN DATE)
    ...
    BEGIN
          ...
          FROM
               DBO.DM_031_RETROFIT_RSV
          WHERE TO_DATE((SUBSTR(RUN_PERIOD,5,6)||'/01/'||SUBSTR(RUN_PERIOD,1,4)),'MM/DD/YYYY') <  ADD_MONTHS(CASE WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM') ELSE G_RUN END, -13);
    ...
    EXCEPTION
               WHEN OTHERS THEN
                ...
                l_level,
                SUBSTR(SQLERRM, 1, 255),
                'Error occurred while deleting older run periods of Retrofit Reserve tables');
