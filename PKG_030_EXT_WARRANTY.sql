...
***********************************************************************************/
	...
	WITH LAG_RULES AS
		...
		FROM R12_GL_ACCOUNT_SCD /* -SS- */ GLA
		RIGHT JOIN R12_TRANE_ACCOUNTS_PS /* -SS- OTR */ PSA
		ON GLA.R12_ACCOUNT = PSA.R12_ACCOUNT -- -SS- R12_2_R12
		-- -ss- GLA.ACCOUNT = PSA.ACCOUNT
		-- -SS- CROSS JOIN ???? ACTUATE_SEC_XREF ASX - why is this a cross join?
		CROSS JOIN SY_EXT_LAG_RULES_UPD LAG
		JOIN UD_030_ACCT_RPT_TYP_XRF ART
		ON LAG.REPORT_TYPE = ART.REPORT_TYPE
		WHERE /* -SS- GLA.COMPANY =(
			CASE
				WHEN ASX.PSGL IS NULL THEN GLA.COMPANY
				ELSE ASX.PSGL
			END)
		AND */ 
		    GLA.R12_ACCOUNT = ART.GL_ACCOUNT -- -SS- ???? issue 70, R12_2_R12
		    -- -SS- GLA.ACCOUNT = ART.GL_ACCOUNT
...
***********************************************************************************/
	...
	BEGIN
		...
		INSERT
			/*+ APPEND */
		INTO TEMP_030_COST_FLOW
	WITH GL_ACCOUNT_FILTER AS
		(SELECT GL_ACCOUNT_SCD_KEY,
			R12_ENTITY /* -SS- COMPANY */,
			R12_ACCOUNT /* -SS- ACCOUNT */
		FROM R12_GL_ACCOUNT_SCD /* -SS- */ GLA1
		INNER JOIN UD_030_ACCT_RPT_TYP_XRF ART
		ON GLA1.R12_ACCOUNT = ART.gl_account -- -SS- ???? issue 70, R12_2_R12
		-- -SS- GLA1.ACCOUNT = ART.gl_account
		)
	SELECT R12_ENTITY /* -SS- COMPANY */ AS BUSINESS_UNIT,
		R12_ACCOUNT /* -SS- ACCOUNT */ AS GL_ACCOUNT,
		(
			CASE
			WHEN GLA.R12_ENTITY NOT IN ('5773', '5588') /* -SS- ASX.NATION_CURR='USD' */ THEN 'USA'
			ELSE 'CAN'
	...
	FROM GL_ACCOUNT_FILTER GLA
	/* -SS- LEFT OUTER JOIN ACTUATE_SEC_XREF ASX
	ON GLA.COMPANY = ASX.PSGL */
	JOIN PROD_CODE_XREF_RCPO_DR PRODGRP
	ON GLA.COMPANY = PRODGRP.GL_LEDGER -- -SS- ???? issue 22
	AND PRODGRP.PRODUCT_CATEGORY IS NOT NULL
	LEFT OUTER JOIN R12_TRANE_ACCOUNTS_PS /* -SS- OTR */ PSA
	ON GLA.R12_ACCOUNT = PSA.R12_ACCOUNT -- -SS- R12_2_R12
	-- -SS- GLA.ACCOUNT = PSA.ACCOUNT
	AND PSA.TRANE_ACCOUNT_IND = 'X'
	JOIN WC_MAT_LBR_ROLLUP MLR
	...
	WHERE TD3.FULL_DATE        >= TO_DATE('1/1/2000', 'MM/DD/YYYY')
...
***********************************************************************************/
	...
	BEGIN
		...
		SELECT SY_030_COST_FLOW_STG_SEQ.NEXTVAL,
			LAG_RULES.REPORT_TYPE,
			TEMP_030_COST_FLOW.BUSINESS_UNIT,
			LAG_RULES.GL_ACCOUNT,
		...
		FROM TEMP_030_LAG_RULE_YR_XRF LAG_RULES
		LEFT OUTER JOIN TEMP_030_COST_FLOW
		ON LAG_RULES.lag_period         = TEMP_030_COST_FLOW.ship_to_trans_lag
		AND LAG_RULES.GENERATED_YEAR    = TEMP_030_COST_FLOW.TRX_YEAR
		AND LAG_RULES.COUNTRY_INDICATOR = TEMP_030_COST_FLOW.COUNTRY_INDICATOR
		AND LAG_RULES.gl_ACCOUNT        = TEMP_030_COST_FLOW.GL_ACCOUNT;
		COMMIT;
		l_level := 2;
	EXCEPTION
	WHEN OTHERS THEN
...
***********************************************************************************/
	...
	BEGIN
		...
		SELECT sy_030_cost_flow_stg_seq.NEXTVAL,
			...
			LAG_RULES.GL_Account        AS GL_ACCOUNT,
...
***********************************************************************************/
	...
	BEGIN
		...
		SELECT Report_Type AS REPORT_TYPE,
			...
			MAX(
			CASE
				WHEN LAG_PERIOD = 1 THEN LAG_PERIOD
			END) AS LAG_PERIOD_1,
			MAX(
			CASE
				WHEN LAG_PERIOD = 2 THEN LAG_PERIOD
			END) AS LAG_PERIOD_2,
			MAX(
			CASE
				WHEN LAG_PERIOD = 3 THEN LAG_PERIOD
			END) AS LAG_PERIOD_3,
			MAX(
			CASE
				WHEN LAG_PERIOD = 4 THEN LAG_PERIOD
			END) AS LAG_PERIOD_4,
			MAX(
			CASE
				WHEN LAG_PERIOD = 5 THEN LAG_PERIOD
			END) AS LAG_PERIOD_5,
			MAX(
			CASE
				WHEN LAG_PERIOD = 6 THEN LAG_PERIOD
			END) AS LAG_PERIOD_6,
			MAX(
			CASE
				WHEN LAG_PERIOD = 7 THEN LAG_PERIOD
			END) AS LAG_PERIOD_7,
			MAX(
			CASE
				WHEN LAG_PERIOD = 8 THEN LAG_PERIOD
			END) AS LAG_PERIOD_8,
			MAX(
			CASE
				WHEN LAG_PERIOD = 9 THEN LAG_PERIOD
			END) AS LAG_PERIOD_9,
			MAX(
			CASE
				WHEN LAG_PERIOD = 10 THEN LAG_PERIOD
			END) AS LAG_PERIOD_10,
			MAX(
			CASE
				WHEN LAG_PERIOD = 11 THEN LAG_PERIOD
			END) AS LAG_PERIOD_11,
			MAX(
			CASE
				WHEN LAG_PERIOD = 12 THEN LAG_PERIOD
			END) AS LAG_PERIOD_12,
			MAX(
			CASE
				WHEN LAG_PERIOD = 13 THEN LAG_PERIOD
			END) AS LAG_PERIOD_13,
			MAX(
			CASE
				WHEN LAG_PERIOD = 14 THEN LAG_PERIOD
			END) AS LAG_PERIOD_14,
			MAX(
			CASE
				WHEN LAG_PERIOD = 15 THEN LAG_PERIOD
			END) AS LAG_PERIOD_15,
			MAX(
			CASE
				WHEN LAG_PERIOD = 16 THEN LAG_PERIOD
			END) AS LAG_PERIOD_16,
			MAX(
			CASE
				WHEN LAG_PERIOD = 17 THEN LAG_PERIOD
			END) AS LAG_PERIOD_17,
			MAX(
			CASE
				WHEN LAG_PERIOD = 18 THEN LAG_PERIOD
			END) AS LAG_PERIOD_18,
			MAX(
			CASE
				WHEN LAG_PERIOD = 19 THEN LAG_PERIOD
			END) AS LAG_PERIOD_19,
			MAX(
			CASE
				WHEN LAG_PERIOD = 20 THEN LAG_PERIOD
			END) AS LAG_PERIOD_20,
			MAX(
			CASE
				WHEN LAG_PERIOD = 21 THEN LAG_PERIOD
			END) AS LAG_PERIOD_21,
			MAX(
			CASE
				WHEN LAG_PERIOD = 22 THEN LAG_PERIOD
			END) AS LAG_PERIOD_22,
			MAX(
			CASE
				WHEN LAG_PERIOD = 23 THEN LAG_PERIOD
			END) AS LAG_PERIOD_23,
			MAX(
			CASE
				WHEN LAG_PERIOD = 24 THEN LAG_PERIOD
			END) AS LAG_PERIOD_24,
			MAX(
			CASE
				WHEN LAG_PERIOD = 25 THEN LAG_PERIOD
			END) AS LAG_PERIOD_25,
			MAX(
			CASE
				WHEN LAG_PERIOD = 26 THEN LAG_PERIOD
			END) AS LAG_PERIOD_26,
			MAX(
			CASE
				WHEN LAG_PERIOD = 27 THEN LAG_PERIOD
			END) AS LAG_PERIOD_27,
			MAX(
			CASE
				WHEN LAG_PERIOD = 28 THEN LAG_PERIOD
			END) AS LAG_PERIOD_28,
			MAX(
			CASE
				WHEN LAG_PERIOD = 29 THEN LAG_PERIOD
			END) AS LAG_PERIOD_29,
			MAX(
			CASE
				WHEN LAG_PERIOD = 30 THEN LAG_PERIOD
			END) AS LAG_PERIOD_30,
			MAX(
			CASE
				WHEN LAG_PERIOD = 31 THEN LAG_PERIOD
			END) AS LAG_PERIOD_31,
			MAX(
			CASE
				WHEN LAG_PERIOD = 32 THEN LAG_PERIOD
			END) AS LAG_PERIOD_32,
			MAX(
			CASE
				WHEN LAG_PERIOD = 33 THEN LAG_PERIOD
			END) AS LAG_PERIOD_33,
			MAX(
			CASE
				WHEN LAG_PERIOD = 34 THEN LAG_PERIOD
			END) AS LAG_PERIOD_34,
			MAX(
			CASE
				WHEN LAG_PERIOD = 35 THEN LAG_PERIOD
			END) AS LAG_PERIOD_35,
			MAX(
			CASE
				WHEN LAG_PERIOD = 36 THEN LAG_PERIOD
			END) AS LAG_PERIOD_36,
			MAX(
			CASE
				WHEN LAG_PERIOD = 37 THEN LAG_PERIOD
			END) AS LAG_PERIOD_37,
			MAX(
			CASE
				WHEN LAG_PERIOD = 38 THEN LAG_PERIOD
			END) AS LAG_PERIOD_38,
			MAX(
			CASE
				WHEN LAG_PERIOD = 39 THEN LAG_PERIOD
			END) AS LAG_PERIOD_39,
			MAX(
			CASE
				WHEN LAG_PERIOD = 40 THEN LAG_PERIOD
			END) AS LAG_PERIOD_40,
			MAX(
			CASE
				WHEN LAG_PERIOD = 41 THEN LAG_PERIOD
			END) AS LAG_PERIOD_41,
			MAX(
			CASE
				WHEN LAG_PERIOD = 42 THEN LAG_PERIOD
			END) AS LAG_PERIOD_42,
			MAX(
			CASE
				WHEN LAG_PERIOD = 43 THEN LAG_PERIOD
			END) AS LAG_PERIOD_43,
			MAX(
			CASE
				WHEN LAG_PERIOD = 44 THEN LAG_PERIOD
			END) AS LAG_PERIOD_44,
			MAX(
			CASE
				WHEN LAG_PERIOD = 45 THEN LAG_PERIOD
			END) AS LAG_PERIOD_45,
			MAX(
			CASE
				WHEN LAG_PERIOD = 46 THEN LAG_PERIOD
			END) AS LAG_PERIOD_46,
			MAX(
			CASE
				WHEN LAG_PERIOD = 47 THEN LAG_PERIOD
			END) AS LAG_PERIOD_47,
			MAX(
			CASE
				WHEN LAG_PERIOD = 48 THEN LAG_PERIOD
			END) AS LAG_PERIOD_48,
			MAX(
			CASE
				WHEN LAG_PERIOD = 49 THEN LAG_PERIOD
			END) AS LAG_PERIOD_49,
			MAX(
			CASE
				WHEN LAG_PERIOD = 50 THEN LAG_PERIOD
			END) AS LAG_PERIOD_50,
			MAX(
			CASE
				WHEN LAG_PERIOD = 51 THEN LAG_PERIOD
			END) AS LAG_PERIOD_51,
			MAX(
			CASE
				WHEN LAG_PERIOD = 52 THEN LAG_PERIOD
			END) AS LAG_PERIOD_52,
			MAX(
			CASE
				WHEN LAG_PERIOD = 53 THEN LAG_PERIOD
			END) AS LAG_PERIOD_53,
			MAX(
			CASE
				WHEN LAG_PERIOD = 54 THEN LAG_PERIOD
			END) AS LAG_PERIOD_54,
			MAX(
			CASE
				WHEN LAG_PERIOD = 55 THEN LAG_PERIOD
			END) AS LAG_PERIOD_55,
			MAX(
			CASE
				WHEN LAG_PERIOD = 56 THEN LAG_PERIOD
			END) AS LAG_PERIOD_56,
			MAX(
			CASE
				WHEN LAG_PERIOD = 57 THEN LAG_PERIOD
			END) AS LAG_PERIOD_57,
			MAX(
			CASE
				WHEN LAG_PERIOD = 58 THEN LAG_PERIOD
			END) AS LAG_PERIOD_58,
			MAX(
			CASE
				WHEN LAG_PERIOD = 59 THEN LAG_PERIOD
			END) AS LAG_PERIOD_59,
			MAX(
			CASE
				WHEN LAG_PERIOD = 60 THEN LAG_PERIOD
			END) AS LAG_PERIOD_60,
			MAX(
			CASE
				WHEN LAG_PERIOD = 61 THEN LAG_PERIOD
			END) AS LAG_PERIOD_61,
			MAX(
			CASE
				WHEN LAG_PERIOD = 62 THEN LAG_PERIOD
			END) AS LAG_PERIOD_62,
			MAX(
			CASE
				WHEN LAG_PERIOD = 63 THEN LAG_PERIOD
			END) AS LAG_PERIOD_63,
			MAX(
			CASE
				WHEN LAG_PERIOD = 64 THEN LAG_PERIOD
			END) AS LAG_PERIOD_64,
			MAX(
			CASE
				WHEN LAG_PERIOD = 65 THEN LAG_PERIOD
			END) AS LAG_PERIOD_65,
			MAX(
			CASE
				WHEN LAG_PERIOD = 66 THEN LAG_PERIOD
			END) AS LAG_PERIOD_66,
			MAX(
			CASE
				WHEN LAG_PERIOD = 67 THEN LAG_PERIOD
			END) AS LAG_PERIOD_67,
			MAX(
			CASE
				WHEN LAG_PERIOD = 68 THEN LAG_PERIOD
			END) AS LAG_PERIOD_68,
			MAX(
			CASE
				WHEN LAG_PERIOD = 69 THEN LAG_PERIOD
			END) AS LAG_PERIOD_69,
			MAX(
			CASE
				WHEN LAG_PERIOD = 70 THEN LAG_PERIOD
			END) AS LAG_PERIOD_70,
			MAX(
			CASE
				WHEN LAG_PERIOD = 71 THEN LAG_PERIOD
			END) AS LAG_PERIOD_71,
			MAX(
			CASE
				WHEN LAG_PERIOD = 72 THEN LAG_PERIOD
			END) AS LAG_PERIOD_72,
			MAX(
			CASE
				WHEN LAG_PERIOD = 73 THEN LAG_PERIOD
			END) AS LAG_PERIOD_73,
			MAX(
			CASE
				WHEN LAG_PERIOD = 74 THEN LAG_PERIOD
			END) AS LAG_PERIOD_74,
			MAX(
			CASE
				WHEN LAG_PERIOD = 75 THEN LAG_PERIOD
			END) AS LAG_PERIOD_75,
			MAX(
			CASE
				WHEN LAG_PERIOD = 76 THEN LAG_PERIOD
			END) AS LAG_PERIOD_76,
			MAX(
			CASE
				WHEN LAG_PERIOD = 77 THEN LAG_PERIOD
			END) AS LAG_PERIOD_77,
			MAX(
			CASE
				WHEN LAG_PERIOD = 78 THEN LAG_PERIOD
			END) AS LAG_PERIOD_78,
			MAX(
			CASE
				WHEN LAG_PERIOD = 79 THEN LAG_PERIOD
			END) AS LAG_PERIOD_79,
			MAX(
			CASE
				WHEN LAG_PERIOD = 80 THEN LAG_PERIOD
			END) AS LAG_PERIOD_80,
			MAX(
			CASE
				WHEN LAG_PERIOD = 81 THEN LAG_PERIOD
			END) AS LAG_PERIOD_81,
			MAX(
			CASE
				WHEN LAG_PERIOD = 82 THEN LAG_PERIOD
			END) AS LAG_PERIOD_82,
			MAX(
			CASE
				WHEN LAG_PERIOD = 83 THEN LAG_PERIOD
			END) AS LAG_PERIOD_83,
			MAX(
			CASE
				WHEN LAG_PERIOD = 84 THEN LAG_PERIOD
			END) AS LAG_PERIOD_84,
			MAX(
			CASE
				WHEN LAG_PERIOD = 85 THEN LAG_PERIOD
			END) AS LAG_PERIOD_85,
			MAX(
			CASE
				WHEN LAG_PERIOD = 86 THEN LAG_PERIOD
			END) AS LAG_PERIOD_86,
			MAX(
			CASE
				WHEN LAG_PERIOD = 87 THEN LAG_PERIOD
			END) AS LAG_PERIOD_87,
			MAX(
			CASE
				WHEN LAG_PERIOD = 88 THEN LAG_PERIOD
			END) AS LAG_PERIOD_88,
			MAX(
			CASE
				WHEN LAG_PERIOD = 89 THEN LAG_PERIOD
			END) AS LAG_PERIOD_89,
			MAX(
			CASE
				WHEN LAG_PERIOD = 90 THEN LAG_PERIOD
			END) AS LAG_PERIOD_90,
			MAX(
			CASE
				WHEN LAG_PERIOD = 91 THEN LAG_PERIOD
			END) AS LAG_PERIOD_91,
			MAX(
			CASE
				WHEN LAG_PERIOD = 92 THEN LAG_PERIOD
			END) AS LAG_PERIOD_92,
			MAX(
			CASE
				WHEN LAG_PERIOD = 93 THEN LAG_PERIOD
			END) AS LAG_PERIOD_93,
			MAX(
			CASE
				WHEN LAG_PERIOD = 94 THEN LAG_PERIOD
			END) AS LAG_PERIOD_94,
			MAX(
			CASE
				WHEN LAG_PERIOD = 95 THEN LAG_PERIOD
			END) AS LAG_PERIOD_95,
			MAX(
			CASE
				WHEN LAG_PERIOD = 96 THEN LAG_PERIOD
			END) AS LAG_PERIOD_96,
			MAX(
			CASE
				WHEN LAG_PERIOD = 97 THEN LAG_PERIOD
			END) AS LAG_PERIOD_97,
			MAX(
			CASE
				WHEN LAG_PERIOD = 98 THEN LAG_PERIOD
			END) AS LAG_PERIOD_98,
			MAX(
			CASE
				WHEN LAG_PERIOD = 99 THEN LAG_PERIOD
			END) AS LAG_PERIOD_99,
			MAX(
			CASE
				WHEN LAG_PERIOD = 100 THEN LAG_PERIOD
			END) AS LAG_PERIOD_100,
			MAX(
			CASE
				WHEN LAG_PERIOD = 101 THEN LAG_PERIOD
			END) AS LAG_PERIOD_101,
			MAX(
			CASE
				WHEN LAG_PERIOD = 102 THEN LAG_PERIOD
			END) AS LAG_PERIOD_102,
			MAX(
			CASE
				WHEN LAG_PERIOD = 103 THEN LAG_PERIOD
			END) AS LAG_PERIOD_103,
			MAX(
			CASE
				WHEN LAG_PERIOD = 104 THEN LAG_PERIOD
			END) AS LAG_PERIOD_104,
			MAX(
			CASE
				WHEN LAG_PERIOD = 105 THEN LAG_PERIOD
			END) AS LAG_PERIOD_105,
			MAX(
			CASE
				WHEN LAG_PERIOD = 106 THEN LAG_PERIOD
			END) AS LAG_PERIOD_106,
			MAX(
			CASE
				WHEN LAG_PERIOD = 107 THEN LAG_PERIOD
			END) AS LAG_PERIOD_107,
			MAX(
			CASE
				WHEN LAG_PERIOD = 108 THEN LAG_PERIOD
			END) AS LAG_PERIOD_108,
			MAX(
			CASE
				WHEN LAG_PERIOD = 109 THEN LAG_PERIOD
			END) AS LAG_PERIOD_109,
			MAX(
			CASE
				WHEN LAG_PERIOD = 110 THEN LAG_PERIOD
			END) AS LAG_PERIOD_110,
			MAX(
			CASE
				WHEN LAG_PERIOD = 111 THEN LAG_PERIOD
			END) AS LAG_PERIOD_111,
			MAX(
			CASE
				WHEN LAG_PERIOD = 112 THEN LAG_PERIOD
			END) AS LAG_PERIOD_112,
			MAX(
			CASE
				WHEN LAG_PERIOD = 113 THEN LAG_PERIOD
			END) AS LAG_PERIOD_113,
			MAX(
			CASE
				WHEN LAG_PERIOD = 114 THEN LAG_PERIOD
			END) AS LAG_PERIOD_114,
			MAX(
			CASE
				WHEN LAG_PERIOD = 115 THEN LAG_PERIOD
			END) AS LAG_PERIOD_115,
			MAX(
			CASE
				WHEN LAG_PERIOD = 116 THEN LAG_PERIOD
			END) AS LAG_PERIOD_116,
			MAX(
			CASE
				WHEN LAG_PERIOD = 117 THEN LAG_PERIOD
			END) AS LAG_PERIOD_117,
			MAX(
			CASE
				WHEN LAG_PERIOD = 118 THEN LAG_PERIOD
			END) AS LAG_PERIOD_118,
			MAX(
			CASE
				WHEN LAG_PERIOD = 119 THEN LAG_PERIOD
			END) AS LAG_PERIOD_119,
			MAX(
			CASE
				WHEN LAG_PERIOD = 120 THEN LAG_PERIOD
			END) AS LAG_PERIOD_120,
			MAX(
			CASE
				WHEN LAG_PERIOD = 121 THEN LAG_PERIOD
			END) AS LAG_PERIOD_121,
			MAX(
			CASE
				WHEN LAG_PERIOD = 122 THEN LAG_PERIOD
			END) AS LAG_PERIOD_122,
			MAX(
			CASE
				WHEN LAG_PERIOD = 123 THEN LAG_PERIOD
			END) AS LAG_PERIOD_123,
			MAX(
			CASE
				WHEN LAG_PERIOD = 124 THEN LAG_PERIOD
			END) AS LAG_PERIOD_124,
			MAX(
			CASE
				WHEN LAG_PERIOD = 125 THEN LAG_PERIOD
			END) AS LAG_PERIOD_125,
			MAX(
			CASE
				WHEN LAG_PERIOD = 126 THEN LAG_PERIOD
			END) AS LAG_PERIOD_126,
			MAX(
			CASE
				WHEN LAG_PERIOD = 127 THEN LAG_PERIOD
			END) AS LAG_PERIOD_127,
			MAX(
			CASE
				WHEN LAG_PERIOD = 128 THEN LAG_PERIOD
			END) AS LAG_PERIOD_128,
			MAX(
			CASE
				WHEN LAG_PERIOD = 129 THEN LAG_PERIOD
			END) AS LAG_PERIOD_129,
			MAX(
			CASE
				WHEN LAG_PERIOD = 130 THEN LAG_PERIOD
			END) AS LAG_PERIOD_130,
			MAX(
			CASE
				WHEN LAG_PERIOD = 131 THEN LAG_PERIOD
			END) AS LAG_PERIOD_131,
			MAX(
			CASE
				WHEN LAG_PERIOD = 132 THEN LAG_PERIOD
			END) AS LAG_PERIOD_132,
			SUM(
			CASE
				WHEN LAG_PERIOD = 1 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_1,
			SUM(
			CASE
				WHEN LAG_PERIOD = 2 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_2,
			SUM(
			CASE
				WHEN LAG_PERIOD = 3 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_3,
			SUM(
			CASE
				WHEN LAG_PERIOD = 4 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_4,
			SUM(
			CASE
				WHEN LAG_PERIOD = 5 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_5,
			SUM(
			CASE
				WHEN LAG_PERIOD = 6 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_6,
			SUM(
			CASE
				WHEN LAG_PERIOD = 7 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_7,
			SUM(
			CASE
				WHEN LAG_PERIOD = 8 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_8,
			SUM(
			CASE
				WHEN LAG_PERIOD = 9 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_9,
			SUM(
			CASE
				WHEN LAG_PERIOD = 10 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_10,
			SUM(
			CASE
				WHEN LAG_PERIOD = 11 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_11,
			SUM(
			CASE
				WHEN LAG_PERIOD = 12 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_12,
			SUM(
			CASE
				WHEN LAG_PERIOD = 13 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_13,
			SUM(
			CASE
				WHEN LAG_PERIOD = 14 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_14,
			SUM(
			CASE
				WHEN LAG_PERIOD = 15 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_15,
			SUM(
			CASE
				WHEN LAG_PERIOD = 16 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_16,
			SUM(
			CASE
				WHEN LAG_PERIOD = 17 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_17,
			SUM(
			CASE
				WHEN LAG_PERIOD = 18 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_18,
			SUM(
			CASE
				WHEN LAG_PERIOD = 19 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_19,
			SUM(
			CASE
				WHEN LAG_PERIOD = 20 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_20,
			SUM(
			CASE
				WHEN LAG_PERIOD = 21 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_21,
			SUM(
			CASE
				WHEN LAG_PERIOD = 22 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_22,
			SUM(
			CASE
				WHEN LAG_PERIOD = 23 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_23,
			SUM(
			CASE
				WHEN LAG_PERIOD = 24 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_24,
			SUM(
			CASE
				WHEN LAG_PERIOD = 25 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_25,
			SUM(
			CASE
				WHEN LAG_PERIOD = 26 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_26,
			SUM(
			CASE
				WHEN LAG_PERIOD = 27 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_27,
			SUM(
			CASE
				WHEN LAG_PERIOD = 28 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_28,
			SUM(
			CASE
				WHEN LAG_PERIOD = 29 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_29,
			SUM(
			CASE
				WHEN LAG_PERIOD = 30 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_30,
			SUM(
			CASE
				WHEN LAG_PERIOD = 31 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_31,
			SUM(
			CASE
				WHEN LAG_PERIOD = 32 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_32,
			SUM(
			CASE
				WHEN LAG_PERIOD = 33 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_33,
			SUM(
			CASE
				WHEN LAG_PERIOD = 34 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_34,
			SUM(
			CASE
				WHEN LAG_PERIOD = 35 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_35,
			SUM(
			CASE
				WHEN LAG_PERIOD = 36 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_36,
			SUM(
			CASE
				WHEN LAG_PERIOD = 37 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_37,
			SUM(
			CASE
				WHEN LAG_PERIOD = 38 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_38,
			SUM(
			CASE
				WHEN LAG_PERIOD = 39 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_39,
			SUM(
			CASE
				WHEN LAG_PERIOD = 40 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_40,
			SUM(
			CASE
				WHEN LAG_PERIOD = 41 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_41,
			SUM(
			CASE
				WHEN LAG_PERIOD = 42 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_42,
			SUM(
			CASE
				WHEN LAG_PERIOD = 43 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_43,
			SUM(
			CASE
				WHEN LAG_PERIOD = 44 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_44,
			SUM(
			CASE
				WHEN LAG_PERIOD = 45 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_45,
			SUM(
			CASE
				WHEN LAG_PERIOD = 46 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_46,
			SUM(
			CASE
				WHEN LAG_PERIOD = 47 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_47,
			SUM(
			CASE
				WHEN LAG_PERIOD = 48 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_48,
			SUM(
			CASE
				WHEN LAG_PERIOD = 49 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_49,
			SUM(
			CASE
				WHEN LAG_PERIOD = 50 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_50,
			SUM(
			CASE
				WHEN LAG_PERIOD = 51 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_51,
			SUM(
			CASE
				WHEN LAG_PERIOD = 52 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_52,
			SUM(
			CASE
				WHEN LAG_PERIOD = 53 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_53,
			SUM(
			CASE
				WHEN LAG_PERIOD = 54 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_54,
			SUM(
			CASE
				WHEN LAG_PERIOD = 55 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_55,
			SUM(
			CASE
				WHEN LAG_PERIOD = 56 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_56,
			SUM(
			CASE
				WHEN LAG_PERIOD = 57 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_57,
			SUM(
			CASE
				WHEN LAG_PERIOD = 58 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_58,
			SUM(
			CASE
				WHEN LAG_PERIOD = 59 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_59,
			SUM(
			CASE
				WHEN LAG_PERIOD = 60 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_60,
			SUM(
			CASE
				WHEN LAG_PERIOD = 61 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_61,
			SUM(
			CASE
				WHEN LAG_PERIOD = 62 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_62,
			SUM(
			CASE
				WHEN LAG_PERIOD = 63 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_63,
			SUM(
			CASE
				WHEN LAG_PERIOD = 64 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_64,
			SUM(
			CASE
				WHEN LAG_PERIOD = 65 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_65,
			SUM(
			CASE
				WHEN LAG_PERIOD = 66 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_66,
			SUM(
			CASE
				WHEN LAG_PERIOD = 67 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_67,
			SUM(
			CASE
				WHEN LAG_PERIOD = 68 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_68,
			SUM(
			CASE
				WHEN LAG_PERIOD = 69 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_69,
			SUM(
			CASE
				WHEN LAG_PERIOD = 70 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_70,
			SUM(
			CASE
				WHEN LAG_PERIOD = 71 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_71,
			SUM(
			CASE
				WHEN LAG_PERIOD = 72 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_72,
			SUM(
			CASE
				WHEN LAG_PERIOD = 73 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_73,
			SUM(
			CASE
				WHEN LAG_PERIOD = 74 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_74,
			SUM(
			CASE
				WHEN LAG_PERIOD = 75 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_75,
			SUM(
			CASE
				WHEN LAG_PERIOD = 76 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_76,
			SUM(
			CASE
				WHEN LAG_PERIOD = 77 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_77,
			SUM(
			CASE
				WHEN LAG_PERIOD = 78 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_78,
			SUM(
			CASE
				WHEN LAG_PERIOD = 79 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_79,
			SUM(
			CASE
				WHEN LAG_PERIOD = 80 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_80,
			SUM(
			CASE
				WHEN LAG_PERIOD = 81 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_81,
			SUM(
			CASE
				WHEN LAG_PERIOD = 82 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_82,
			SUM(
			CASE
				WHEN LAG_PERIOD = 83 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_83,
			SUM(
			CASE
				WHEN LAG_PERIOD = 84 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_84,
			SUM(
			CASE
				WHEN LAG_PERIOD = 85 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_85,
			SUM(
			CASE
				WHEN LAG_PERIOD = 86 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_86,
			SUM(
			CASE
				WHEN LAG_PERIOD = 87 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_87,
			SUM(
			CASE
				WHEN LAG_PERIOD = 88 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_88,
			SUM(
			CASE
				WHEN LAG_PERIOD = 89 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_89,
			SUM(
			CASE
				WHEN LAG_PERIOD = 90 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_90,
			SUM(
			CASE
				WHEN LAG_PERIOD = 91 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_91,
			SUM(
			CASE
				WHEN LAG_PERIOD = 92 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_92,
			SUM(
			CASE
				WHEN LAG_PERIOD = 93 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_93,
			SUM(
			CASE
				WHEN LAG_PERIOD = 94 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_94,
			SUM(
			CASE
				WHEN LAG_PERIOD = 95 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_95,
			SUM(
			CASE
				WHEN LAG_PERIOD = 96 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_96,
			SUM(
			CASE
				WHEN LAG_PERIOD = 97 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_97,
			SUM(
			CASE
				WHEN LAG_PERIOD = 98 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_98,
			SUM(
			CASE
				WHEN LAG_PERIOD = 99 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_99,
			SUM(
			CASE
				WHEN LAG_PERIOD = 100 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_100,
			SUM(
			CASE
				WHEN LAG_PERIOD = 101 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_101,
			SUM(
			CASE
				WHEN LAG_PERIOD = 102 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_102,
			SUM(
			CASE
				WHEN LAG_PERIOD = 103 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_103,
			SUM(
			CASE
				WHEN LAG_PERIOD = 104 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_104,
			SUM(
			CASE
				WHEN LAG_PERIOD = 105 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_105,
			SUM(
			CASE
				WHEN LAG_PERIOD = 106 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_106,
			SUM(
			CASE
				WHEN LAG_PERIOD = 107 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_107,
			SUM(
			CASE
				WHEN LAG_PERIOD = 108 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_108,
			SUM(
			CASE
				WHEN LAG_PERIOD = 109 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_109,
			SUM(
			CASE
				WHEN LAG_PERIOD = 110 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_110,
			SUM(
			CASE
				WHEN LAG_PERIOD = 111 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_111,
			SUM(
			CASE
				WHEN LAG_PERIOD = 112 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_112,
			SUM(
			CASE
				WHEN LAG_PERIOD = 113 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_113,
			SUM(
			CASE
				WHEN LAG_PERIOD = 114 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_114,
			SUM(
			CASE
				WHEN LAG_PERIOD = 115 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_115,
			SUM(
			CASE
				WHEN LAG_PERIOD = 116 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_116,
			SUM(
			CASE
				WHEN LAG_PERIOD = 117 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_117,
			SUM(
			CASE
				WHEN LAG_PERIOD = 118 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_118,
			SUM(
			CASE
				WHEN LAG_PERIOD = 119 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_119,
			SUM(
			CASE
				WHEN LAG_PERIOD = 120 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_120,
			SUM(
			CASE
				WHEN LAG_PERIOD = 121 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_121,
			SUM(
			CASE
				WHEN LAG_PERIOD = 122 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_122,
			SUM(
			CASE
				WHEN LAG_PERIOD = 123 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_123,
			SUM(
			CASE
				WHEN LAG_PERIOD = 124 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_124,
			SUM(
			CASE
				WHEN LAG_PERIOD = 125 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_125,
			SUM(
			CASE
				WHEN LAG_PERIOD = 126 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_126,
			SUM(
			CASE
				WHEN LAG_PERIOD = 127 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_127,
			SUM(
			CASE
				WHEN LAG_PERIOD = 128 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_128,
			SUM(
			CASE
				WHEN LAG_PERIOD = 129 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_129,
			SUM(
			CASE
				WHEN LAG_PERIOD = 130 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_130,
			SUM(
			CASE
				WHEN LAG_PERIOD = 131 THEN LAG_PERIOD_EXP
			END) AS LAG_PERIOD_EXP_131,
			SUM(
			CASE
				WHEN LAG_PERIOD = 132 THEN LAG_PERIOD_EXP
...
***********************************************************************************/
	...
	BEGIN
		...
		SELECT SY_030_CALC_SUM_STG_SEQ.NEXTVAL                                                                         AS SY_030_CALC_SUM_STG_KEY,
			...
			NVL(RATIO_TO_REPORT(COST_FLOW_SELECTED_DLR) OVER(PARTITION BY report_type, GL_ACCOUNT, COUNTRY_INDICATOR), 0) AS COST_FLOW_RATE,
			NVL(RATIO_TO_REPORT(CALC_FACTOR) OVER(PARTITION BY REPORT_TYPE, GL_ACCOUNT, COUNTRY_INDICATOR), 0)            AS STRAIGHT_LINE_RATE,
			SYSDATE                                                                                                       AS ED_CREATE_DATE,
			l_PROC_NAME                                                                                                   AS ED_CREATE_ID,
			SYSDATE                                                                                                       AS ED_UPDATE_DATE,
			l_proc_name                                                                                                   AS ED_UPDATE_ID
		FROM
			(SELECT DISTINCT CFS.REPORT_TYPE,
				CFS.GL_ACCOUNT,
...
***********************************************************************************/
	...
	BEGIN
		...
		INSERT
			/*+ APPEND */
		INTO DBO.TEMP_030_CS_CF_DLR_PVT
			(SELECT REPORT_TYPE AS REPORT_TYPE,
					GL_ACCOUNT        AS GL_ACCOUNT,
					GL_ACCOUNT_DESCR,
					COUNTRY_INDICATOR AS COUNTRY_INDICATOR,
					...
					MAX(
					CASE
						WHEN LAG_PERIOD = 1 THEN CALC_FACTOR
					END) AS CALC_FACTOR_1,
					MAX(
					CASE
						WHEN LAG_PERIOD = 2 THEN CALC_FACTOR
					END) AS CALC_FACTOR_2,
					MAX(
					CASE
						WHEN LAG_PERIOD = 3 THEN CALC_FACTOR
					END) AS CALC_FACTOR_3,
					MAX(
					CASE
						WHEN LAG_PERIOD = 4 THEN CALC_FACTOR
					END) AS CALC_FACTOR_4,
					MAX(
					CASE
						WHEN LAG_PERIOD = 5 THEN CALC_FACTOR
					END) AS CALC_FACTOR_5,
					MAX(
					CASE
						WHEN LAG_PERIOD = 6 THEN CALC_FACTOR
					END) AS CALC_FACTOR_6,
					MAX(
					CASE
						WHEN LAG_PERIOD = 7 THEN CALC_FACTOR
					END) AS CALC_FACTOR_7,
					MAX(
					CASE
						WHEN LAG_PERIOD = 8 THEN CALC_FACTOR
					END) AS CALC_FACTOR_8,
					MAX(
					CASE
						WHEN LAG_PERIOD = 9 THEN CALC_FACTOR
					END) AS CALC_FACTOR_9,
					MAX(
					CASE
						WHEN LAG_PERIOD = 10 THEN CALC_FACTOR
					END) AS CALC_FACTOR_10,
					MAX(
					CASE
						WHEN LAG_PERIOD = 11 THEN CALC_FACTOR
					END) AS CALC_FACTOR_11,
					MAX(
					CASE
						WHEN LAG_PERIOD = 12 THEN CALC_FACTOR
					END) AS CALC_FACTOR_12,
					MAX(
					CASE
						WHEN LAG_PERIOD = 13 THEN CALC_FACTOR
					END) AS CALC_FACTOR_13,
					MAX(
					CASE
						WHEN LAG_PERIOD = 14 THEN CALC_FACTOR
					END) AS CALC_FACTOR_14,
					MAX(
					CASE
						WHEN LAG_PERIOD = 15 THEN CALC_FACTOR
					END) AS CALC_FACTOR_15,
					MAX(
					CASE
						WHEN LAG_PERIOD = 16 THEN CALC_FACTOR
					END) AS CALC_FACTOR_16,
					MAX(
					CASE
						WHEN LAG_PERIOD = 17 THEN CALC_FACTOR
					END) AS CALC_FACTOR_17,
					MAX(
					CASE
						WHEN LAG_PERIOD = 18 THEN CALC_FACTOR
					END) AS CALC_FACTOR_18,
					MAX(
					CASE
						WHEN LAG_PERIOD = 19 THEN CALC_FACTOR
					END) AS CALC_FACTOR_19,
					MAX(
					CASE
						WHEN LAG_PERIOD = 20 THEN CALC_FACTOR
					END) AS CALC_FACTOR_20,
					MAX(
					CASE
						WHEN LAG_PERIOD = 21 THEN CALC_FACTOR
					END) AS CALC_FACTOR_21,
					MAX(
					CASE
						WHEN LAG_PERIOD = 22 THEN CALC_FACTOR
					END) AS CALC_FACTOR_22,
					MAX(
					CASE
						WHEN LAG_PERIOD = 23 THEN CALC_FACTOR
					END) AS CALC_FACTOR_23,
					MAX(
					CASE
						WHEN LAG_PERIOD = 24 THEN CALC_FACTOR
					END) AS CALC_FACTOR_24,
					MAX(
					CASE
						WHEN LAG_PERIOD = 25 THEN CALC_FACTOR
					END) AS CALC_FACTOR_25,
					MAX(
					CASE
						WHEN LAG_PERIOD = 26 THEN CALC_FACTOR
					END) AS CALC_FACTOR_26,
					MAX(
					CASE
						WHEN LAG_PERIOD = 27 THEN CALC_FACTOR
					END) AS CALC_FACTOR_27,
					MAX(
					CASE
						WHEN LAG_PERIOD = 28 THEN CALC_FACTOR
					END) AS CALC_FACTOR_28,
					MAX(
					CASE
						WHEN LAG_PERIOD = 29 THEN CALC_FACTOR
					END) AS CALC_FACTOR_29,
					MAX(
					CASE
						WHEN LAG_PERIOD = 30 THEN CALC_FACTOR
					END) AS CALC_FACTOR_30,
					MAX(
					CASE
						WHEN LAG_PERIOD = 31 THEN CALC_FACTOR
					END) AS CALC_FACTOR_31,
					MAX(
					CASE
						WHEN LAG_PERIOD = 32 THEN CALC_FACTOR
					END) AS CALC_FACTOR_32,
					MAX(
					CASE
						WHEN LAG_PERIOD = 33 THEN CALC_FACTOR
					END) AS CALC_FACTOR_33,
					MAX(
					CASE
						WHEN LAG_PERIOD = 34 THEN CALC_FACTOR
					END) AS CALC_FACTOR_34,
					MAX(
					CASE
						WHEN LAG_PERIOD = 35 THEN CALC_FACTOR
					END) AS CALC_FACTOR_35,
					MAX(
					CASE
						WHEN LAG_PERIOD = 36 THEN CALC_FACTOR
					END) AS CALC_FACTOR_36,
					MAX(
					CASE
						WHEN LAG_PERIOD = 37 THEN CALC_FACTOR
					END) AS CALC_FACTOR_37,
					MAX(
					CASE
						WHEN LAG_PERIOD = 38 THEN CALC_FACTOR
					END) AS CALC_FACTOR_38,
					MAX(
					CASE
						WHEN LAG_PERIOD = 39 THEN CALC_FACTOR
					END) AS CALC_FACTOR_39,
					MAX(
					CASE
						WHEN LAG_PERIOD = 40 THEN CALC_FACTOR
					END) AS CALC_FACTOR_40,
					MAX(
					CASE
						WHEN LAG_PERIOD = 41 THEN CALC_FACTOR
					END) AS CALC_FACTOR_41,
					MAX(
					CASE
						WHEN LAG_PERIOD = 42 THEN CALC_FACTOR
					END) AS CALC_FACTOR_42,
					MAX(
					CASE
						WHEN LAG_PERIOD = 43 THEN CALC_FACTOR
					END) AS CALC_FACTOR_43,
					MAX(
					CASE
						WHEN LAG_PERIOD = 44 THEN CALC_FACTOR
					END) AS CALC_FACTOR_44,
					MAX(
					CASE
						WHEN LAG_PERIOD = 45 THEN CALC_FACTOR
					END) AS CALC_FACTOR_45,
					MAX(
					CASE
						WHEN LAG_PERIOD = 46 THEN CALC_FACTOR
					END) AS CALC_FACTOR_46,
					MAX(
					CASE
						WHEN LAG_PERIOD = 47 THEN CALC_FACTOR
					END) AS CALC_FACTOR_47,
					MAX(
					CASE
						WHEN LAG_PERIOD = 48 THEN CALC_FACTOR
					END) AS CALC_FACTOR_48,
					MAX(
					CASE
						WHEN LAG_PERIOD = 49 THEN CALC_FACTOR
					END) AS CALC_FACTOR_49,
					MAX(
					CASE
						WHEN LAG_PERIOD = 50 THEN CALC_FACTOR
					END) AS CALC_FACTOR_50,
					MAX(
					CASE
						WHEN LAG_PERIOD = 51 THEN CALC_FACTOR
					END) AS CALC_FACTOR_51,
					MAX(
					CASE
						WHEN LAG_PERIOD = 52 THEN CALC_FACTOR
					END) AS CALC_FACTOR_52,
					MAX(
					CASE
						WHEN LAG_PERIOD = 53 THEN CALC_FACTOR
					END) AS CALC_FACTOR_53,
					MAX(
					CASE
						WHEN LAG_PERIOD = 54 THEN CALC_FACTOR
					END) AS CALC_FACTOR_54,
					MAX(
					CASE
						WHEN LAG_PERIOD = 55 THEN CALC_FACTOR
					END) AS CALC_FACTOR_55,
					MAX(
					CASE
						WHEN LAG_PERIOD = 56 THEN CALC_FACTOR
					END) AS CALC_FACTOR_56,
					MAX(
					CASE
						WHEN LAG_PERIOD = 57 THEN CALC_FACTOR
					END) AS CALC_FACTOR_57,
					MAX(
					CASE
						WHEN LAG_PERIOD = 58 THEN CALC_FACTOR
					END) AS CALC_FACTOR_58,
					MAX(
					CASE
						WHEN LAG_PERIOD = 59 THEN CALC_FACTOR
					END) AS CALC_FACTOR_59,
					MAX(
					CASE
						WHEN LAG_PERIOD = 60 THEN CALC_FACTOR
					END) AS CALC_FACTOR_60,
					MAX(
					CASE
						WHEN LAG_PERIOD = 61 THEN CALC_FACTOR
					END) AS CALC_FACTOR_61,
					MAX(
					CASE
						WHEN LAG_PERIOD = 62 THEN CALC_FACTOR
					END) AS CALC_FACTOR_62,
					MAX(
					CASE
						WHEN LAG_PERIOD = 63 THEN CALC_FACTOR
					END) AS CALC_FACTOR_63,
					MAX(
					CASE
						WHEN LAG_PERIOD = 64 THEN CALC_FACTOR
					END) AS CALC_FACTOR_64,
					MAX(
					CASE
						WHEN LAG_PERIOD = 65 THEN CALC_FACTOR
					END) AS CALC_FACTOR_65,
					MAX(
					CASE
						WHEN LAG_PERIOD = 66 THEN CALC_FACTOR
					END) AS CALC_FACTOR_66,
					MAX(
					CASE
						WHEN LAG_PERIOD = 67 THEN CALC_FACTOR
					END) AS CALC_FACTOR_67,
					MAX(
					CASE
						WHEN LAG_PERIOD = 68 THEN CALC_FACTOR
					END) AS CALC_FACTOR_68,
					MAX(
					CASE
						WHEN LAG_PERIOD = 69 THEN CALC_FACTOR
					END) AS CALC_FACTOR_69,
					MAX(
					CASE
						WHEN LAG_PERIOD = 70 THEN CALC_FACTOR
					END) AS CALC_FACTOR_70,
					MAX(
					CASE
						WHEN LAG_PERIOD = 71 THEN CALC_FACTOR
					END) AS CALC_FACTOR_71,
					MAX(
					CASE
						WHEN LAG_PERIOD = 72 THEN CALC_FACTOR
					END) AS CALC_FACTOR_72,
					MAX(
					CASE
						WHEN LAG_PERIOD = 73 THEN CALC_FACTOR
					END) AS CALC_FACTOR_73,
					MAX(
					CASE
						WHEN LAG_PERIOD = 74 THEN CALC_FACTOR
					END) AS CALC_FACTOR_74,
					MAX(
					CASE
						WHEN LAG_PERIOD = 75 THEN CALC_FACTOR
					END) AS CALC_FACTOR_75,
					MAX(
					CASE
						WHEN LAG_PERIOD = 76 THEN CALC_FACTOR
					END) AS CALC_FACTOR_76,
					MAX(
					CASE
						WHEN LAG_PERIOD = 77 THEN CALC_FACTOR
					END) AS CALC_FACTOR_77,
					MAX(
					CASE
						WHEN LAG_PERIOD = 78 THEN CALC_FACTOR
					END) AS CALC_FACTOR_78,
					MAX(
					CASE
						WHEN LAG_PERIOD = 79 THEN CALC_FACTOR
					END) AS CALC_FACTOR_79,
					MAX(
					CASE
						WHEN LAG_PERIOD = 80 THEN CALC_FACTOR
					END) AS CALC_FACTOR_80,
					MAX(
					CASE
						WHEN LAG_PERIOD = 81 THEN CALC_FACTOR
					END) AS CALC_FACTOR_81,
					MAX(
					CASE
						WHEN LAG_PERIOD = 82 THEN CALC_FACTOR
					END) AS CALC_FACTOR_82,
					MAX(
					CASE
						WHEN LAG_PERIOD = 83 THEN CALC_FACTOR
					END) AS CALC_FACTOR_83,
					MAX(
					CASE
						WHEN LAG_PERIOD = 84 THEN CALC_FACTOR
					END) AS CALC_FACTOR_84,
					MAX(
					CASE
						WHEN LAG_PERIOD = 85 THEN CALC_FACTOR
					END) AS CALC_FACTOR_85,
					MAX(
					CASE
						WHEN LAG_PERIOD = 86 THEN CALC_FACTOR
					END) AS CALC_FACTOR_86,
					MAX(
					CASE
						WHEN LAG_PERIOD = 87 THEN CALC_FACTOR
					END) AS CALC_FACTOR_87,
					MAX(
					CASE
						WHEN LAG_PERIOD = 88 THEN CALC_FACTOR
					END) AS CALC_FACTOR_88,
					MAX(
					CASE
						WHEN LAG_PERIOD = 89 THEN CALC_FACTOR
					END) AS CALC_FACTOR_89,
					MAX(
					CASE
						WHEN LAG_PERIOD = 90 THEN CALC_FACTOR
					END) AS CALC_FACTOR_90,
					MAX(
					CASE
						WHEN LAG_PERIOD = 91 THEN CALC_FACTOR
					END) AS CALC_FACTOR_91,
					MAX(
					CASE
						WHEN LAG_PERIOD = 92 THEN CALC_FACTOR
					END) AS CALC_FACTOR_92,
					MAX(
					CASE
						WHEN LAG_PERIOD = 93 THEN CALC_FACTOR
					END) AS CALC_FACTOR_93,
					MAX(
					CASE
						WHEN LAG_PERIOD = 94 THEN CALC_FACTOR
					END) AS CALC_FACTOR_94,
					MAX(
					CASE
						WHEN LAG_PERIOD = 95 THEN CALC_FACTOR
					END) AS CALC_FACTOR_95,
					MAX(
					CASE
						WHEN LAG_PERIOD = 96 THEN CALC_FACTOR
					END) AS CALC_FACTOR_96,
					MAX(
					CASE
						WHEN LAG_PERIOD = 97 THEN CALC_FACTOR
					END) AS CALC_FACTOR_97,
					MAX(
					CASE
						WHEN LAG_PERIOD = 98 THEN CALC_FACTOR
					END) AS CALC_FACTOR_98,
					MAX(
					CASE
						WHEN LAG_PERIOD = 99 THEN CALC_FACTOR
					END) AS CALC_FACTOR_99,
					MAX(
					CASE
						WHEN LAG_PERIOD = 100 THEN CALC_FACTOR
					END) AS CALC_FACTOR_100,
					MAX(
					CASE
						WHEN LAG_PERIOD = 101 THEN CALC_FACTOR
					END) AS CALC_FACTOR_101,
					MAX(
					CASE
						WHEN LAG_PERIOD = 102 THEN CALC_FACTOR
					END) AS CALC_FACTOR_102,
					MAX(
					CASE
						WHEN LAG_PERIOD = 103 THEN CALC_FACTOR
					END) AS CALC_FACTOR_103,
					MAX(
					CASE
						WHEN LAG_PERIOD = 104 THEN CALC_FACTOR
					END) AS CALC_FACTOR_104,
					MAX(
					CASE
						WHEN LAG_PERIOD = 105 THEN CALC_FACTOR
					END) AS CALC_FACTOR_105,
					MAX(
					CASE
						WHEN LAG_PERIOD = 106 THEN CALC_FACTOR
					END) AS CALC_FACTOR_106,
					MAX(
					CASE
						WHEN LAG_PERIOD = 107 THEN CALC_FACTOR
					END) AS CALC_FACTOR_107,
					MAX(
					CASE
						WHEN LAG_PERIOD = 108 THEN CALC_FACTOR
					END) AS CALC_FACTOR_108,
					MAX(
					CASE
						WHEN LAG_PERIOD = 109 THEN CALC_FACTOR
					END) AS CALC_FACTOR_109,
					MAX(
					CASE
						WHEN LAG_PERIOD = 110 THEN CALC_FACTOR
					END) AS CALC_FACTOR_110,
					MAX(
					CASE
						WHEN LAG_PERIOD = 111 THEN CALC_FACTOR
					END) AS CALC_FACTOR_111,
					MAX(
					CASE
						WHEN LAG_PERIOD = 112 THEN CALC_FACTOR
					END) AS CALC_FACTOR_112,
					MAX(
					CASE
						WHEN LAG_PERIOD = 113 THEN CALC_FACTOR
					END) AS CALC_FACTOR_113,
					MAX(
					CASE
						WHEN LAG_PERIOD = 114 THEN CALC_FACTOR
					END) AS CALC_FACTOR_114,
					MAX(
					CASE
						WHEN LAG_PERIOD = 115 THEN CALC_FACTOR
					END) AS CALC_FACTOR_115,
					MAX(
					CASE
						WHEN LAG_PERIOD = 116 THEN CALC_FACTOR
					END) AS CALC_FACTOR_116,
					MAX(
					CASE
						WHEN LAG_PERIOD = 117 THEN CALC_FACTOR
					END) AS CALC_FACTOR_117,
					MAX(
					CASE
						WHEN LAG_PERIOD = 118 THEN CALC_FACTOR
					END) AS CALC_FACTOR_118,
					MAX(
					CASE
						WHEN LAG_PERIOD = 119 THEN CALC_FACTOR
					END) AS CALC_FACTOR_119,
					MAX(
					CASE
						WHEN LAG_PERIOD = 120 THEN CALC_FACTOR
					END) AS CALC_FACTOR_120,
					MAX(
					CASE
						WHEN LAG_PERIOD = 121 THEN CALC_FACTOR
					END) AS CALC_FACTOR_121,
					MAX(
					CASE
						WHEN LAG_PERIOD = 122 THEN CALC_FACTOR
					END) AS CALC_FACTOR_122,
					MAX(
					CASE
						WHEN LAG_PERIOD = 123 THEN CALC_FACTOR
					END) AS CALC_FACTOR_123,
					MAX(
					CASE
						WHEN LAG_PERIOD = 124 THEN CALC_FACTOR
					END) AS CALC_FACTOR_124,
					MAX(
					CASE
						WHEN LAG_PERIOD = 125 THEN CALC_FACTOR
					END) AS CALC_FACTOR_125,
					MAX(
					CASE
						WHEN LAG_PERIOD = 126 THEN CALC_FACTOR
					END) AS CALC_FACTOR_126,
					MAX(
					CASE
						WHEN LAG_PERIOD = 127 THEN CALC_FACTOR
					END) AS CALC_FACTOR_127,
					MAX(
					CASE
						WHEN LAG_PERIOD = 128 THEN CALC_FACTOR
					END) AS CALC_FACTOR_128,
					MAX(
					CASE
						WHEN LAG_PERIOD = 129 THEN CALC_FACTOR
					END) AS CALC_FACTOR_129,
					MAX(
					CASE
						WHEN LAG_PERIOD = 130 THEN CALC_FACTOR
					END) AS CALC_FACTOR_130,
					MAX(
					CASE
						WHEN LAG_PERIOD = 131 THEN CALC_FACTOR
					END) AS CALC_FACTOR_131,
					MAX(
					CASE
						WHEN LAG_PERIOD = 132 THEN CALC_FACTOR
					END) AS CALC_FACTOR_132,
					SUM(
					CASE
						WHEN LAG_PERIOD = 1 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_1,
					SUM(
					CASE
						WHEN LAG_PERIOD = 2 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_2,
					SUM(
					CASE
						WHEN LAG_PERIOD = 3 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_3,
					SUM(
					CASE
						WHEN LAG_PERIOD = 4 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_4,
					SUM(
					CASE
						WHEN LAG_PERIOD = 5 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_5,
					SUM(
					CASE
						WHEN LAG_PERIOD = 6 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_6,
					SUM(
					CASE
						WHEN LAG_PERIOD = 7 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_7,
					SUM(
					CASE
						WHEN LAG_PERIOD = 8 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_8,
					SUM(
					CASE
						WHEN LAG_PERIOD = 9 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_9,
					SUM(
					CASE
						WHEN LAG_PERIOD = 10 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_10,
					SUM(
					CASE
						WHEN LAG_PERIOD = 11 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_11,
					SUM(
					CASE
						WHEN LAG_PERIOD = 12 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_12,
					SUM(
					CASE
						WHEN LAG_PERIOD = 13 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_13,
					SUM(
					CASE
						WHEN LAG_PERIOD = 14 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_14,
					SUM(
					CASE
						WHEN LAG_PERIOD = 15 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_15,
					SUM(
					CASE
						WHEN LAG_PERIOD = 16 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_16,
					SUM(
					CASE
						WHEN LAG_PERIOD = 17 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_17,
					SUM(
					CASE
						WHEN LAG_PERIOD = 18 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_18,
					SUM(
					CASE
						WHEN LAG_PERIOD = 19 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_19,
					SUM(
					CASE
						WHEN LAG_PERIOD = 20 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_20,
					SUM(
					CASE
						WHEN LAG_PERIOD = 21 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_21,
					SUM(
					CASE
						WHEN LAG_PERIOD = 22 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_22,
					SUM(
					CASE
						WHEN LAG_PERIOD = 23 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_23,
					SUM(
					CASE
						WHEN LAG_PERIOD = 24 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_24,
					SUM(
					CASE
						WHEN LAG_PERIOD = 25 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_25,
					SUM(
					CASE
						WHEN LAG_PERIOD = 26 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_26,
					SUM(
					CASE
						WHEN LAG_PERIOD = 27 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_27,
					SUM(
					CASE
						WHEN LAG_PERIOD = 28 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_28,
					SUM(
					CASE
						WHEN LAG_PERIOD = 29 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_29,
					SUM(
					CASE
						WHEN LAG_PERIOD = 30 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_30,
					SUM(
					CASE
						WHEN LAG_PERIOD = 31 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_31,
					SUM(
					CASE
						WHEN LAG_PERIOD = 32 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_32,
					SUM(
					CASE
						WHEN LAG_PERIOD = 33 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_33,
					SUM(
					CASE
						WHEN LAG_PERIOD = 34 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_34,
					SUM(
					CASE
						WHEN LAG_PERIOD = 35 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_35,
					SUM(
					CASE
						WHEN LAG_PERIOD = 36 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_36,
					SUM(
					CASE
						WHEN LAG_PERIOD = 37 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_37,
					SUM(
					CASE
						WHEN LAG_PERIOD = 38 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_38,
					SUM(
					CASE
						WHEN LAG_PERIOD = 39 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_39,
					SUM(
					CASE
						WHEN LAG_PERIOD = 40 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_40,
					SUM(
					CASE
						WHEN LAG_PERIOD = 41 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_41,
					SUM(
					CASE
						WHEN LAG_PERIOD = 42 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_42,
					SUM(
					CASE
						WHEN LAG_PERIOD = 43 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_43,
					SUM(
					CASE
						WHEN LAG_PERIOD = 44 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_44,
					SUM(
					CASE
						WHEN LAG_PERIOD = 45 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_45,
					SUM(
					CASE
						WHEN LAG_PERIOD = 46 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_46,
					SUM(
					CASE
						WHEN LAG_PERIOD = 47 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_47,
					SUM(
					CASE
						WHEN LAG_PERIOD = 48 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_48,
					SUM(
					CASE
						WHEN LAG_PERIOD = 49 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_49,
					SUM(
					CASE
						WHEN LAG_PERIOD = 50 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_50,
					SUM(
					CASE
						WHEN LAG_PERIOD = 51 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_51,
					SUM(
					CASE
						WHEN LAG_PERIOD = 52 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_52,
					SUM(
					CASE
						WHEN LAG_PERIOD = 53 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_53,
					SUM(
					CASE
						WHEN LAG_PERIOD = 54 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_54,
					SUM(
					CASE
						WHEN LAG_PERIOD = 55 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_55,
					SUM(
					CASE
						WHEN LAG_PERIOD = 56 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_56,
					SUM(
					CASE
						WHEN LAG_PERIOD = 57 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_57,
					SUM(
					CASE
						WHEN LAG_PERIOD = 58 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_58,
					SUM(
					CASE
						WHEN LAG_PERIOD = 59 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_59,
					SUM(
					CASE
						WHEN LAG_PERIOD = 60 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_60,
					SUM(
					CASE
						WHEN LAG_PERIOD = 61 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_61,
					SUM(
					CASE
						WHEN LAG_PERIOD = 62 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_62,
					SUM(
					CASE
						WHEN LAG_PERIOD = 63 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_63,
					SUM(
					CASE
						WHEN LAG_PERIOD = 64 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_64,
					SUM(
					CASE
						WHEN LAG_PERIOD = 65 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_65,
					SUM(
					CASE
						WHEN LAG_PERIOD = 66 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_66,
					SUM(
					CASE
						WHEN LAG_PERIOD = 67 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_67,
					SUM(
					CASE
						WHEN LAG_PERIOD = 68 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_68,
					SUM(
					CASE
						WHEN LAG_PERIOD = 69 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_69,
					SUM(
					CASE
						WHEN LAG_PERIOD = 70 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_70,
					SUM(
					CASE
						WHEN LAG_PERIOD = 71 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_71,
					SUM(
					CASE
						WHEN LAG_PERIOD = 72 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_72,
					SUM(
					CASE
						WHEN LAG_PERIOD = 73 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_73,
					SUM(
					CASE
						WHEN LAG_PERIOD = 74 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_74,
					SUM(
					CASE
						WHEN LAG_PERIOD = 75 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_75,
					SUM(
					CASE
						WHEN LAG_PERIOD = 76 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_76,
					SUM(
					CASE
						WHEN LAG_PERIOD = 77 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_77,
					SUM(
					CASE
						WHEN LAG_PERIOD = 78 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_78,
					SUM(
					CASE
						WHEN LAG_PERIOD = 79 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_79,
					SUM(
					CASE
						WHEN LAG_PERIOD = 80 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_80,
					SUM(
					CASE
						WHEN LAG_PERIOD = 81 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_81,
					SUM(
					CASE
						WHEN LAG_PERIOD = 82 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_82,
					SUM(
					CASE
						WHEN LAG_PERIOD = 83 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_83,
					SUM(
					CASE
						WHEN LAG_PERIOD = 84 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_84,
					SUM(
					CASE
						WHEN LAG_PERIOD = 85 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_85,
					SUM(
					CASE
						WHEN LAG_PERIOD = 86 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_86,
					SUM(
					CASE
						WHEN LAG_PERIOD = 87 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_87,
					SUM(
					CASE
						WHEN LAG_PERIOD = 88 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_88,
					SUM(
					CASE
						WHEN LAG_PERIOD = 89 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_89,
					SUM(
					CASE
						WHEN LAG_PERIOD = 90 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_90,
					SUM(
					CASE
						WHEN LAG_PERIOD = 91 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_91,
					SUM(
					CASE
						WHEN LAG_PERIOD = 92 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_92,
					SUM(
					CASE
						WHEN LAG_PERIOD = 93 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_93,
					SUM(
					CASE
						WHEN LAG_PERIOD = 94 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_94,
					SUM(
					CASE
						WHEN LAG_PERIOD = 95 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_95,
					SUM(
					CASE
						WHEN LAG_PERIOD = 96 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_96,
					SUM(
					CASE
						WHEN LAG_PERIOD = 97 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_97,
					SUM(
					CASE
						WHEN LAG_PERIOD = 98 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_98,
					SUM(
					CASE
						WHEN LAG_PERIOD = 99 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_99,
					SUM(
					CASE
						WHEN LAG_PERIOD = 100 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_100,
					SUM(
					CASE
						WHEN LAG_PERIOD = 101 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_101,
					SUM(
					CASE
						WHEN LAG_PERIOD = 102 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_102,
					SUM(
					CASE
						WHEN LAG_PERIOD = 103 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_103,
					SUM(
					CASE
						WHEN LAG_PERIOD = 104 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_104,
					SUM(
					CASE
						WHEN LAG_PERIOD = 105 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_105,
					SUM(
					CASE
						WHEN LAG_PERIOD = 106 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_106,
					SUM(
					CASE
						WHEN LAG_PERIOD = 107 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_107,
					SUM(
					CASE
						WHEN LAG_PERIOD = 108 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_108,
					SUM(
					CASE
						WHEN LAG_PERIOD = 109 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_109,
					SUM(
					CASE
						WHEN LAG_PERIOD = 110 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_110,
					SUM(
					CASE
						WHEN LAG_PERIOD = 111 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_111,
					SUM(
					CASE
						WHEN LAG_PERIOD = 112 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_112,
					SUM(
					CASE
						WHEN LAG_PERIOD = 113 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_113,
					SUM(
					CASE
						WHEN LAG_PERIOD = 114 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_114,
					SUM(
					CASE
						WHEN LAG_PERIOD = 115 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_115,
					SUM(
					CASE
						WHEN LAG_PERIOD = 116 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_116,
					SUM(
					CASE
						WHEN LAG_PERIOD = 117 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_117,
					SUM(
					CASE
						WHEN LAG_PERIOD = 118 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_118,
					SUM(
					CASE
						WHEN LAG_PERIOD = 119 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_119,
					SUM(
					CASE
						WHEN LAG_PERIOD = 120 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_120,
					SUM(
					CASE
						WHEN LAG_PERIOD = 121 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_121,
					SUM(
					CASE
						WHEN LAG_PERIOD = 122 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_122,
					SUM(
					CASE
						WHEN LAG_PERIOD = 123 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_123,
					SUM(
					CASE
						WHEN LAG_PERIOD = 124 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_124,
					SUM(
					CASE
						WHEN LAG_PERIOD = 125 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_125,
					SUM(
					CASE
						WHEN LAG_PERIOD = 126 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_126,
					SUM(
					CASE
						WHEN LAG_PERIOD = 127 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_127,
					SUM(
					CASE
						WHEN LAG_PERIOD = 128 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_128,
					SUM(
					CASE
						WHEN LAG_PERIOD = 129 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_129,
					SUM(
					CASE
						WHEN LAG_PERIOD = 130 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_130,
					SUM(
					CASE
						WHEN LAG_PERIOD = 131 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_131,
					SUM(
					CASE
						WHEN LAG_PERIOD = 132 THEN COST_FLOW_SELECTED_DLR
					END) AS COST_FLOW_SELECTED_DLR_132
				FROM DBO.SY_030_CALC_SUM_STG
				GROUP BY REPORT_TYPE,
					GL_ACCOUNT,
					GL_ACCOUNT_DESCR,
					COUNTRY_INDICATOR,
	...
	BEGIN
		...
		INTO DBO.TEMP_030_CS_RATE_PVT
			(SELECT REPORT_TYPE AS REPORT_TYPE,
					...
					SUM(
					CASE
						WHEN LAG_PERIOD = 1 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_1,
					SUM(
					CASE
						WHEN LAG_PERIOD = 2 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_2,
					SUM(
					CASE
						WHEN LAG_PERIOD = 3 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_3,
					SUM(
					CASE
						WHEN LAG_PERIOD = 4 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_4,
					SUM(
					CASE
						WHEN LAG_PERIOD = 5 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_5,
					SUM(
					CASE
						WHEN LAG_PERIOD = 6 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_6,
					SUM(
					CASE
						WHEN LAG_PERIOD = 7 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_7,
					SUM(
					CASE
						WHEN LAG_PERIOD = 8 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_8,
					SUM(
					CASE
						WHEN LAG_PERIOD = 9 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_9,
					SUM(
					CASE
						WHEN LAG_PERIOD = 10 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_10,
					SUM(
					CASE
						WHEN LAG_PERIOD = 11 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_11,
					SUM(
					CASE
						WHEN LAG_PERIOD = 12 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_12,
					SUM(
					CASE
						WHEN LAG_PERIOD = 13 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_13,
					SUM(
					CASE
						WHEN LAG_PERIOD = 14 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_14,
					SUM(
					CASE
						WHEN LAG_PERIOD = 15 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_15,
					SUM(
					CASE
						WHEN LAG_PERIOD = 16 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_16,
					SUM(
					CASE
						WHEN LAG_PERIOD = 17 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_17,
					SUM(
					CASE
						WHEN LAG_PERIOD = 18 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_18,
					SUM(
					CASE
						WHEN LAG_PERIOD = 19 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_19,
					SUM(
					CASE
						WHEN LAG_PERIOD = 20 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_20,
					SUM(
					CASE
						WHEN LAG_PERIOD = 21 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_21,
					SUM(
					CASE
						WHEN LAG_PERIOD = 22 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_22,
					SUM(
					CASE
						WHEN LAG_PERIOD = 23 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_23,
					SUM(
					CASE
						WHEN LAG_PERIOD = 24 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_24,
					SUM(
					CASE
						WHEN LAG_PERIOD = 25 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_25,
					SUM(
					CASE
						WHEN LAG_PERIOD = 26 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_26,
					SUM(
					CASE
						WHEN LAG_PERIOD = 27 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_27,
					SUM(
					CASE
						WHEN LAG_PERIOD = 28 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_28,
					SUM(
					CASE
						WHEN LAG_PERIOD = 29 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_29,
					SUM(
					CASE
						WHEN LAG_PERIOD = 30 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_30,
					SUM(
					CASE
						WHEN LAG_PERIOD = 31 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_31,
					SUM(
					CASE
						WHEN LAG_PERIOD = 32 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_32,
					SUM(
					CASE
						WHEN LAG_PERIOD = 33 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_33,
					SUM(
					CASE
						WHEN LAG_PERIOD = 34 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_34,
					SUM(
					CASE
						WHEN LAG_PERIOD = 35 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_35,
					SUM(
					CASE
						WHEN LAG_PERIOD = 36 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_36,
					SUM(
					CASE
						WHEN LAG_PERIOD = 37 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_37,
					SUM(
					CASE
						WHEN LAG_PERIOD = 38 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_38,
					SUM(
					CASE
						WHEN LAG_PERIOD = 39 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_39,
					SUM(
					CASE
						WHEN LAG_PERIOD = 40 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_40,
					SUM(
					CASE
						WHEN LAG_PERIOD = 41 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_41,
					SUM(
					CASE
						WHEN LAG_PERIOD = 42 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_42,
					SUM(
					CASE
						WHEN LAG_PERIOD = 43 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_43,
					SUM(
					CASE
						WHEN LAG_PERIOD = 44 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_44,
					SUM(
					CASE
						WHEN LAG_PERIOD = 45 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_45,
					SUM(
					CASE
						WHEN LAG_PERIOD = 46 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_46,
					SUM(
					CASE
						WHEN LAG_PERIOD = 47 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_47,
					SUM(
					CASE
						WHEN LAG_PERIOD = 48 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_48,
					SUM(
					CASE
						WHEN LAG_PERIOD = 49 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_49,
					SUM(
					CASE
						WHEN LAG_PERIOD = 50 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_50,
					SUM(
					CASE
						WHEN LAG_PERIOD = 51 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_51,
					SUM(
					CASE
						WHEN LAG_PERIOD = 52 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_52,
					SUM(
					CASE
						WHEN LAG_PERIOD = 53 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_53,
					SUM(
					CASE
						WHEN LAG_PERIOD = 54 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_54,
					SUM(
					CASE
						WHEN LAG_PERIOD = 55 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_55,
					SUM(
					CASE
						WHEN LAG_PERIOD = 56 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_56,
					SUM(
					CASE
						WHEN LAG_PERIOD = 57 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_57,
					SUM(
					CASE
						WHEN LAG_PERIOD = 58 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_58,
					SUM(
					CASE
						WHEN LAG_PERIOD = 59 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_59,
					SUM(
					CASE
						WHEN LAG_PERIOD = 60 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_60,
					SUM(
					CASE
						WHEN LAG_PERIOD = 61 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_61,
					SUM(
					CASE
						WHEN LAG_PERIOD = 62 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_62,
					SUM(
					CASE
						WHEN LAG_PERIOD = 63 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_63,
					SUM(
					CASE
						WHEN LAG_PERIOD = 64 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_64,
					SUM(
					CASE
						WHEN LAG_PERIOD = 65 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_65,
					SUM(
					CASE
						WHEN LAG_PERIOD = 66 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_66,
					SUM(
					CASE
						WHEN LAG_PERIOD = 67 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_67,
					SUM(
					CASE
						WHEN LAG_PERIOD = 68 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_68,
					SUM(
					CASE
						WHEN LAG_PERIOD = 69 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_69,
					SUM(
					CASE
						WHEN LAG_PERIOD = 70 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_70,
					SUM(
					CASE
						WHEN LAG_PERIOD = 71 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_71,
					SUM(
					CASE
						WHEN LAG_PERIOD = 72 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_72,
					SUM(
					CASE
						WHEN LAG_PERIOD = 73 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_73,
					SUM(
					CASE
						WHEN LAG_PERIOD = 74 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_74,
					SUM(
					CASE
						WHEN LAG_PERIOD = 75 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_75,
					SUM(
					CASE
						WHEN LAG_PERIOD = 76 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_76,
					SUM(
					CASE
						WHEN LAG_PERIOD = 77 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_77,
					SUM(
					CASE
						WHEN LAG_PERIOD = 78 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_78,
					SUM(
					CASE
						WHEN LAG_PERIOD = 79 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_79,
					SUM(
					CASE
						WHEN LAG_PERIOD = 80 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_80,
					SUM(
					CASE
						WHEN LAG_PERIOD = 81 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_81,
					SUM(
					CASE
						WHEN LAG_PERIOD = 82 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_82,
					SUM(
					CASE
						WHEN LAG_PERIOD = 83 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_83,
					SUM(
					CASE
						WHEN LAG_PERIOD = 84 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_84,
					SUM(
					CASE
						WHEN LAG_PERIOD = 85 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_85,
					SUM(
					CASE
						WHEN LAG_PERIOD = 86 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_86,
					SUM(
					CASE
						WHEN LAG_PERIOD = 87 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_87,
					SUM(
					CASE
						WHEN LAG_PERIOD = 88 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_88,
					SUM(
					CASE
						WHEN LAG_PERIOD = 89 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_89,
					SUM(
					CASE
						WHEN LAG_PERIOD = 90 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_90,
					SUM(
					CASE
						WHEN LAG_PERIOD = 91 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_91,
					SUM(
					CASE
						WHEN LAG_PERIOD = 92 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_92,
					SUM(
					CASE
						WHEN LAG_PERIOD = 93 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_93,
					SUM(
					CASE
						WHEN LAG_PERIOD = 94 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_94,
					SUM(
					CASE
						WHEN LAG_PERIOD = 95 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_95,
					SUM(
					CASE
						WHEN LAG_PERIOD = 96 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_96,
					SUM(
					CASE
						WHEN LAG_PERIOD = 97 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_97,
					SUM(
					CASE
						WHEN LAG_PERIOD = 98 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_98,
					SUM(
					CASE
						WHEN LAG_PERIOD = 99 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_99,
					SUM(
					CASE
						WHEN LAG_PERIOD = 100 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_100,
					SUM(
					CASE
						WHEN LAG_PERIOD = 101 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_101,
					SUM(
					CASE
						WHEN LAG_PERIOD = 102 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_102,
					SUM(
					CASE
						WHEN LAG_PERIOD = 103 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_103,
					SUM(
					CASE
						WHEN LAG_PERIOD = 104 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_104,
					SUM(
					CASE
						WHEN LAG_PERIOD = 105 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_105,
					SUM(
					CASE
						WHEN LAG_PERIOD = 106 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_106,
					SUM(
					CASE
						WHEN LAG_PERIOD = 107 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_107,
					SUM(
					CASE
						WHEN LAG_PERIOD = 108 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_108,
					SUM(
					CASE
						WHEN LAG_PERIOD = 109 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_109,
					SUM(
					CASE
						WHEN LAG_PERIOD = 110 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_110,
					SUM(
					CASE
						WHEN LAG_PERIOD = 111 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_111,
					SUM(
					CASE
						WHEN LAG_PERIOD = 112 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_112,
					SUM(
					CASE
						WHEN LAG_PERIOD = 113 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_113,
					SUM(
					CASE
						WHEN LAG_PERIOD = 114 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_114,
					SUM(
					CASE
						WHEN LAG_PERIOD = 115 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_115,
					SUM(
					CASE
						WHEN LAG_PERIOD = 116 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_116,
					SUM(
					CASE
						WHEN LAG_PERIOD = 117 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_117,
					SUM(
					CASE
						WHEN LAG_PERIOD = 118 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_118,
					SUM(
					CASE
						WHEN LAG_PERIOD = 119 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_119,
					SUM(
					CASE
						WHEN LAG_PERIOD = 120 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_120,
					SUM(
					CASE
						WHEN LAG_PERIOD = 121 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_121,
					SUM(
					CASE
						WHEN LAG_PERIOD = 122 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_122,
					SUM(
					CASE
						WHEN LAG_PERIOD = 123 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_123,
					SUM(
					CASE
						WHEN LAG_PERIOD = 124 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_124,
					SUM(
					CASE
						WHEN LAG_PERIOD = 125 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_125,
					SUM(
					CASE
						WHEN LAG_PERIOD = 126 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_126,
					SUM(
					CASE
						WHEN LAG_PERIOD = 127 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_127,
					SUM(
					CASE
						WHEN LAG_PERIOD = 128 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_128,
					SUM(
					CASE
						WHEN LAG_PERIOD = 129 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_129,
					SUM(
					CASE
						WHEN LAG_PERIOD = 130 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_130,
					SUM(
					CASE
						WHEN LAG_PERIOD = 131 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_131,
					SUM(
					CASE
						WHEN LAG_PERIOD = 132 THEN COST_FLOW_RATE
					END) AS COST_FLOW_RATE_132,
					SUM(
					CASE
						WHEN LAG_PERIOD = 1 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_1,
					SUM(
					CASE
						WHEN LAG_PERIOD = 2 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_2,
					SUM(
					CASE
						WHEN LAG_PERIOD = 3 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_3,
					SUM(
					CASE
						WHEN LAG_PERIOD = 4 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_4,
					SUM(
					CASE
						WHEN LAG_PERIOD = 5 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_5,
					SUM(
					CASE
						WHEN LAG_PERIOD = 6 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_6,
					SUM(
					CASE
						WHEN LAG_PERIOD = 7 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_7,
					SUM(
					CASE
						WHEN LAG_PERIOD = 8 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_8,
					SUM(
					CASE
						WHEN LAG_PERIOD = 9 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_9,
					SUM(
					CASE
						WHEN LAG_PERIOD = 10 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_10,
					SUM(
					CASE
						WHEN LAG_PERIOD = 11 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_11,
					SUM(
					CASE
						WHEN LAG_PERIOD = 12 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_12,
					SUM(
					CASE
						WHEN LAG_PERIOD = 13 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_13,
					SUM(
					CASE
						WHEN LAG_PERIOD = 14 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_14,
					SUM(
					CASE
						WHEN LAG_PERIOD = 15 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_15,
					SUM(
					CASE
						WHEN LAG_PERIOD = 16 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_16,
					SUM(
					CASE
						WHEN LAG_PERIOD = 17 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_17,
					SUM(
					CASE
						WHEN LAG_PERIOD = 18 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_18,
					SUM(
					CASE
						WHEN LAG_PERIOD = 19 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_19,
					SUM(
					CASE
						WHEN LAG_PERIOD = 20 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_20,
					SUM(
					CASE
						WHEN LAG_PERIOD = 21 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_21,
					SUM(
					CASE
						WHEN LAG_PERIOD = 22 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_22,
					SUM(
					CASE
						WHEN LAG_PERIOD = 23 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_23,
					SUM(
					CASE
						WHEN LAG_PERIOD = 24 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_24,
					SUM(
					CASE
						WHEN LAG_PERIOD = 25 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_25,
					SUM(
					CASE
						WHEN LAG_PERIOD = 26 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_26,
					SUM(
					CASE
						WHEN LAG_PERIOD = 27 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_27,
					SUM(
					CASE
						WHEN LAG_PERIOD = 28 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_28,
					SUM(
					CASE
						WHEN LAG_PERIOD = 29 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_29,
					SUM(
					CASE
						WHEN LAG_PERIOD = 30 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_30,
					SUM(
					CASE
						WHEN LAG_PERIOD = 31 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_31,
					SUM(
					CASE
						WHEN LAG_PERIOD = 32 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_32,
					SUM(
					CASE
						WHEN LAG_PERIOD = 33 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_33,
					SUM(
					CASE
						WHEN LAG_PERIOD = 34 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_34,
					SUM(
					CASE
						WHEN LAG_PERIOD = 35 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_35,
					SUM(
					CASE
						WHEN LAG_PERIOD = 36 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_36,
					SUM(
					CASE
						WHEN LAG_PERIOD = 37 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_37,
					SUM(
					CASE
						WHEN LAG_PERIOD = 38 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_38,
					SUM(
					CASE
						WHEN LAG_PERIOD = 39 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_39,
					SUM(
					CASE
						WHEN LAG_PERIOD = 40 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_40,
					SUM(
					CASE
						WHEN LAG_PERIOD = 41 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_41,
					SUM(
					CASE
						WHEN LAG_PERIOD = 42 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_42,
					SUM(
					CASE
						WHEN LAG_PERIOD = 43 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_43,
					SUM(
					CASE
						WHEN LAG_PERIOD = 44 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_44,
					SUM(
					CASE
						WHEN LAG_PERIOD = 45 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_45,
					SUM(
					CASE
						WHEN LAG_PERIOD = 46 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_46,
					SUM(
					CASE
						WHEN LAG_PERIOD = 47 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_47,
					SUM(
					CASE
						WHEN LAG_PERIOD = 48 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_48,
					SUM(
					CASE
						WHEN LAG_PERIOD = 49 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_49,
					SUM(
					CASE
						WHEN LAG_PERIOD = 50 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_50,
					SUM(
					CASE
						WHEN LAG_PERIOD = 51 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_51,
					SUM(
					CASE
						WHEN LAG_PERIOD = 52 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_52,
					SUM(
					CASE
						WHEN LAG_PERIOD = 53 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_53,
					SUM(
					CASE
						WHEN LAG_PERIOD = 54 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_54,
					SUM(
					CASE
						WHEN LAG_PERIOD = 55 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_55,
					SUM(
					CASE
						WHEN LAG_PERIOD = 56 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_56,
					SUM(
					CASE
						WHEN LAG_PERIOD = 57 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_57,
					SUM(
					CASE
						WHEN LAG_PERIOD = 58 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_58,
					SUM(
					CASE
						WHEN LAG_PERIOD = 59 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_59,
					SUM(
					CASE
						WHEN LAG_PERIOD = 60 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_60,
					SUM(
					CASE
						WHEN LAG_PERIOD = 61 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_61,
					SUM(
					CASE
						WHEN LAG_PERIOD = 62 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_62,
					SUM(
					CASE
						WHEN LAG_PERIOD = 63 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_63,
					SUM(
					CASE
						WHEN LAG_PERIOD = 64 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_64,
					SUM(
					CASE
						WHEN LAG_PERIOD = 65 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_65,
					SUM(
					CASE
						WHEN LAG_PERIOD = 66 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_66,
					SUM(
					CASE
						WHEN LAG_PERIOD = 67 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_67,
					SUM(
					CASE
						WHEN LAG_PERIOD = 68 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_68,
					SUM(
					CASE
						WHEN LAG_PERIOD = 69 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_69,
					SUM(
					CASE
						WHEN LAG_PERIOD = 70 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_70,
					SUM(
					CASE
						WHEN LAG_PERIOD = 71 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_71,
					SUM(
					CASE
						WHEN LAG_PERIOD = 72 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_72,
					SUM(
					CASE
						WHEN LAG_PERIOD = 73 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_73,
					SUM(
					CASE
						WHEN LAG_PERIOD = 74 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_74,
					SUM(
					CASE
						WHEN LAG_PERIOD = 75 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_75,
					SUM(
					CASE
						WHEN LAG_PERIOD = 76 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_76,
					SUM(
					CASE
						WHEN LAG_PERIOD = 77 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_77,
					SUM(
					CASE
						WHEN LAG_PERIOD = 78 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_78,
					SUM(
					CASE
						WHEN LAG_PERIOD = 79 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_79,
					SUM(
					CASE
						WHEN LAG_PERIOD = 80 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_80,
					SUM(
					CASE
						WHEN LAG_PERIOD = 81 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_81,
					SUM(
					CASE
						WHEN LAG_PERIOD = 82 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_82,
					SUM(
					CASE
						WHEN LAG_PERIOD = 83 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_83,
					SUM(
					CASE
						WHEN LAG_PERIOD = 84 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_84,
					SUM(
					CASE
						WHEN LAG_PERIOD = 85 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_85,
					SUM(
					CASE
						WHEN LAG_PERIOD = 86 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_86,
					SUM(
					CASE
						WHEN LAG_PERIOD = 87 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_87,
					SUM(
					CASE
						WHEN LAG_PERIOD = 88 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_88,
					SUM(
					CASE
						WHEN LAG_PERIOD = 89 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_89,
					SUM(
					CASE
						WHEN LAG_PERIOD = 90 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_90,
					SUM(
					CASE
						WHEN LAG_PERIOD = 91 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_91,
					SUM(
					CASE
						WHEN LAG_PERIOD = 92 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_92,
					SUM(
					CASE
						WHEN LAG_PERIOD = 93 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_93,
					SUM(
					CASE
						WHEN LAG_PERIOD = 94 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_94,
					SUM(
					CASE
						WHEN LAG_PERIOD = 95 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_95,
					SUM(
					CASE
						WHEN LAG_PERIOD = 96 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_96,
					SUM(
					CASE
						WHEN LAG_PERIOD = 97 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_97,
					SUM(
					CASE
						WHEN LAG_PERIOD = 98 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_98,
					SUM(
					CASE
						WHEN LAG_PERIOD = 99 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_99,
					SUM(
					CASE
						WHEN LAG_PERIOD = 100 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_100,
					SUM(
					CASE
						WHEN LAG_PERIOD = 101 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_101,
					SUM(
					CASE
						WHEN LAG_PERIOD = 102 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_102,
					SUM(
					CASE
						WHEN LAG_PERIOD = 103 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_103,
					SUM(
					CASE
						WHEN LAG_PERIOD = 104 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_104,
					SUM(
					CASE
						WHEN LAG_PERIOD = 105 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_105,
					SUM(
					CASE
						WHEN LAG_PERIOD = 106 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_106,
					SUM(
					CASE
						WHEN LAG_PERIOD = 107 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_107,
					SUM(
					CASE
						WHEN LAG_PERIOD = 108 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_108,
					SUM(
					CASE
						WHEN LAG_PERIOD = 109 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_109,
					SUM(
					CASE
						WHEN LAG_PERIOD = 110 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_110,
					SUM(
					CASE
						WHEN LAG_PERIOD = 111 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_111,
					SUM(
					CASE
						WHEN LAG_PERIOD = 112 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_112,
					SUM(
					CASE
						WHEN LAG_PERIOD = 113 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_113,
					SUM(
					CASE
						WHEN LAG_PERIOD = 114 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_114,
					SUM(
					CASE
						WHEN LAG_PERIOD = 115 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_115,
					SUM(
					CASE
						WHEN LAG_PERIOD = 116 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_116,
					SUM(
					CASE
						WHEN LAG_PERIOD = 117 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_117,
					SUM(
					CASE
						WHEN LAG_PERIOD = 118 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_118,
					SUM(
					CASE
						WHEN LAG_PERIOD = 119 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_119,
					SUM(
					CASE
						WHEN LAG_PERIOD = 120 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_120,
					SUM(
					CASE
						WHEN LAG_PERIOD = 121 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_121,
					SUM(
					CASE
						WHEN LAG_PERIOD = 122 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_122,
					SUM(
					CASE
						WHEN LAG_PERIOD = 123 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_123,
					SUM(
					CASE
						WHEN LAG_PERIOD = 124 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_124,
					SUM(
					CASE
						WHEN LAG_PERIOD = 125 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_125,
					SUM(
					CASE
						WHEN LAG_PERIOD = 126 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_126,
					SUM(
					CASE
						WHEN LAG_PERIOD = 127 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_127,
					SUM(
					CASE
						WHEN LAG_PERIOD = 128 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_128,
					SUM(
					CASE
						WHEN LAG_PERIOD = 129 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_129,
					SUM(
					CASE
						WHEN LAG_PERIOD = 130 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_130,
					SUM(
					CASE
						WHEN LAG_PERIOD = 131 THEN STRAIGHT_LINE_RATE
					END) AS STRAIGHT_LINE_RATE_131,
					SUM(
					CASE
						WHEN LAG_PERIOD = 132 THEN STRAIGHT_LINE_RATE
...
***********************************************************************************/
	...
	BEGIN
		...
		SELECT TCDP.REPORT_TYPE,
			...
			SYSDATE     AS ED_UPDATE_DATE,
			l_PROC_NAME AS ED_UPDATE_ID
		FROM TEMP_030_CS_CF_DLR_PVT TCDP,
			TEMP_030_CS_RATE_PVT TCRP
		WHERE TCDP.GL_ACCOUNT      = TCRP.GL_ACCOUNT
...
***********************************************************************************/
	...
	BEGIN
		...
		SELECT DISTINCT COUNTRY_INDICATOR,
			TRANSACTION_YEAR,
			TRANSACTION_MONTH,
			...
			l_PROC_NAME                                                                                                                                            AS ED_UPDATE_ID
		FROM DBO.SY_030_COST_FLOW_STG
		WHERE TRANSACTION_YEAR < TO_CHAR(SYSDATE, 'YYYY') ;
		COMMIT;
		l_level := 2;
	EXCEPTION
	WHEN OTHERS THEN
...
***********************************************************************************/
	...
	BEGIN
		...
		SELECT DISTINCT country_indicator,
			...
			SYSDATE     AS ED_UPDATE_DATE,
			l_PROC_NAME AS ED_UPDATE_ID
		FROM DM_030_SEASONALIZATION
		CROSS JOIN sy_030_year
		WHERE transaction_month BETWEEN 1 AND 12
		AND GENERATED_YEAR >= TO_CHAR(SYSDATE, 'YYYY')
		ORDER BY country_indicator,
			generated_year,
			transaction_month;
		COMMIT;
		l_level := 2;
...
***********************************************************************************/
	...
	BEGIN
		...
		FROM(
			...
			FROM
				(SELECT CS.report_type AS REPORT_TYPE,
					cs.gl_ACCOUNT         AS GL_ACCOUNT,
					CS.GL_ACCOUNT_DESCR,
					country_indicator      AS COUNTRY_INDICATOR,
					...
					CASE UPPER(TRIM(amortization_method))
						...
						ELSE 0
					END) amort_rate
				FROM DBO.sy_030_calc_sum_stg cs,
					(SELECT GL_ACCOUNT,
						LAG_PERIOD,
						cost_flow_rate,
						straight_line_rate
					FROM DBO.sy_030_calc_sum_stg
					WHERE country_indicator = 'USA'
					) USR,
					DBO.ud_030_account_rules ar
				WHERE cs.gl_account = ar.GL_ACCOUNT
				AND CS.GL_ACCOUNT   = USR.GL_ACCOUNT
			...
			WHERE RR.GL_ACCOUNT                                          = B.GL_ACCOUNT
			...
			AND TO_CHAR(ADD_MONTHS(B.MONTH, RR.LAG_PERIOD - 1), 'YYYY') <= TO_NUMBER(TO_CHAR(
				...
				END, 'YYYY')) + 2
			UNION ALL
			SELECT DISTINCT RR.REPORT_TYPE,
				RR.GL_ACCOUNT,
				RR.GL_ACCOUNT_DESCR,
				RR.COUNTRY_INDICATOR,
				RR.WARRANTY_FIRST_YR_IND,
				...
				B.REVENUE,
				SUM(NVL((B.REVENUE * RR.AMORT_RATE), 0)) OVER(PARTITION BY RR.GL_ACCOUNT, RR.country_indicator, B.RUN_MONTH, B.MONTH, TO_CHAR(ADD_MONTHS(B.MONTH, RR.LAG_PERIOD - 1), 'YYYY')) AS FORECAST_REVENUE
			FROM
				(SELECT CS.report_type AS REPORT_TYPE,
					cs.gl_ACCOUNT         AS GL_ACCOUNT,
					CS.GL_ACCOUNT_DESCR,
		...
		WHERE PERIOD BETWEEN ADD_MONTHS(RUN_MONTH, - 132) AND ADD_MONTHS(RUN_MONTH, - 1)
		ORDER BY 2,
			6,
			7,
			8;
...
***********************************************************************************/
	...
	BEGIN
		...
		WHERE TO_CHAR(RUN_PERIOD, 'MM/DD/YYYY') = TO_CHAR(
			CASE
				WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM')
				ELSE G_RUN
			END, 'MM/DD/YYYY') ;
...
***********************************************************************************/
	...
	BEGIN
		...
		SELECT DM_030_REV_RELEASE_SEQ.NEXTVAL AS DM_030_REV_RLSE_KEY,
			REPORT_TYPE,
			GL_ACCOUNT,
			GL_ACCOUNT_DESCR,
			COUNTRY_INDICATOR,
		...
		FROM
			(SELECT A.REPORT_TYPE,
				...
				CASE
					WHEN COUNT(A.FORECAST_PERIOD) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'yyyy')) < 12 AND A.FORECAST_PERIOD <(RUN_MONTH) THEN A.MONTH_RR
					ELSE A.Total_Recognized_REVENUE
				END                           AS REC_REV_MNTHLY,
				A.Previous_Recognized_REVENUE AS REC_REV_PRE_MNTHLY,
				CASE
					WHEN A.FORECAST_PERIOD =(RUN_MONTH) THEN A. DEFERRED_REVENUE
					ELSE 0
				...
				l_PROC_NAME AS ED_UPDATE_ID,
				CASE
					WHEN TO_CHAR(A.FORECAST_PERIOD, 'MON') = 'DEC' THEN(SUM(Total_Recognized_REVENUE) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.PERIOD, 'MMYYYY'), TO_CHAR(A.FORECAST_PERIOD, 'YYYY')))
					ELSE 0
			...
			FROM
				(SELECT A.REPORT_TYPE,
					...
					REVENUE,
					FORECAST_REVENUE,
					SUM(A.FORECAST_REVENUE) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'MMYYYY')) AS MONTH_RR,
					CASE
						WHEN TO_CHAR(A.FORECAST_PERIOD, 'MON') = 'DEC' THEN SUM(A.FORECAST_REVENUE) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'YYYY'))
						ELSE 0
					END AS TOTAL_YEAR_RR,
					CASE
						WHEN A.WARRANTY_FIRST_YR_IND IS NULL THEN(
							CASE
								WHEN TO_NUMBER(TO_CHAR(A.FORECAST_PERIOD, 'YYYY')) <= TO_NUMBER(TO_CHAR(RUN_MONTH, 'YYYY')) + 2 THEN NVL(B.SEASONAL_RATE, 0)
								ELSE 1
							END)
						ELSE 0
					...
					(
					CASE
						WHEN A.FORECAST_PERIOD <(RUN_MONTH) THEN(
							CASE
								WHEN A.WARRANTY_FIRST_YR_IND IS NULL THEN(SUM(A.FORECAST_REVENUE) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'YYYY')) *
									CASE
										WHEN TO_NUMBER(TO_CHAR(A.FORECAST_PERIOD, 'YYYY')) <= TO_NUMBER(TO_CHAR(RUN_MONTH, 'YYYY')) + 2 THEN NVL(B.SEASONAL_RATE, 0)
										ELSE 1
									END)
								ELSE SUM(A.FORECAST_REVENUE) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'MMYYYY'))
							END)
						ELSE 0
					END) AS Total_Recognized_REVENUE,
					(
					CASE
						WHEN A.FORECAST_PERIOD < TRUNC(add_months((A.RUN_MONTH), - 1), 'MM') THEN(
							CASE
								WHEN A.WARRANTY_FIRST_YR_IND IS NULL THEN(SUM(A.FORECAST_REVENUE) OVER(PARTITION BY A.GL_ACCOUNT, TRUNC(add_months(A.RUN_MONTH, - 1), 'MM'), A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'YYYY')) *
									CASE
										WHEN TO_NUMBER(TO_CHAR(A.FORECAST_PERIOD, 'YYYY')) <= TO_NUMBER(TO_CHAR(RUN_MONTH, 'YYYY')) + 2 THEN NVL(B.SEASONAL_RATE, 0)
										ELSE 1
									END)
								ELSE SUM(A.FORECAST_REVENUE) OVER(PARTITION BY A.GL_ACCOUNT, TRUNC(add_months(A.RUN_MONTH, - 1), 'MM'), A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'MMYYYY'))
							END)
						ELSE 0
				...
				FROM DBO.SY_030_FORECAST_REV_STG A,
					(SELECT DISTINCT COUNTRY_INDICATOR,
						TRANSACTION_MONTH,
						SEASONAL_RATE
					FROM DBO.DM_030_SEASONALIZATION
					WHERE COUNTRY_INDICATOR = 'USA'
					AND TRANSACTION_YEAR    = TO_CHAR(SYSDATE, 'YYYY') - 1
					) B
				WHERE TO_CHAR(A.FORECAST_PERIOD, 'MM') = B.TRANSACTION_MONTH
				ORDER BY 1 DESC,
					5,
					6
				) A,
				...
				FROM
					...
					FROM
						...
						WHERE FORECAST_PERIOD >= TRUNC(A.RUN_MONTH, 'MM')
						) A
					WHERE FORECAST_PERIOD BETWEEN TRUNC(RUN_MONTH, 'MM') AND TRUNC(add_months(A.RUN_MONTH, 11), 'MM')
					...
					FROM
						...
						WHERE FORECAST_PERIOD >= TRUNC(RUN_MONTH, 'MM')
						) B
					WHERE FORECAST_PERIOD > TRUNC(add_months(RUN_MONTH, 11), 'MM')
				...
				WHERE AA.REPORT_TYPE     = BB.REPORT_TYPE
				AND AA.GL_ACCOUNT        = BB.GL_ACCOUNT
				AND AA.COUNTRY_INDICATOR = BB.COUNTRY_INDICATOR
				) DF
			WHERE A.REPORT_TYPE     = DF.REPORT_TYPE (+)
			AND A.GL_ACCOUNT        = DF.GL_ACCOUNT(+)
	...
	BEGIN
		...
		FROM(
			...
			SELECT DISTINCT RR.REPORT_TYPE,
				...
				TO_DATE(('12/01/' || TO_CHAR(ADD_MONTHS(B.MONTH, RR.LAG_PERIOD - 1), 'YYYY')), 'mm/dd/yyyy') AS FORECAST_PERIOD,
				B.COMMISSION,
				SUM(NVL((B.COMMISSION * RR.AMORT_RATE), 0)) OVER(PARTITION BY RR.GL_ACCOUNT, RR.country_indicator, B.RUN_MONTH, B.MONTH, TO_CHAR(ADD_MONTHS(B.MONTH, RR.LAG_PERIOD - 1), 'YYYY')) AS FORECAST_COMMISSION
			FROM
				(SELECT CS.report_type AS REPORT_TYPE,
					cs.gl_ACCOUNT         AS GL_ACCOUNT,
					CS.GL_ACCOUNT_DESCR,
		...
		WHERE JOURNAL_PERIOD BETWEEN ADD_MONTHS(RUN_MONTH, - 132) AND ADD_MONTHS(RUN_MONTH, - 1)
		ORDER BY 2,
			6,
			7,
			8;
...
***********************************************************************************/
	...
	BEGIN
		l_level := 1;
		DELETE
		FROM DBO.DM_030_COMM_AMORTIZATION
		WHERE TO_CHAR(RUN_PERIOD, 'MM/DD/YYYY') = TO_CHAR(
			CASE
				WHEN G_RUN IS NULL THEN TRUNC(SYSDATE, 'MM')
				ELSE G_RUN
...
***********************************************************************************/
	...
	BEGIN
		...
		SELECT DBO.DM_030_COMM_AMORT_SEQ.NEXTVAL AS DM_030_COMM_AMORT_KEY,
			REPORT_TYPE,
			GL_ACCOUNT,
			GL_ACCOUNT_DESCR,
			COUNTRY_INDICATOR,
		...
		FROM
			(SELECT A.REPORT_TYPE,
				...
				A. SEASONAL_RATE   AS SEASONLIZATION_RATE,
				CASE
					WHEN COUNT(A.FORECAST_PERIOD) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'yyyy')) < 12 AND A.FORECAST_PERIOD <(RUN_MONTH) THEN A.MONTH_COMM
					ELSE A.Total_Recognized_COMMISSION
				END                              AS COMM_AMORT_MNTHLY,
				A.Previous_Recognized_COMMISSION AS COMM_AMORT_PREV_MNTHLY,
				CASE
					WHEN A.FORECAST_PERIOD =(RUN_MONTH) THEN A. PREPAID_COMMISSION
					ELSE 0
				...
				CASE
					WHEN TO_CHAR(A.FORECAST_PERIOD, 'MON') = 'DEC' THEN(SUM(Total_Recognized_COMMISSION) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.JOURNAL_PERIOD, 'MMYYYY'), TO_CHAR(A.FORECAST_PERIOD, 'YYYY')))
					ELSE 0
			...
			FROM
				(SELECT A.REPORT_TYPE,
					...
					COMMISSION_DLR,
					FORECAST_COMMISSION,
					SUM(A.FORECAST_COMMISSION) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'MMYYYY')) AS MONTH_COMM,
					CASE
						WHEN TO_CHAR(A.FORECAST_PERIOD, 'MON') = 'DEC' THEN SUM(A.FORECAST_COMMISSION) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'YYYY'))
						ELSE 0
					...
					CASE
						WHEN A.FORECAST_PERIOD <(RUN_MONTH) THEN(
							CASE
								WHEN A.WARRANTY_FIRST_YR_IND IS NULL THEN(SUM(A.FORECAST_COMMISSION) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'YYYY')) *
									CASE
										WHEN TO_NUMBER(TO_CHAR(A.FORECAST_PERIOD, 'YYYY')) <= TO_NUMBER(TO_CHAR(RUN_MONTH, 'YYYY')) + 2 THEN NVL(B.SEASONAL_RATE, 0)
										ELSE 1
									END)
								ELSE SUM(A.FORECAST_COMMISSION) OVER(PARTITION BY A.GL_ACCOUNT, A.RUN_MONTH, A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'MMYYYY'))
							END)
						ELSE 0
					...
					CASE
						WHEN A.FORECAST_PERIOD < TRUNC(add_months(A.RUN_MONTH, - 1), 'MM') THEN(
							CASE
								WHEN A.WARRANTY_FIRST_YR_IND IS NULL THEN(SUM(A.FORECAST_COMMISSION) OVER(PARTITION BY A.GL_ACCOUNT, TRUNC(add_months(A.RUN_MONTH, - 1), 'MM'), A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'YYYY')) *
									CASE
										WHEN TO_NUMBER(TO_CHAR(A.FORECAST_PERIOD, 'YYYY')) <= TO_NUMBER(TO_CHAR(RUN_MONTH, 'YYYY')) + 2 THEN NVL(B.SEASONAL_RATE, 0)
										ELSE 1
									END)
								ELSE SUM(A.FORECAST_COMMISSION) OVER(PARTITION BY A.GL_ACCOUNT, TRUNC(add_months(A.RUN_MONTH, - 1), 'MM'), A.COUNTRY_INDICATOR, TO_CHAR(A.FORECAST_PERIOD, 'MMYYYY'))
							END)
						ELSE 0
				...
				FROM DBO.SY_030_FORECAST_COMM_STG A,
					(SELECT DISTINCT COUNTRY_INDICATOR,
						TRANSACTION_MONTH,
						SEASONAL_RATE
					FROM DBO.DM_030_SEASONALIZATION
