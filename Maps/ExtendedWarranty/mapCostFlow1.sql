...
SELECT 
ROWNUM AS RN,
ROW_NUMBER() OVER (PARTITION BY report_type,    country_indicator 
order by report_type,country_indicator)   AS REPORT_TYPE_NO ,
report_type,
ROW_NUMBER() OVER (PARTITION BY gl_account, gl_account_description 
order by  gl_account, gl_account_description)   AS GL_ACCOUNT_NO,
gl_account,

ROW_NUMBER() OVER (PARTITION BY gl_account, gl_account_description 
order by  gl_account, gl_account_description)   AS gl_account_description_no,
gl_account_description,

 ROW_NUMBER() OVER (PARTITION BY COUNTRY_INDICATOR  
order by country_indicator) AS COUNTRY_INDICATOR_NO,
