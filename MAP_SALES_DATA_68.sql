...
SELECT
	...
	INNER JOIN OTR_PROD_CODE_XREF_RCPO PX ON PS.PLNT_GL_PROD = PX.MANF_PROD_CODE AND PS.PLNT_GL_BU = PX.GL_LEDGER
	LEFT OUTER JOIN
		/* R12_TRANE_LOCATIONS */
		R12_LOCATION_STG DP ON DP.R12_LOCATION = PS.R12_LOCATION
	WHERE
...
SELECT
	...
	FROM
		R12_JRNL_LN_PS L
	INNER JOIN R12_JRNL_HEADER_PS GA      ON GA.BUSINESS_UNIT = L.BUSINESS_UNIT AND GA.JOURNAL_ID = L.JOURNAL_ID AND GA.JOURNAL_DATE = L.JOURNAL_DATE AND GA.UNPOST_SEQ = L.UNPOST_SEQ
	INNER JOIN R12_ACCOUNT_FILTER_UPD AFU ON AFU.R12_ACCOUNT = L.R12_ACCOUNT
	LEFT OUTER JOIN
		/* R12_TRANE_PRODUCTS_PS */
		R12_PRODUCT_STG PR ON L.R12_PRODUCT = PR.R12_PRODUCT
	LEFT OUTER JOIN
		/* R12_TRANE_LOCATIONS */
		R12_LOCATION_STG DP                  ON L.R12_LOCATION = DP.R12_LOCATION
	INNER JOIN OTR_PROD_CODE_XREF_RCPO PX ON L.PS_PRODUCT = PX.MANF_PROD_CODE AND L.BUSINESS_UNIT = PX.GL_LEDGER
	WHERE
...
SELECT
	...
	LEFT OUTER JOIN
		/* R12_TRANE_PRODUCTS_PS */
		R12_PRODUCT_STG PR ON PS.R12_PRODUCT = PR.R12_PRODUCT
	LEFT OUTER JOIN
		/* R12_TRANE_LOCATIONS */
		R12_LOCATION_STG DP                  ON PS.R12_LOCATION = DP.R12_LOCATION
	INNER JOIN OTR_PROD_CODE_XREF_RCPO PX ON PS.PS_PRODUCT = PX.MANF_PROD_CODE AND PS.BUSINESS_UNIT = PX.GL_LEDGER
	WHERE
...
SELECT
		...
		RS_LEDGER.R12_ACCOUNT AS GL_ACCOUNT,
		RS_LEDGER.R12_LOCATION AS DEPT_ID,
		/* DP.DESCR */
		DP.R12_LOCATION_DESCR AS DEPT_DESCR,
		RS_LEDGER.PS_PRODUCT_ID AS MANF_PROD_ID,
		/* PR.DESCR */
		PR.R12_PRODUCT_DESCR AS MANF_PROD_DESCR,
		RS_LEDGER.R12_PRODUCT AS DIST_GL_PRODUCT,
	...
	LEFT OUTER JOIN
		/* R12_TRANE_PRODUCTS_PS */
		R12_PRODUCT_STG PR ON RS_LEDGER.R12_PRODUCT = PR.R12_PRODUCT
	LEFT OUTER JOIN
		/* R12_TRANE_LOCATIONS */
		R12_LOCATION_STG DP                  ON RS_LEDGER.R12_LOCATION = DP.R12_LOCATION
	...
	FROM
		(
			SELECT
				...
				LEFT OUTER JOIN
					/* R12_TRANE_PRODUCTS_PS */
					R12_PRODUCT_STG PR ON D.R12_PRODUCT = PR.R12_PRODUCT
				LEFT OUTER JOIN
					/* R12_TRANE_LOCATIONS */
					R12_LOCATION_STG DP ON D.R12_LOCATION = DP.R12_LOCATION
				WHERE
	...
	FROM
		(
			SELECT
				...
				LEFT OUTER JOIN
					/* R12_TRANE_LOCATIONS */
					R12_LOCATION_STG DP ON DP.R12_LOCATION = D.R12_LOCATION
				WHERE
					D.JOURNAL_DATE BETWEEN TO_DATE('03/01/2006', 'MM/DD/YYYY') AND LAST_DAY(ADD_MONTHS(SYSDATE, - 1))
	...
	FROM
		(
			SELECT
				...
				FROM
					R12_BI_LINE_STG A
				INNER JOIN R12_BI_ACCT_ENTRY_STG D    ON D.LINE_SEQ_NUM = A.LINE_SEQ_NUM AND D.INVOICE = A.INVOICE AND D.BUSINESS_UNIT = A.BUSINESS_UNIT AND D.CUSTOMER_TRX_ID = A.CUSTOMER_TRX_ID
				INNER JOIN R12_ACCOUNT_FILTER_UPD AFU ON AFU.R12_ACCOUNT = D.R12_ACCOUNT
				INNER JOIN R12_BI_HDR_STG BH          ON 1 = 1 AND D.INVOICE = BH.INVOICE AND D.BUSINESS_UNIT = BH.PS_BUSINESS_UNIT AND D.CUSTOMER_TRX_ID = BH.CUSTOMER_TRX_ID
				INNER JOIN R12_TRNBI_BI_HDR_STG TBH   ON D.INVOICE = TBH.INVOICE AND D.BUSINESS_UNIT = TBH.BUSINESS_UNIT AND D.CUSTOMER_TRX_ID = TBH.CUSTOMER_TRX_ID
				INNER JOIN OTR_PROD_CODE_XREF_RCPO X  ON A.MANF_PROD_ID = X.MANF_PROD_CODE
