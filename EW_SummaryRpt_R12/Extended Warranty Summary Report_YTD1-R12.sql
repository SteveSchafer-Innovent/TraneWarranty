...
SELECT  /*+ NO_CPU_COSTING */ 
 ...
 CASE WHEN sales.COUNTRY_INDICATOR IS NULL 
      ...
      ELSE sales.COUNTRY_INDICATOR END AS COUNTRY_INDICATOR,
 CAST(begbalances.ACCOUNT AS NUMBER)ACCOUNT,
 CASE WHEN sales.DESCR IS NULL
      THEN begbalances.DESCR
      ELSE sales.DESCR END AS GL_ACC_DESCR,
 (NVL(begbalances.begbal_base,0)) * -1 AS Begning_Balance,
 (NVL(perioddata.EndBal_base,0))  * -1 AS END_Blance,
 CASE WHEN sales.REVENUE_AMOUNT IS NULL
    THEN 0 ELSE sales.REVENUE_AMOUNT END AS REVENUE_AMOUNT ,
...
/*TAY:       A.ACCOUNT, PSA.DESCR, SUM(A.MONETARY_AMOUNT *-1 ) AS REVENUE_AMOUNT WIP*/
       A.PS_ACCOUNT Account, PSA.DESCR, SUM(A.MONETARY_AMOUNT *-1 ) AS REVENUE_AMOUNT
...
/*TAY:      GROUP BY ASX.NATION_CURR, A.ACCOUNT, PSA.DESCR*/
     ...
     (SELECT B.gl_account        AS account ,
      ...
      FROM (SELECT a.gl_account, A.GL_ACCOUNT_DESCR, to_date('1-'||:RunDate,'dd-mon-yy'),
             CASE WHEN TRUNC(TO_DATE(TO_DATE('1-'||:RunDate,'dd-mon-yy')),'YEAR') = To_date('1-'||:RunDate,'dd-mon-yy')
                  THEN CASE WHEN A.FORECAST_PERIOD = to_date('1-'||:RunDate,'dd-mon-yy') 
                            THEN MAX(A.DEFERRED_REVENUE) ELSE 0 END
                  ELSE (MAX(a.rec_rev_mnthly) + CASE WHEN A.FORECAST_PERIOD = to_date('1-'||:RunDate,'dd-mon-yy')
                  THEN MAX(A.DEFERRED_REVENUE) ELSE 0 END) END AS DEFERRED_REVENUE,
...
/*TAY:              AND a.gl_account LIKE '5%'*/
              ...
              AND A.SHIP_PERIOD >=
                   CASE WHEN to_date('1-'||:RunDate,'dd-mon-yy') = TRUNC(TO_DATE(TO_DATE('1-'||:RunDate,'dd-mon-yy')),'YEAR')
      ...
      GROUP BY gL_ACCOUNT, B.GL_ACCOUNT_DESCR
     ) Rev
