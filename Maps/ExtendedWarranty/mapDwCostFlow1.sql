CREATE OR REPLACE VIEW MAP_COSTFLOW1_VW AS 
SELECT
		ROWNUM AS RN,
		ROW_NUMBER() OVER(PARTITION BY REPORT_TYPE, COUNTRY_INDICATOR ORDER BY REPORT_TYPE, COUNTRY_INDICATOR) AS REPORT_TYPE_NO,
		REPORT_TYPE,
		ROW_NUMBER() OVER(PARTITION BY GL_ACCOUNT, GL_ACCOUNT_DESCRIPTION ORDER BY GL_ACCOUNT, GL_ACCOUNT_DESCRIPTION) AS GL_ACCOUNT_NO,
		GL_ACCOUNT,
		ROW_NUMBER() OVER(PARTITION BY GL_ACCOUNT, GL_ACCOUNT_DESCRIPTION ORDER BY GL_ACCOUNT, GL_ACCOUNT_DESCRIPTION) AS GL_ACCOUNT_DESCRIPTION_NO,
		GL_ACCOUNT_DESCRIPTION,
		ROW_NUMBER() OVER(PARTITION BY COUNTRY_INDICATOR ORDER BY COUNTRY_INDICATOR) AS COUNTRY_INDICATOR_NO,
	...
	WHERE
		SHIP_YEAR >= 2000
		--AND report_type IN ('13-63',
		--'16-126')
		--AND country_indicator IN ( 'USA','CAN')
	ORDER BY
		ROWNUM,
		REPORT_TYPE,
		GL_ACCOUNT,
