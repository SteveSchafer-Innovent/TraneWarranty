		...
		(
			SELECT
				...
				LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO X
					ON A.MANF_PROD_ID = X.MANF_PROD_CODE AND X.GL_LEDGER = 'CSD'
					-- GL_LEDGER filter must be in ON clause for outer join to work properly
				...
				WHERE
					...
					-- AND BH.BILL_SOURCE_ID IN ('P21', 'ORDER ENTRY', 'TCS MANUAL')
