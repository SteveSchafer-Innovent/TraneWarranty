...
UNION
  ...
  SELECT
    ...
    0 AS EXPENSE_AMT_NOT_IN_RES
  FROM WC_MAT_LBR_ROLLUP MLR
    INNER JOIN EXPENSE_TYPE_SCD ET 0N MLR.EXPENSE_TYPE_SCD_KEY = ET.EXPENSE_TYPE_SCD_KEY
    LEFT OUTER JOIN DM_FAL_CLAIMS_WARRANTY_XRF FCW 0N 
        MLR.CLAIM_NBR = FCW.CLAIM_NBR
        AND MLR.DETAIL_NBR = FCW.DETAIL_NBR
        AND MLR.STEP_NBR = FCW.STEP_NBR
    INNER JOIN TIME_DAY TD3 0N MLR.CCN_TRX_DATE_KEY = TD3.TIME_KEY /* TD3 FOR TRX DATE */
    INNER JOIN TIME_DAY TD2 0N MLR.ORIGINAL_SHIP_DATE_KEY = TD2.TIME_KEY    /* TD2 FOR ORIGINAL SHIP DATE */
    INNER JOIN TIME_DAY TD1 0N MLR.FAIL_DATE_KEY = TD1.TIME_KEY    /* TD1 FOR FAIL DATE */
    INNER JOIN TIME_DAY TD 0N MLR.START_DATE_KEY = TD.TIME_KEY    /* TD FOR START DATE */
    INNER JOIN CLAIM_TASK_SCD CTASKS 0N MLR.CLAIM_TASK_SCD_KEY = CTASKS.CLAIM_TASK_SCD_KEY
    INNER JOIN CLAIM_TYPE_SCD CTYPES 0N MLR.CLAIM_TYPE_SCD_KEY = CTYPES.CLAIM_TYPE_SCD_KEY
    /* -SS- */
    INNER JOIN R12_GL_ACCOUNT_SCD GLA 0N MLR.GL_ACCOUNT_SCD_KEY = GLA.GL_ACCOUNT_SCD_KEY
    INNER JOIN EXPENSE_TYPE_SCD ETS 0N MLR.EXPENSE_TYPE_SCD_KEY = ETS.EXPENSE_TYPE_SCD_KEY
    INNER JOIN PROD_CODE_SCD PCS 0N MLR.PROD_CODE_SCD_KEY = PCS.PROD_CODE_SCD_KEY
    INNER JOIN CUST_ACCOUNT_SCD CACCT 0N MLR.CUST_ACCOUNT_SCD_KEY = CACCT.CUST_ACCOUNT_SCD_KEY
    INNER JOIN SUBMIT_OFFICE_SCD SOS 0N MLR.SUBMIT_OFFICE_SCD_KEY = SOS.SUBMIT_OFFICE_SCD_KEY
    INNER JOIN PROD_CODE_XREF_RCPO_DR PRODGRP  0N 
        PCS.PROD_CODE = PRODGRP.MANF_PROD_CODE
        AND GLA.R12_ENTITY = PRODGRP.GL_LEDGER    /* -SS- COMPANY */
    /* -SS- ,ACTUATE_SEC_XREF ASX */
    INNER JOIN DM_WAR_CSN_RSV_PCT_REF RES_PCT 0N 
        CTYPES.CLAIM_TYPE_DESCR = RES_PCT.CLAIM_TYPE
        AND ETS.EXPENSE_TYPE_DESCR = RES_PCT.EXPENSE_TYPE_DESCR
        AND SOS.COMPANY_OWNED_IND = RES_PCT.COMPANY_OWNED_IND
    LEFT OUTER JOIN DM_WAR_CSN_RSV_PCT_REF RES_PCT1 0N mlr.CLAIM_NBR = a.CLAIM_NUMBER 
...
UNION ALL
  ...
  FROM
    ...
    /* CONCESSION CLAIM TYPE */
    INNER JOIN (SELECT DISTINCT CLAIM_NBR, STEP_NBR, WA_POLICY_TYPE, WA_RANGE FROM DM_FAL_CLAIMS_WARRANTY_XRF
    ) FCW ON 
        CCN_DATA.CLAIM_NBR = FCW.CLAIM_NBR (+)
        AND CCN_DATA.STEP_NBR = FCW.STEP_NBR(+)
        --AND CCN_DATA.DETAIL_NBR=FCW.DETAIL_NBR
    ...
    INNER JOIN TIME_DAY TD1 0N CCN_DATA.FAIL_DATE_KEY = TD1.TIME_KEY /* TD1 FOR FAIL DATE */
    INNER JOIN TIME_DAY TD 0N CCN_DATA.START_DATE_KEY = TD.TIME_KEY /* TD FOR START DATE */
    INNER JOIN R12_GL_ACCOUNT_SCD GLA 0N CCN_DATA.GL_ACCOUNT_SCD_KEY = GLA.GL_ACCOUNT_SCD_KEY
    /* -SS- */
    --,EXPENSE_TYPE_SCD ETS
    INNER JOIN PROD_CODE_SCD PCS 0N CCN_DATA.PROD_CODE_SCD_KEY = PCS.PROD_CODE_SCD_KEY
    INNER JOIN CUST_ACCOUNT_SCD CACCT 0N CCN_DATA.CUST_ACCOUNT_SCD_KEY = CACCT.CUST_ACCOUNT_SCD_KEY
    ...
    /* -SS- ,ACTUATE_SEC_XREF ASX */
    INNER JOIN PROD_CODE_XREF_RCPO_DR PRODGRP 0N 
        /* FOR RESERVE GROUP */  /* -SS- COMPANY */
          GLA.R12_ENTITY = PRODGRP.GL_LEDGER
          AND PCS.PROD_CODE = PRODGRP.MANF_PROD_CODE
    INNER JOIN DM_WAR_CSN_RSV_PCT_REF RES_PCT 0N 
          CCN_DATA.EXPENSE_TYPE_DESCR = RES_PCT.EXPENSE_TYPE_DESCR
          AND CCN_DATA.EXPENSE_TYPE_CATG = UPPER(RES_PCT.EXPENSE_TYPE_CATG)
