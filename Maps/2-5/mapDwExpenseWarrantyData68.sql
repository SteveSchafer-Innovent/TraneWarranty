...
FROM
	...
	INNER JOIN WC_LABOR_ROLLUP LR
	ON MLR.CCN_TRX_DATE_KEY = LR.CCN_TRX_DATE_KEY
	AND MLR.DETAIL_NBR      = LR.DETAIL_NBR
	AND MLR.CLAIM_NBR       = LR.CLAIM_NBR
	INNER JOIN TIME_DAY TD
	ON TD.TIME_KEY  = MLR.CCN_TRX_DATE_KEY
...
ON AFU.R12_ACCOUNT = GLA.R12_ACCOUNT
	-- -SS- /NEW
				WHERE LR1.GL_ACCOUNT_SCD_KEY = GLA.GL_ACCOUNT_SCD_KEY
	-- -SS- NEW
AND((GLA.PS_ACCOUNT = 'NA'
AND(AFU.LIKE_0620   = 'Y'
OR AFU.LIKE_8062    = 'Y'))
...
OR GLA.PS_ACCOUNT LIKE '8062%')
	...
	ON MLR.CCN_TRX_DATE_KEY = LR2.CCN_TRX_DATE_KEY
...
ON GLA.R12_ENTITY = PRODGRP.GL_LEDGER
	-- -SS- COMPANY
...
ON A.CLAIM_TYPE = RES_PCT1.CLAIM_TYPE
WHERE
...
UNION
	...
	INNER JOIN PROD_CODE_XREF_RCPO_DR PRODGRP
	ON PCS.PROD_CODE   = PRODGRP.ORA_PRODUCT
	AND GLA.R12_ENTITY = PRODGRP.GL_LEDGER -- -SS- MANF_PROD_CODE, COMPANY, FIXME neither join with work
		-- -SS- ,ACTUATE_SEC_XREF ASX
	INNER JOIN DM_WAR_CSN_RSV_PCT_REF RES_PCT
...
UNION ALL
	...
	AND CCN_DATA.STEP_NBR = FCW.STEP_NBR
		--AND CCN_DATA.DETAIL_NBR=FCW.DETAIL_NBR
	INNER JOIN TIME_DAY TD3
	ON CCN_DATA.CCN_TRX_DATE_KEY = TD3.TIME_KEY -- /* TD3 FOR TRX DATE */
	INNER JOIN TIME_DAY TD2
	ON CCN_DATA.ORIGINAL_SHIP_DATE_KEY = TD2.TIME_KEY -- /* TD2 FOR ORIGINAL SHIP DATE */
	INNER JOIN TIME_DAY TD1
	ON CCN_DATA.FAIL_DATE_KEY = TD1.TIME_KEY --  /* TD1 FOR FAIL DATE */
	INNER JOIN TIME_DAY TD
	ON CCN_DATA.START_DATE_KEY = TD.TIME_KEY --   /* TD FOR START DATE */
	INNER JOIN R12_GL_ACCOUNT_SCD GLA
	ON CCN_DATA.GL_ACCOUNT_SCD_KEY = GLA.GL_ACCOUNT_SCD_KEY --   /* -SS- */
		--,EXPENSE_TYPE_SCD ETS
	INNER JOIN PROD_CODE_SCD PCS
	ON CCN_DATA.PROD_CODE_SCD_KEY = PCS.PROD_CODE_SCD_KEY
	INNER JOIN CUST_ACCOUNT_SCD CACCT
	...
	ON CCN_DATA.SUBMIT_OFFICE_SCD_KEY = SOS.SUBMIT_OFFICE_SCD_KEY
		-- -SS- ,ACTUATE_SEC_XREF ASX
	INNER JOIN PROD_CODE_XREF_RCPO_DR PRODGRP
	ON GLA.R12_ENTITY = PRODGRP.GL_LEDGER   -- FOR RESERVE GROUP -- -SS- COMPANY
	AND PCS.PROD_CODE = PRODGRP.ORA_PRODUCT -- -SS- MANF_PROD_CODE, FIXME join won't work
	INNER JOIN DM_WAR_CSN_RSV_PCT_REF RES_PCT
	ON CCN_DATA.EXPENSE_TYPE_DESCR = RES_PCT.EXPENSE_TYPE_DESCR
	AND CCN_DATA.EXPENSE_TYPE_CATG = UPPER(RES_PCT.EXPENSE_TYPE_CATG)
