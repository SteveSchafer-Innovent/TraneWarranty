SELECT
	...
	(
	CASE
		WHEN PS.PART_TYPE = 'Y' AND PS.PARTS_PROD_CODE_IND = 'PCR' THEN '804900' -- -SS- ????
...
AND PS.PLNT_GL_PROD = PX.MANF_PROD_CODE -- SS- ???? missing PLNT_GL_PROD on R12_ORACLE_PS_REV_RCPO, issue 55

LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_PS
ON PS.PS_GL_DPT_ID != 'NA' AND PS.PS_GL_DPT_ID = AOL_PS.DEPT_ID AND PS.PS_GL_BU_ID != 'NA' AND PS.PS_GL_BU_ID = AOL_PS.BU_UNIT -- -SS- ???? issue 73
LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_R12
ON AOL_R12.ORA_LOCATION IS NOT NULL AND PS.R12_LOCATION = AOL_R12.ORA_LOCATION AND AOL_R12.ORA_ENTITY IS NOT NULL AND PS.R12_ENTITY = AOL_R12.ORA_ENTITY -- -SS- ???? issue 73
...
GROUP BY PS.GL_BU_ID,
	...
	(
	CASE
		WHEN PS.PART_TYPE = 'Y' AND PS.PARTS_PROD_CODE_IND = 'PCR' -- -SS- ???? missing columns issue 55
		THEN '804900' -- -SS- ????
		ELSE PS.R12_PRODUCT -- -SS- GL_PROD
	END),
...
AND GA.UNPOST_SEQ   = L.UNPOST_SEQ -- -SS- OTR
	-- -SS- NEW
INNER JOIN R12_ACCOUNT_FILTER_UPD AFU
ON AFU.R12_ACCOUNT = L.R12_ACCOUNT
	-- -SS- /NEW
LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR
ON L.R12_PRODUCT = PR.R12_PRODUCT -- -SS- OTR, PRODUCT, PRODUCT
LEFT OUTER JOIN OTR_TRANE_DEPTS_PS DP
ON L.R12_LOCATION = DP.DEPTID -- -SS- DEPTID -> R12_LOCATION, ???? issue 69
LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO PX
...
ON PS.R12_PRODUCT = PR.R12_PRODUCT
	...
	-- -SS- OTR
LEFT OUTER JOIN OTR_TRANE_DEPTS_PS DP
ON PS.R12_LOCATION = DP.DEPTID -- -SS- DEPTID, ???? issue 69
LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO PX
...
SELECT
	...
	ASX.NATION_CURR                                                                                 AS COUNTRY_INDICATOR
FROM R12_COM_SALES_RS_LEDGER RS_LEDGER -- -SS- OTR
	-- -SS- NEW
INNER JOIN R12_ACCOUNT_FILTER_UPD AFU
ON AFU.R12_ACCOUNT = RS_LEDGER.R12_ACCOUNT
	-- -SS- /NEW
LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR
ON RS_LEDGER.R12_PRODUCT = PR.R12_PRODUCT -- -SS- PRODUCT_ID, PRODUCT, OTR
LEFT OUTER JOIN OTR_TRANE_DEPTS_PS DP
ON RS_LEDGER.R12_LOCATION = DP.DEPTID -- -SS- DEPT_ID, (+)
LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO PX
ON RS_LEDGER.PRODUCT_ID     = PX.MANF_PROD_CODE
AND RS_LEDGER.BUSINESS_UNIT = PX.GL_LEDGER
LEFT OUTER JOIN ACTUATE_SEC_XREF ASX
ON RS_LEDGER.BUSINESS_UNIT = ASX.PSGL
WHERE PX.TWO_FIVE          = 'Y'
	-- -SS- NEW
...
FROM
	...
	AND D.BUSINESS_UNIT = A.BUSINESS_UNIT -- -SS- OTR
		-- -SS- NEW
	INNER JOIN R12_ACCOUNT_FILTER_UPD AFU
	ON AFU.R12_ACCOUNT = D.R12_ACCOUNT
		-- -SS- /NEW
	LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO X
	ON A.PS_IDENTIFIER = X.MANF_PROD_CODE
	LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR
	ON D.R12_PRODUCT = PR.R12_PRODUCT -- -SS- PRODUCT, PRODUCT, OTR

	LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_PS
	ON D.PS_DEPTID = AOL_PS.DEPT_ID
	LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_R12
	ON AOL_R12.ORA_LOCATION IS NOT NULL AND PS.R12_LOCATION = AOL_R12.ORA_LOCATION
	-- -SS- ACTUATE_OFFICE_LOCATION AOL
...
FROM
	...
	ON A.PS_IDENTIFIER = X.MANF_PROD_CODE
	LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR
	ON D.R12_PRODUCT = PR.R12_PRODUCT -- -SS- OTR, PRODUCT

	LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_PS
	ON D.PS_DEPTID != 'NA' AND D.PS_DEPTID = AOL_PS.DEPT_ID
	LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_R12
	ON AOL_R12.ORA_LOCATION IS NOT NULL AND PS.R12_LOCATION = AOL_R12.ORA_LOCATION
	-- -SS- ACTUATE_OFFICE_LOCATION AOL
	-- -SS- D.DEPTID = AOL.DEPT_ID (+), issues 63, 73
