...
SELECT
	...
	-- -SS- COST_CENTER
	AS GL_DEPT
	/*New Logic by Jackie (6/12/07)Only for concession detail Report(All >548 days are ‘Not In Reserve no matter what) */
	,
	CASE
...
FROM
	...
	ON MLR.DETAIL_NBR = LR.DETAIL_NBR
	...
	AND GLA.PS_ACCOUNT  IN('710000', '806300')))
		-- -SS- NEW
	...
	OR(GLA.PS_ACCOUNT   <> 'NA'
	AND GLA.PS_ACCOUNT   = '428000'))
		-- -SS- /NEW
		-- -SS- AND GLA.ACCOUNT = '428000'
		--                 AND MLR.CLAIM_NBR IN ('8705840',
		--                                       '8667362',
		--                                       '8728663',
		--                                       '8707634',
	...
	ON GLA.GL_ACCOUNT_SCD_KEY  = MLR.GL_ACCOUNT_SCD_KEY --  /* -SS- */
	...
	INNER JOIN
		...
		AND EXISTS
			(SELECT 'X'
			FROM R12_GL_ACCOUNT_SCD -- -SS- OCD
				-- -SS- NEW
			INNER JOIN R12_ACCOUNT_FILTER_UPD AFU
			...
			AND PS_ACCOUNT  = '428000'))
				-- -SS- /NEW
				-- -SS- OLD: AND ACCOUNT = '428000'
			)
		) LR2 ON MLR.STEP_NBR = LR2.STEP_NBR
	AND MLR.DETAIL_NBR     = LR2.DETAIL_NBR
	AND MLR.CLAIM_NBR      = LR2.CLAIM_NBR
		--, CLAIM_TYPE_SCD CT
	INNER JOIN R12_GL_ACCOUNT_SCD GLA -- -SS- OCD
	ON GLA.GL_ACCOUNT_SCD_KEY  = MLR.GL_ACCOUNT_SCD_KEY
	AND GLA.GL_ACCOUNT_SCD_KEY = LR.GL_ACCOUNT_SCD_KEY
		-- -SS- NEW
	INNER JOIN R12_ACCOUNT_FILTER_UPD AFU
...
ON CCN_DATA.CLAIM_TYPE          = RES_PCT.CLAIM_TYPE
...
ON MLR.EXPENSE_TYPE_SCD_KEY = ET.EXPENSE_TYPE_SCD_KEY
	--, CLAIM_TYPE_SCD CT
INNER JOIN TIME_DAY TD3
ON MLR.CCN_TRX_DATE_KEY = TD3.TIME_KEY -- /* TD3 FOR TRX DATE */
INNER JOIN TIME_DAY TD2
ON MLR.ORIGINAL_SHIP_DATE_KEY = TD2.TIME_KEY -- /* TD2 FOR ORIGINAL SHIP DATE */
INNER JOIN TIME_DAY TD1
ON MLR.FAIL_DATE_KEY = TD1.TIME_KEY -- /* TD1 FOR FAIL DATE */
INNER JOIN TIME_DAY TD
ON MLR.START_DATE_KEY = TD.TIME_KEY -- /* TD FOR START DATE */
...
ON MLR.SUBMIT_OFFICE_SCD_KEY = SOS.SUBMIT_OFFICE_SCD_KEY
	-- -SS- ACTUATE_SEC_XREF ASX,
INNER JOIN DM_WAR_CSN_RSV_PCT_REF RES_PCT
ON ETS.EXPENSE_TYPE_DESCR = RES_PCT.EXPENSE_TYPE_DESCR
AND SOS.COMPANY_OWNED_IND = RES_PCT.COMPANY_OWNED_IND
	-- -SS- NEW
INNER JOIN R12_ACCOUNT_FILTER_UPD AFU
ON AFU.R12_ACCOUNT = GLA.R12_ACCOUNT
...
AND GLA.PS_ACCOUNT  IN('710000', '428000', '806300')))
	-- -SS- /NEW
	-- -SS- OLD: AND (GLA.ACCOUNT IN('710000', '428000', '806300'))
	--       AND MLR.CLAIM_NBR IN ('8705840',
	--                             '8667362',
	--                             '8728663',
	--                             '8707634',
