...
-- order by 1, 2, 3, 4, 5
	...
	LEFT OUTER JOIN
		(	SELECT DISTINCT EXPENSE_TYPE_DESCR, EXPENSE_TYPE_CATG, COMPANY_OWNED_IND, RESERVE_PCT
			FROM DM_WAR_CSN_RSV_PCT_REF
			WHERE CUST_CREDIT_CATG_CODE = 'Y' -- not RIGHT...
		) RES_PCT ON UPPER(EXT2.EXPENSE_TYPE_DESCR) = UPPER(RES_PCT.EXPENSE_TYPE_DESCR) 
