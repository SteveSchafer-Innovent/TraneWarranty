...
FROM
  ...
  INNER JOIN (SELECT DISTINCT CLAIM_NBR,
    ...
    AND EXISTS
      ( SELECT 'X' FROM R12_GL_ACCOUNT_SCD /* -SS- */ WHERE LR1.GL_ACCOUNT_SCD_KEY = GL_ACCOUNT_SCD_KEY
      AND(R12_ACCOUNT
        /* -SS- ACCOUNT */
        LIKE '0620%'
        /* -SS- ???? */
  ...
  LEFT OUTER JOIN DM_FAL_CLAIMS_WARRANTY_XRF FCW ON 
    CCN_DATA.CLAIM_NBR = FCW.CLAIM_NBR /* CONCESSION CLAIM TYPE */
  ...
  INNER JOIN DM_WAR_CSN_RSV_PCT_REF RES_PCT ON 
...
UNION
  ...
  AND(GLA.R12_ACCOUNT  /* -SS- ACCOUNT */ LIKE '8062%'
    /* -SS- ???? */
  OR GLA.R12_ACCOUNT
    /* -SS- ACCOUNT */
    LIKE '0620%'
...
UNION ALL
  ...
  FROM
    ...
    INNER JOIN R12_GL_ACCOUNT_SCD GLA ON GLA.GL_ACCOUNT_SCD_KEY = LR.GL_ACCOUNT_SCD_KEY  /* -SS- */
    WHERE 1 = 1
    ...
    INNER JOIN TIME_DAY TD1 ON CCN_DATA.FAIL_DATE_KEY = TD1.TIME_KEY /* TD1 FOR FAIL DATE */
    INNER JOIN TIME_DAY TD ON CCN_DATA.START_DATE_KEY = TD.TIME_KEY /* TD FOR START DATE */
    INNER JOIN R12_GL_ACCOUNT_SCD GLA CCN_DATA.GL_ACCOUNT_SCD_KEY = GLA.GL_ACCOUNT_SCD_KEY
     /* -SS- */
    --,EXPENSE_TYPE_SCD ETS
    INNER JOIN PROD_CODE_SCD PCS ON CCN_DATA.PROD_CODE_SCD_KEY = PCS.PROD_CODE_SCD_KEY
    INNER JOIN CUST_ACCOUNT_SCD CACCT ON CCN_DATA.CUST_ACCOUNT_SCD_KEY = CACCT.CUST_ACCOUNT_SCD_KEY
