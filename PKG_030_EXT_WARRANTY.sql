	...
	WITH
			...
			(
				SELECT
						...
						-- -SS- issue 88: PSA.DESCR GL_ACCOUNT_DESCR,
					...
					-- -SS- NEW
					RIGHT JOIN R12_ACCOUNT_FILTER_UPD AFU ON AFU.R12_ACCOUNT = GLA.R12_ACCOUNT
					-- -SS- /NEW
					-- -SS- issue 88: RIGHT JOIN R12_TRANE_ACCOUNTS_PS PSA ON GLA.R12_ACCOUNT = PSA.R12_ACCOUNT -- -SS- R12_2_R12, OTR
		...
		SELECT
				REPORT_TYPE,
				'NA' AS PS_ACCOUNT, -- -SS- GL_ACCOUNT
				COUNTRY_INDICATOR,
				'NA' AS DNU_GL_ACCOUNT_DESCR,
				LAG_PERIOD,
				CALC_FACTOR,
				GENERATED_YEAR,
				R12_ACCOUNT
...
***********************************************************************************/
	...
	WITH
		...
		SELECT
			...
			JOIN PROD_CODE_XREF_RCPO_DR PRODGRP ON PRODGRP.PRODUCT_CATEGORY IS NOT NULL
...
-- sr not used			LEFT OUTER JOIN DM_FAL_CLAIMS_WARRANTY_XRF FCW ON MLR.CLAIM_NBR = FCW.CLAIM_NBR AND MLR.DETAIL_NBR = FCW.DETAIL_NBR AND MLR.STEP_NBR = FCW.STEP_NBR
			JOIN CLAIM_TYPE_SCD CTYPES                     ON MLR.CLAIM_TYPE_SCD_KEY = CTYPES.CLAIM_TYPE_SCD_KEY
-- SR Not used no affect			JOIN CLAIM_TASK_SCD CTASKS                     ON MLR.CLAIM_TASK_SCD_KEY = CTASKS.CLAIM_TASK_SCD_KEY
...
***********************************************************************************/
	...
	BEGIN
		...
		SELECT
				...
				CF.R12_ACCOUNT, -- -SS- GL_ACCOUNT
				AFU.DESCR AS GL_ACCOUNT_DESCRIPTION,
			...
			FROM
				DBO.SY_030_COST_FLOW_STG CF
				LEFT OUTER JOIN
				R12_ACCOUNT_FILTER_UPD AFU ON AFU.R12_ACCOUNT = CF.R12_ACCOUNT
			GROUP BY
				REPORT_TYPE,
				CF.R12_ACCOUNT, -- -SS- GL_ACCOUNT
				COUNTRY_INDICATOR,
			...
			FROM
				(
					SELECT DISTINCT
						...
						WHERE
							CFS.SHIP_YEAR BETWEEN ATR.START_SHIP_YEAR AND ATR.END_SHIP_YEAR
						GROUP BY
							CFS.REPORT_TYPE,
							CFS.R12_ACCOUNT, -- -SS- GL_ACCOUNT
							CFS.COUNTRY_INDICATOR,
	...
	BEGIN
		...
		SELECT
			...
			FROM
				...
				ON 1=1
				AND TCDP.R12_ACCOUNT = TCRP.R12_ACCOUNT -- -SS- R12_2_R12
				AND TCDP.COUNTRY_INDICATOR = TCRP.COUNTRY_INDICATOR
				LEFT OUTER JOIN
				R12_ACCOUNT_FILTER_UPD AFU
				ON 1=1
				AND AFU.R12_ACCOUNT = TCRP.R12_ACCOUNT;
		COMMIT;
		L_LEVEL := 2;
	EXCEPTION
	WHEN OTHERS THEN
...
***********************************************************************************/
	...
	BEGIN
		...
		SELECT
				...
				A.REPORT_TYPE,
--                'NA' AS PS_GL_ACCOUNT,
--                'NA' AS PS_GL_ACCOUNT_DESCR,
				A.COUNTRY_INDICATOR,
				A.WARRANTY_FIRST_YR_IND,
				A.SHIP_PERIOD,
				A.FORECAST_PERIOD,
				A.RUN_PERIOD,
				A.SHIP_PERIOD_SALES_DLR,
				A.FORECAST_REVENUE,
				A.FORECAST_MNTHLY_TOTAL,
				A.FORECAST_YR_TOTAL,
				A.SEASONLIZATION_RATE,
				A.REC_REV_MNTHLY,
				A.REC_REV_PRE_MNTHLY,
				A.REC_REV_FOR_PERIOD,
				A.DEFERRED_REVENUE,
				A.SHORT_TERM_DR,
				A.LONG_TERM_DR,
				A.ED_CREATE_DATE,
			...
			FROM
				...
				R12_ACCOUNT_FILTER_UPD AFU
				WHERE 1=1
				AND AFU.R12_ACCOUNT = A.R12_ACCOUNT;
		COMMIT;
		L_LEVEL := 2;
	EXCEPTION
	WHEN OTHERS THEN
...
--                'NA' AS PS_GL_ACCOUNT_DESCR,
				...
				A.FORECAST_PERIOD,
				A.RUN_PERIOD,
				A.SHIP_PERIOD_COMM_DLR,
				A.FORECAST_COMMISSION,
				A.FORECAST_MNTHLY_TOTAL,
				A.FORECAST_YR_TOTAL,
				A.SEASONLIZATION_RATE,
				A.COMM_AMORT_MNTHLY,
				A.COMM_AMORT_PREV_MNTHLY,
				A.COMM_AMORT_FOR_PERIOD,
				A.PREPAID_COMMISSION,
				A.SHORT_TERM_PP_COMM,
				A.LONG_TERM_PP_COMM,
				A.COMM_AMORT_TOTAL,
				A.ED_CREATE_DATE,
				A.ED_CREATE_ID,
