SELECT
	...
	LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_PS  ON PS.PS_GL_DPT_ID != 'NA' AND PS.PS_GL_DPT_ID = AOL_PS.DEPT_ID AND PS.PS_GL_BU_ID != 'NA' AND PS.PS_GL_BU_ID = AOL_PS.BU_UNIT                           -- -SS- ???? issue 73
	LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_R12 ON AOL_R12.ORA_LOCATION IS NOT NULL AND PS.R12_LOCATION = AOL_R12.ORA_LOCATION AND AOL_R12.ORA_ENTITY IS NOT NULL AND PS.R12_ENTITY = AOL_R12.ORA_ENTITY -- -SS- ???? issue 73
		 -- R12_2_R12 
...
SELECT
	...
	LEFT OUTER JOIN R12_TRANE_DEPTS_PS DP      ON L.R12_LOCATION = DP.DEPTID     -- R12_2_R12  -- -SS- DEPTID -> R12_LOCATION 
...
SELECT
	...
	LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR ON PS.R12_PRODUCT = PR.R12_PRODUCT -- R12_2_R12 
		...
		-- -SS- PRODUCT
		-- -SS- OTR
	LEFT OUTER JOIN R12_TRANE_DEPTS_PS DP      ON PS.R12_LOCATION = DP.DEPTID -- R12_2_R12 -- -SS- DEPTID, ???? issue 69
...
SELECT
	...
	LEFT OUTER JOIN R12_TRANE_DEPTS_PS DP      ON RS_LEDGER.R12_LOCATION = DP.DEPTID   -- R12_2_R12  -- -SS- DEPT_ID, (+)
	...
	FROM
		(
			SELECT
				...
				LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_R12 ON AOL_R12.ORA_LOCATION IS NOT NULL AND PS.R12_LOCATION = AOL_R12.ORA_LOCATION
					-- -SS- ACTUATE_OFFICE_LOCATION AOL
	...
	FROM
		(
			SELECT
				...
				LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_PS  ON D.PS_DEPTID != 'NA' AND D.PS_DEPTID = AOL_PS.DEPT_ID
				LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_R12 ON AOL_R12.ORA_LOCATION IS NOT NULL AND PS.R12_LOCATION = AOL_R12.ORA_LOCATION
					-- -SS- ACTUATE_OFFICE_LOCATION AOL
					-- -SS- D.DEPTID = AOL.DEPT_ID (+), issues 63, 73
				WHERE
