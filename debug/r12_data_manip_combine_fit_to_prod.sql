...
delete from R12_BI_HDR_STG where exists (select * from r12_bi_acct_entry_stg_t where R12_BI_HDR_STG.invoice = r12_bi_acct_entry_stg_t.invoice);
delete from R12_BI_LINE_STG where exists (select * from r12_bi_acct_entry_stg_t where R12_BI_LINE_STG.invoice = r12_bi_acct_entry_stg_t.invoice);
delete from R12_TRNBI_BI_HDR_STG where exists (select * from r12_bi_acct_entry_stg_t where R12_TRNBI_BI_HDR_STG.invoice = r12_bi_acct_entry_stg_t.invoice);
commit;
...
select count(*), min(journal_date),max(journal_date) from r12_bi_acct_entry_stg_t where claim_number like 'C%';
select count(*), min(journal_date),max(journal_date) from r12_bi_acct_entry_stg where claim_number like 'C%';
select count(*) from r12_bi_acct_entry_stg;
select count(*) from R12_BI_HDR_STG_T;
select count(*) from R12_BI_LINE_STG_T;
select count(*) from R12_TRNBI_BI_HDR_STG_T;

select * from r12_bi_acct_entry_stg_T;
select * from R12_BI_HDR_STG_T;
select * from R12_BI_LINE_STG_T;
select * from R12_TRNBI_BI_HDR_STG_T;
