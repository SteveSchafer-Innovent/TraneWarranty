...
SELECT
	...
	WHERE
		...
		-- -SS- 20161207
		AND PS.PLNT_GL_BU IN ('CSD', 'CAN')
		AND PX.GL_LEDGER = 'CSD'
	GROUP BY
		PS.GL_BU_ID,
		PS.R12_ACCOUNT,
		PS.R12_LOCATION,
...
select * from R12_TRANE_PRODUCTS_PS;
...
SELECT
	...
	LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR   ON L.R12_PRODUCT = PR.R12_PRODUCT
	LEFT OUTER JOIN R12_TRANE_LOCATIONS DP     ON L.R12_LOCATION = DP.R12_LOCATION
	LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO PX ON L.PS_PRODUCT = PX.MANF_PROD_CODE
	WHERE
		...
		-- -SS- 20161207
		AND L.BUSINESS_UNIT IN ('CSD', 'CAN')
		AND PX.GL_LEDGER = 'CSD'
	GROUP BY
		GA.BUSINESS_UNIT,
		L.R12_ACCOUNT,  
...
SELECT
	...
	LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR ON PS.R12_PRODUCT = PR.R12_PRODUCT -- R12_2_R12_ok  
	LEFT OUTER JOIN R12_TRANE_LOCATIONS DP     ON PS.R12_LOCATION = DP.R12_LOCATION -- R12_2_R12_ok 
	...
	WHERE
		...
		-- -SS- 20161207
		AND PS.BUSINESS_UNIT IN ('CSD', 'CAN')
		AND PX.GL_LEDGER = 'CSD'
	GROUP BY
		PS.BUSINESS_UNIT,
		PS.R12_ACCOUNT,  
...
SELECT
	...
	FROM (
			SELECT
				...
				LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO X ON D.PS_PRODUCT = X.MANF_PROD_CODE
				LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR ON D.R12_PRODUCT = PR.R12_PRODUCT
				LEFT OUTER JOIN R12_TRANE_LOCATIONS DP ON D.R12_LOCATION = DP.R12_LOCATION 
				WHERE
					...
					-- -SS- 20161207
					 AND D.BUSINESS_UNIT IN ('CSD', 'CAN')
					 AND X.GL_LEDGER = 'CSD'
		)
	GROUP BY
		BUSINESS_UNIT,
			...
			*/
			...
			SELECT
				...
				INNER JOIN R12_TRNBI_BI_HDR_PSB TBH ON 1=1
					...
					AND D.CUSTOMER_TRX_ID = TBH.CUSTOMER_TRX_ID
				LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO X ON D.PS_PRODUCT = X.MANF_PROD_CODE -- (+)
				LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR ON D.R12_PRODUCT = PR.R12_PRODUCT -- R12_2_R12_ok 
				LEFT OUTER JOIN R12_TRANE_LOCATIONS DP ON D.R12_LOCATION = DP.R12_LOCATION 
				WHERE
					D.PS_PRODUCT <> 'NA'
