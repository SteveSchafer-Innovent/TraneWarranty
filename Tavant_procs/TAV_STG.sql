...
SELECT
		...
		EXT2.CLAIM_TYPE,
		CASE WARRANTY_DURATION_CODE WHEN '<'
			THEN '<= 548 DAYS'         WHEN '>'
			THEN '> 548 DAYS'
			ELSE 'NA'
		...
		CASE WARRANTY_DURATION_CODE            WHEN '<'
			THEN '<= 548 DAYS'                    WHEN '>'
			THEN '> 548 DAYS'                     WHEN '1'
			THEN '1st Year Standard Warranty'     WHEN '2'
			THEN '2nd-5th Year Standard Warranty' WHEN '5'
			THEN '> 5th Year Standard Warranty'   WHEN '0'
		...
		EXT2.MANF_PROD_CODE,
		CASE WHEN EXT2.COMPANY_OWNED_IND <> 0
			THEN 'Y'
			ELSE 'N'
		END AS COMPANY_OWNED,
		EXT2.CUSTOMER_NUMBER,
		EXT2.CUSTOMER_NAME,
		CASE WHEN EXT2.INTERNAL_IND <> 0
			THEN 'Y'
			ELSE 'N'
		...
		CASE EXT2.WARRANTY_TYPE WHEN 'EXTENDED'
			THEN 'EXT'             WHEN 'GOODWILL'
			THEN 'GW'              WHEN 'STANDARD'
			THEN 'STD'
			ELSE 'NO'
		END AS WARRANTY_TYPE,
		EXT2.GL_EXPENSE_CURR AS CURRENCY,
		CASE        WHEN EXT2.SEGMENT1 IN('5773', '5588')
			THEN 'CAD' WHEN EXT2.SEGMENT1 IN('5575', '5612', '5743', '9256', '9258', '9298', '9299', '9984')
			THEN 'USD'
			ELSE 'other'
		END AS COUNTRY_INDICATOR,
		'unknown' AS RETROFIT_ID,
		10000 * COALESCE(RES_PCT.RESERVE_PCT, - 1) AS IN_RESERVE_PERCENT,
		10000 *(CASE WHEN(                                                                                           
							EXT2.MANF_PROD_CODE IN('0054', '0197')) 											-- PCS.PROD_CODE
	...
	FROM
		(
			...
			SELECT
					EXTRACT.*,
					-- CLAIM_TYPE
					CASE COMMERCIAL_POLICY WHEN 0
						THEN
							CASE TAVANT_CLAIM_TYPE WHEN 'MACHINE'
								THEN
									CASE WARRANTY_TYPE 
									...
									END WHEN 'CAMPAIGN'
								THEN
									CASE -- NOT SURE ABOUT THIS - LOCAL PURCHASE?
										WHEN(UPPER(COST_CATEGORY) IN('LABOR') OR UPPER(COST_CATEGORY) LIKE 'TRAVEL%') 
											THEN 'RETROFIT LABOR'
	...
	LEFT OUTER JOIN
		(SELECT DISTINCT EXPENSE_TYPE_DESCR, EXPENSE_TYPE_CATG, COMPANY_OWNED_IND, RESERVE_PCT FROM DM_WAR_CSN_RSV_PCT_REF
		) RES_PCT ON UPPER(EXT2.EXPENSE_TYPE_DESCR) = UPPER(RES_PCT.EXPENSE_TYPE_DESCR) AND UPPER(EXT2.MATERIAL_LABOR) = UPPER(RES_PCT.EXPENSE_TYPE_CATG) AND CASE EXT2.COMPANY_OWNED_IND WHEN 1 THEN 'Y' ELSE 'N' END = RES_PCT.COMPANY_OWNED_IND
	WHERE
		0 = 0
