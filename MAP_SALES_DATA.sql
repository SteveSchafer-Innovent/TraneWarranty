...
CREATE TABLE MAP_SALES_DATA (
	...
	"TWO_FIVE" NVARCHAR2(1)
);
COMMIT;
grant SELECT on "DBO"."MAP_SALES_DATA" to "READ_DBO" ;
*/

-- CREATE TABLE MAP_SALES_DATA_SS AS SELECT * FROM MAP_SALES_DATA;
-- DESCRIBE MAP_SALES_DATA_SS;

-- CREATE TABLE MAP_SALES_DATA AS SELECT * FROM MAP_SALES_DATA_SS;
-- COMMIT;
-- DESCRIBE MAP_SALES_DATA;

TRUNCATE TABLE MAP_SALES_DATA;
COMMIT;

-- DELETE FROM MAP_SALES_DATA WHERE QUERY_SOURCE = 'RCPO';
SELECT TO_CHAR(SYSDATE, 'MM-DD-YYYY HH24:MI:SS') "Start RCPO"  FROM DUAL;
...
-- 2,402,817 rows inserted. 673.563 SECONDS
-- 20161208 - 2,402,817 rows inserted. 483 seconds

COMMIT;
-- select count(*), max(JRNL_DATE) from MAP_SALES_DATA where QUERY_SOURCE = 'RCPO';
-- select distinct COUNTRY_INDICATOR from MAP_SALES_DATA where QUERY_SOURCE = 'RCPO';
-- 2402817	31-OCT-16

-- select * from R12_PRODUCT_STG;

-- DELETE FROM MAP_SALES_DATA WHERE QUERY_SOURCE = 'P/S GL';
SELECT TO_CHAR(SYSDATE, 'MM-DD-YYYY HH24:MI:SS') "Start P/S GL"  FROM DUAL;
INSERT /*+ APPEND */ INTO MAP_SALES_DATA_ss
SELECT
		/*+ NO_CPU_COSTING */
...
-- 79,780 rows inserted.  128.034 seconds.
-- 20161208 - 79,780 rows inserted. 114 seconds.

COMMIT;
-- select count(*), max(JRNL_DATE) from MAP_SALES_DATA where QUERY_SOURCE = 'P/S GL';
-- 79780	31-DEC-04
-- select distinct COUNTRY_INDICATOR from MAP_SALES_DATA where QUERY_SOURCE = 'P/S GL';
-- 31-DEC-04

-- DELETE FROM MAP_SALES_DATA WHERE QUERY_SOURCE = 'P/S LEDGER';
SELECT TO_CHAR(SYSDATE, 'MM-DD-YYYY HH24:MI:SS') "Start P/S LEDGER"  FROM DUAL;
INSERT /*+ APPEND */ INTO MAP_SALES_DATA_ss
SELECT
		/*+ NO_CPU_COSTING */
...
-- 5,492 rows inserted.
-- 5,492 rows inserted.  12.132 seconds
-- 20161208 - 5,492 rows inserted. 14.7 seconds

COMMIT;
-- select count(*), max(JRNL_DATE) from MAP_SALES_DATA where QUERY_SOURCE = 'P/S LEDGER';
-- 5492	15-DEC-02
-- select distinct COUNTRY_INDICATOR from MAP_SALES_DATA where QUERY_SOURCE = 'P/S LEDGER';

-- select count(*) from MAP_SALES_DATA where QUERY_SOURCE = 'PBS';
-- DELETE FROM MAP_SALES_DATA WHERE QUERY_SOURCE = 'PBS';
SELECT TO_CHAR(SYSDATE, 'MM-DD-YYYY HH24:MI:SS') "Start PBS"  FROM DUAL;
...
-- 1,173,466 rows inserted. 2530.245 seconds
-- 20161208 - 1,173,466 rows inserted. 3158 seconds (52.6 minutes)

COMMIT;
-- select count(*), max(JRNL_DATE) from MAP_SALES_DATA where QUERY_SOURCE = 'PBS';
-- 1173466	18-MAY-16
-- select distinct COUNTRY_INDICATOR from MAP_SALES_DATA where QUERY_SOURCE = 'PBS';
-- USD only
...
SELECT TO_CHAR(SYSDATE, 'MM-DD-YYYY HH24:MI:SS') "Start P21" FROM DUAL;
INSERT /*+ APPEND */ INTO MAP_SALES_DATA
SELECT
		/*+ NO_CPU_COSTING */
		'P21' AS QUERY_SOURCE,
...
-- 451,993 rows inserted. 1501.521 seconds
-- 20161208 - 451,993 rows inserted. 1757 seconds (29.3 minutes)

COMMIT;
-- select count(*), max(JRNL_DATE) from MAP_SALES_DATA where QUERY_SOURCE = 'P21';
-- 451993	06-OCT-16
-- select distinct COUNTRY_INDICATOR from MAP_SALES_DATA where QUERY_SOURCE = 'P21';
-- USD, CAD

-- select count(*) from MAP_SALES_DATA where QUERY_SOURCE = 'P21R12';
-- DELETE FROM MAP_SALES_DATA WHERE QUERY_SOURCE = 'P21R12';
SELECT TO_CHAR(SYSDATE, 'MM-DD-YYYY HH24:MI:SS') "Start P21R21" FROM DUAL;
INSERT /*+ APPEND */ INTO MAP_SALES_DATA
SELECT
		/*+ NO_CPU_COSTING */
...
-- 1,943,061 rows inserted.
-- 20161208 - 2,429,689 rows inserted. 1169 seconds (19.5 minutes)

COMMIT;
-- select count(*), max(JRNL_DATE) from MAP_SALES_DATA where QUERY_SOURCE = 'PUEBLO';
-- 1943061	16-AUG-15
-- select distinct COUNTRY_INDICATOR from MAP_SALES_DATA where QUERY_SOURCE = 'PUEBLO';
-- USD, CAD

-- select QUERY_SOURCE, count(*), max(jrnl_date) from MAP_SALES_DATA group by QUERY_SOURCE;
-- select QUERY_SOURCE, JRNL_YEAR, COUNT(*), sum(REVENUE_AMOUNT) from MAP_SALES_DATA group by QUERY_SOURCE, JRNL_YEAR order by QUERY_SOURCE, JRNL_YEAR;
