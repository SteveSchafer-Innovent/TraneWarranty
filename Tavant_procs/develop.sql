...
'????' AS TRX_DATE, -- WHAT IS TRX DATE?
'????' AS TRX_YEAR,
'????' AS TRX_MONTH,
'????' AS INTMONTHS_TRX_TO_BASE, -- WHAT IS BASE DATE?
'????' AS INTMONTHS_SHIP_TO_BASE,
SHIP_DATE,
TO_NUMBER(TO_CHAR(TO_DATE(SHIP_DATE), 'YYYY')) * 100 + TO_NUMBER(TO_CHAR(TO_DATE(SHIP_DATE), 'MM')) AS SHIP_YEAR_MONTH,
...
CASE
	WHEN SEGMENT1 IN('5773', '5588') THEN 'CAD'
	WHEN SEGMENT1 IN('5575', '5612', '5743', '9256', '9258', '9298', '9299', '9984') THEN 'USD'
	ELSE '???'
