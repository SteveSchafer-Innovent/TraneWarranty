SELECT
	...
	LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_PS  ON PS.PS_GL_DPT_ID = AOL_PS.DEPT_ID AND AND null /* -SS- ???? issue 86 - PS.PS_GL_BU_ID */ = AOL_PS.BU_UNIT
	LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_R12 ON AOL_R12.ORA_LOCATION IS NOT NULL AND PS.R12_LOCATION = AOL_R12.ORA_LOCATION AND AOL_R12.ORA_ENTITY IS NOT NULL AND PS.R12_ENTITY = AOL_R12.ORA_ENTITY
		-- R12_2_R12 
...
SELECT
	...
	LEFT OUTER JOIN R12_TRANE_DEPTS_PS DP      ON L.R12_LOCATION = DP.R12_LOCATION   -- R12_2_R12_ok  -- -SS- DEPTID -> R12_LOCATION 
...
SELECT
	...
	LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR ON PS.R12_PRODUCT = PR.R12_PRODUCT -- R12_2_R12_ok 
		...
		-- -SS- PRODUCT
		-- -SS- OTR
	LEFT OUTER JOIN R12_TRANE_DEPTS_PS DP      ON PS.R12_LOCATION = DP.R12_LOCATION -- R12_2_R12_ok -- -SS- DEPTID
...
SELECT
	...
	LEFT OUTER JOIN R12_TRANE_DEPTS_PS DP      ON RS_LEDGER.R12_LOCATION = DP.R12_LOCATION   -- R12_2_R12_ok  -- -SS- DEPT_ID, (+)
		...
		(
			SELECT
				...
				INNER JOIN R12_ACCOUNT_FILTER_UPD AFU ON AFU.R12_ACCOUNT = D.R12_ACCOUNT
					-- -SS- /NEW
				LEFT OUTER JOIN OTR_PROD_CODE_XREF_RCPO X       ON A.PS_IDENTIFIER = X.MANF_PROD_CODE
				LEFT OUTER JOIN R12_TRANE_PRODUCTS_PS PR        ON D.R12_PRODUCT = PR.R12_PRODUCT  -- R12_2_R12_ok -- -SS- OTR, PRODUCT
				LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_PS  ON D.PS_DEPTID = AOL_PS.DEPT_ID
				LEFT OUTER JOIN ACTUATE_OFFICE_LOCATION AOL_R12 ON AOL_R12.ORA_LOCATION IS NOT NULL AND D.R12_LOCATION = AOL_R12.ORA_LOCATION
					-- -SS- ACTUATE_OFFICE_LOCATION AOL
					-- -SS- D.DEPTID = AOL.DEPT_ID (+), issues 63, 73
