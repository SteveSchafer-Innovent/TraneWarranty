		...
		(
			SELECT
				...
				FROM
					...
					-- -SS- NEW
				INNER JOIN R12_ACCOUNT_FILTER_UPD AFU ON AFU.R12_ACCOUNT = A.R12_ACCOUNT -- -SS- R12_2_R12
					-- -SS- /NEW
			...
			SELECT
				...
				INNER JOIN R12_BI_HDR_PSB C ON B.BUSINESS_UNIT = C.PS_BUSINESS_UNIT AND B.INVOICE = C.INVOICE AND C.ENTRY_TYPE = 'IN'
					-- -SS- NEW
				INNER JOIN R12_ACCOUNT_FILTER_UPD AFU ON AFU.R12_ACCOUNT = A.R12_ACCOUNT -- -SS- R12_2_R12
					-- -SS- /NEW
				LEFT OUTER JOIN R12_TRANE_ACCOUNTS_PS PSA
			...
			SELECT
				...
				INNER JOIN R12_BI_HDR_PSB C               ON B.BUSINESS_UNIT = C.PS_BUSINESS_UNIT AND B.INVOICE = C.INVOICE AND C.ENTRY_TYPE = 'IN'
				LEFT OUTER JOIN R12_TRANE_ACCOUNTS_PS PSA ON A.R12_ACCOUNT = PSA.R12_ACCOUNT -- -SS- R12_2_R12
					AND PSA.TRANE_ACCOUNT_IND = 'X'
					-- -SS- OTR
					/* -SS- ,
					DBO.ACTUATE_SEC_XREF ASX */
			...
			SELECT
				...
				LEFT OUTER JOIN R12_TRANE_ACCOUNTS_PS PSA ON UPD.GL_ACCOUNT = PSA.R12_ACCOUNT -- -SS- ???? issue 72, R12_2_R12
					AND PSA.TRANE_ACCOUNT_IND = 'X'
					-- -SS- OTR
					-- -SS- WHERE
