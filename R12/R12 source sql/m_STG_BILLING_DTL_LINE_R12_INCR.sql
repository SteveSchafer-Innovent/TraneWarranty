 ...
 WHERE aps.customer_trx_id(+) = rct.customer_trx_id
  ...
  -- AND RCTL.line_type <> 'TAX'
   ...
   AND msi.organization_id(+) = nvl(rctl.warehouse_id,99)
      AND (rct.last_update_date > sysdate-2
   OR rctl.last_update_date > sysdate-2
   OR rctgd.last_update_date > sysdate-2
   OR hca_billto.last_update_date > sysdate-2
   OR hca_shipto.last_update_date > sysdate-2
   OR loc_billto.last_update_date > sysdate-2
   OR loc_shipto.last_update_date > sysdate-2
   OR jrre.last_update_date > sysdate-2
   OR gjb.last_update_date > sysdate-2)