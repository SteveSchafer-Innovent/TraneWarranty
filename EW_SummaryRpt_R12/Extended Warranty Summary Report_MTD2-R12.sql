...
FROM (/* Begning Balance DRTRNP */
      SELECT  /*+ NO_CPU_COSTING */     
       Cross_Ref.PeopleSoft_ac AS ACCOUNT,
       gl_ledgers.ledger_id         AS ledger,
       gl_balances.period_name      AS fiscal_year,
       CASE WHEN gl_ledgers.ledger_id=2022 THEN 'USA' WHEN gl_ledgers.ledger_id = 2041 THEN 'CAN' END AS COUNTRY_INDICATOR,
...
/*TAY:        AND Cross_Ref.PeopleSoft_ac (+)       = PSA.ACCOUNT WIP*/
      ...
      GROUP BY Cross_Ref.PeopleSoft_ac, gl_ledgers.ledger_id, gl_balances.period_name, psa.DESCR,
               CASE WHEN gl_ledgers.ledger_id=2022 THEN 'USA' WHEN gl_ledgers.ledger_id =2041then 'CAN' END,
               psa.DESCR
                || ' - (R12 A/C : '
                ||gl_code_combinations.segment4
                ||'-'
...
GROUP BY CAST(begbalances.ACCOUNT AS NUMBER), begbalances.COUNTRY_INDICATOR,
         begbalances.begbal_base, endingbalances.EndBal_base
