	...
	FROM
		(
			SELECT
					...
					SUM(
					CASE   WHEN A.MATERIAL_LABOR LIKE'%M%' AND TRX_YEAR =
							CASE WHEN :SDO = 'Run for Retrofit End Date 6 Months Prior'
					...
					SUM(
					CASE   WHEN MATERIAL_LABOR LIKE 'LABOR%' AND TRX_YEAR =
							CASE WHEN :SDO = 'Run for Retrofit End Date 6 Months Prior'
								THEN TO_CHAR((LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE), - 1))), 'YYYY')
								ELSE :TRX_YEAR
							END AND TRX_MONTH =
		...
		(
			SELECT
					...
					RETROFIT_ID AS RETRO_ID,
					(
					CASE WHEN COUNT(DISTINCT A. GL_DEPT) > 1
