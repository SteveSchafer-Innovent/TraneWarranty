...
SELECT
  /*+ NO_CPU_COSTING */
  TRUNC(TO_DATE(TO_DATE('1-' ||:RunDate,'dd-mon-yy')),'YEAR') -1 AS gl_BeginDate, LAST_DAY(to_date('1-' ||:RunDate,'dd-mon-yy')) gl_End_Date,
  CASE
  ...
  CASE
    ...
    ELSE sales.DESCR
  END AS GL_ACC_DESCR, (NVL(begbalances.begbal_base,0)) * -1 AS Begning_Balance, (NVL(perioddata.EndBal_base,0)) * -1 AS END_Blance,
  CASE
    WHEN sales.REVENUE_AMOUNT IS NULL
    THEN 0
    ELSE sales.REVENUE_AMOUNT
  END AS REVENUE_AMOUNT , NVL(rev.DEFERRED_REVENUE,0) AS DEFERRED_REVENUE, NVL(rev.SHORT_TERM_BALA,0) AS SHORT_TERM_BALA, NVL(rev.LONG_TERM_BALA,0) AS LONG_TERM_BALA
FROM
  (SELECT
    /*+ NO_CPU_COSTING */
...
SELECT
  ...
  CAST (PSA.PS_ACCOUNT AS NUMBER)AS ACCOUNT, psa.DESCR AS GL_ACC_DESCR, 0 AS Begning_Balance, 0 AS END_Blance, 0 AS REVENUE_AMOUNT , 0 AS DEFERRED_REVENUE, 0 AS SHORT_TERM_BALA, 0 AS LONG_TERM_BALA
