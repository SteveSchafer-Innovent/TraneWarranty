	...
	FROM
		(
			SELECT
					...
					SUM (CASE
						WHEN A.MATERIAL_LABOR LIKE'%M%'
							AND TRX_YEAR = TO_CHAR((LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE), - 1))), 'YYYY')
							AND TRX_MONTH = TO_NUMBER(TO_CHAR((LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE), - 1))), 'MM'))
						THEN A.EXPENSE_AMOUNT
						ELSE 0
						END)
						AS MTD_MATERIL_EXP,
					SUM (CASE
						WHEN MATERIAL_LABOR LIKE 'LABOR%'
							AND TRX_YEAR = TO_CHAR((LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE), - 1))), 'YYYY')
							AND TRX_MONTH = TO_NUMBER(TO_CHAR((LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE), - 1))), 'MM'))
						THEN A.EXPENSE_AMOUNT
						ELSE 0
						...
						AS MTD_LABOR_EXP,
					SUM (CASE
						WHEN TRX_YEAR = TO_CHAR((LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE), - 1))), 'YYYY')
							AND TRX_MONTH = TO_NUMBER(TO_CHAR((LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE), - 1))), 'MM'))
						THEN A.EXPENSE_AMOUNT
						ELSE 0
						END)
					...
					SUM(
					CASE
						WHEN A.MATERIAL_LABOR LIKE'LABOR%'
						THEN A.EXPENSE_AMOUNT
						ELSE 0
					...
					RP.RETRO_NOTES,
					CASE
						WHEN(A.BUSINESS_UNIT = 'CSD' OR A.BUSINESS_UNIT = 'CAN')
					...
					(
						SELECT
								...
								CASE
									...
									THEN 10000 *(
										CASE
											WHEN((PCS.PROD_CODE IN('0054', '0197')) OR(MLR.SIOP_PRODUCT_CODE IS NOT NULL)) OR(RD.NEW_RESOLVED_IND = 'R' AND RD.SPECIFIC_RESERVE_IND = 'Y' OR RD.PCT_100_RECOVERY_IND = 'Y')
											THEN 0
						...
						SELECT
								...
								CASE
									...
									THEN 10000 *(
										CASE
											WHEN((PCS.PROD_CODE IN('0054', '0197')) OR(CCN_DATA.SIOP_PRODUCT_CODE IS NOT NULL)) OR(RD.NEW_RESOLVED_IND = 'R' AND RD.SPECIFIC_RESERVE_IND = 'Y' OR RD.PCT_100_RECOVERY_IND = 'Y')
											THEN 0
											ELSE RES_PCT.RESERVE_PCT
										END)
									ELSE RES_PCT.RESERVE_PCT
							...
							INNER JOIN
								...
								ON CCN_DATA.CLAIM_NBR = FCW.CLAIM_NBR
								AND CCN_DATA.STEP_NBR = FCW.STEP_NBR
							INNER JOIN
								TIME_DAY TD3
								ON CCN_DATA.CCN_TRX_DATE_KEY = TD3.TIME_KEY
							INNER JOIN
								TIME_DAY TD2
								ON CCN_DATA.ORIGINAL_SHIP_DATE_KEY = TD2.TIME_KEY
							INNER JOIN
								TIME_DAY TD1
								ON CCN_DATA.FAIL_DATE_KEY = TD1.TIME_KEY
							INNER JOIN
								TIME_DAY TD
								ON CCN_DATA.START_DATE_KEY = TD.TIME_KEY
							INNER JOIN
								R12_GL_ACCOUNT_SCD GLA
								ON CCN_DATA.GL_ACCOUNT_SCD_KEY = GLA.GL_ACCOUNT_SCD_KEY
							INNER JOIN
								PROD_CODE_SCD PCS
								ON CCN_DATA.PROD_CODE_SCD_KEY = PCS.PROD_CODE_SCD_KEY
							INNER JOIN
								CUST_ACCOUNT_SCD CACCT
								ON CCN_DATA.CUST_ACCOUNT_SCD_KEY = CACCT.CUST_ACCOUNT_SCD_KEY
							INNER JOIN
								SUBMIT_OFFICE_SCD SOS
								ON CCN_DATA.SUBMIT_OFFICE_SCD_KEY = SOS.SUBMIT_OFFICE_SCD_KEY
							INNER JOIN
								PROD_CODE_XREF_RCPO_DR PRODGRP
								ON 'CSD' = PRODGRP.GL_LEDGER
								AND PCS.PROD_CODE = PRODGRP.MANF_PROD_CODE
							INNER JOIN
								UD_031_RETROFIT_RULES RES_PCT
								ON CCN_DATA.EXPENSE_TYPE_DESCR = RES_PCT.EXPENSE_TYPE_DESCR
							...
							INNER JOIN
								UD_031_STDWTY_RSV_CLM_ADJ A
								ON CCN_DATA.CLAIM_NBR = A.CLAIM_NUMBER
							INNER JOIN
				...
				INNER JOIN
					(
						SELECT
							...
							INNER JOIN
								WC_MAT_LBR_ROLLUP MLR
								ON RP.RETRO_ID = MLR.RETRO_ID
							INNER JOIN
								SUBMIT_OFFICE_SCD SO
								ON MLR.SUBMIT_OFFICE_SCD_KEY = SO.SUBMIT_OFFICE_SCD_KEY
							INNER JOIN
								EXPENSE_TYPE_SCD ET
								ON MLR.EXPENSE_TYPE_SCD_KEY = ET.EXPENSE_TYPE_SCD_KEY
							INNER JOIN
								TIME_DAY TD
								ON MLR.CCN_TRX_DATE_KEY = TD.TIME_KEY
							WHERE
								TRUNC(TD.FULL_DATE) < (LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE), - 1)) + 1)
						...
						SELECT
							...
							INNER JOIN
								CLAIM_TASK_SCD CK
								ON MLR.CLAIM_TASK_SCD_KEY = CK.CLAIM_TASK_SCD_KEY
							INNER JOIN
								FLN_RETROFIT_PROGRAM RP
								ON RP.RETRO_ID = CK.TASK_CODE
							INNER JOIN
								TIME_DAY TD
								ON MLR.CCN_TRX_DATE_KEY = TD.TIME_KEY
							WHERE
				...
				WHERE
					...
					AND TRUNC(RP.RETRO_END_DATE) >= (LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE), - 7)))
					AND TRUNC(A.TRX_DATE) < (LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE), - 1)) + 1)
					AND A.BUSINESS_UNIT = 'CSD' OR A.BUSINESS_UNIT = 'CAN'
				GROUP BY
					A.COUNTRY_INDICATOR,
					A.GL_ACCOUNT,
					CASE
				...
				FROM
					(
						...
						SELECT
							...
							INNER JOIN
								UD_031_RETROFIT_ID RD
								ON CCN_DATA.RETRO_ID = RD.RETROFIT_ID
							LEFT OUTER JOIN
								UD_031_STDWTY_RSV_CLM_ADJ A
								ON CCN_DATA.CLAIM_NBR = A.CLAIM_NUMBER
							LEFT OUTER JOIN
